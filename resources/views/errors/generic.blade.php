...
<div class="space-y-8 flex items-center justify-center bg-gradient-to-br from-slate-50 to-slate-100 p-4">
    <div class="max-w-3xl w-full">
        ...
        <div class="bg-white/80 rounded-2xl shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300 overflow-hidden">
            ...
            <div class="bg-gradient-to-r @if($color === 'red') from-red-500 to-pink-500 @elseif($color === 'orange') from-orange-500 to-red-500 @elseif($color === 'yellow') from-yellow-500 to-orange-500 @elseif($color === 'blue') from-blue-500 to-cyan-500 @elseif($color === 'purple') from-purple-500 to-pink-500 @else from-slate-500 to-slate-600 @endif p-8 text-center relative overflow-hidden">
                <!-- Éléments décoratifs -->
                ...
                <h1 class="text-4xl font-bold text-white mb-2">{{ $statusCode }}</h1>
                <p class="@if($color === 'red') text-red-100 @elseif($color === 'orange') text-orange-100 @elseif($color === 'yellow') text-yellow-100 @elseif($color === 'blue') text-blue-100 @elseif($color === 'purple') text-purple-100 @else text-slate-100 @endif text-xl">{{ $title }}</p>
            </div>
            ...
            <div class="p-8">
                ...
                <div class="text-center mb-8">
                    <div class="inline-flex items-center px-6 py-3 bg-gradient-to-r @if($color === 'red') from-red-100 to-pink-100 text-red-800 @elseif($color === 'orange') from-orange-100 to-red-100 text-orange-800 @elseif($color === 'yellow') from-yellow-100 to-orange-100 text-yellow-800 @elseif($color === 'blue') from-blue-100 to-cyan-100 text-blue-800 @elseif($color === 'purple') from-purple-100 to-pink-100 text-purple-800 @else from-slate-100 to-gray-100 text-slate-800 @endif rounded-xl mb-6 shadow-md">
                ...
                @if($statusCode === 419 && isset($refresh_required))
                ...
                @if($statusCode === 429 && isset($retry_after))
                    <div class="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl p-6 mb-8 border border-orange-200">
                        ...
                        <p class="text-slate-600 mb-4">
                            Vous avez effectué trop de requêtes. Veuillez patienter {{ $retry_after }} secondes avant de réessayer.
                        </p>
                        <div id="countdown" class="text-2xl font-bold text-orange-600"></div>
                    </div>
                @endif

                @if($statusCode === 500 && isset($support_info))
                ...
                @if($statusCode === 404)
                    @if(isset($similar_pages) && count($similar_pages) > 0)
                        <div class="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-6 mb-8 border border-purple-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                @foreach(array_slice($similar_pages, 0, 4) as $page)
                                    <a href="{{ $page['url'] }}"
                                       class="flex items-center p-3 bg-white/70 rounded-lg hover:bg-white/90 transition-all duration-200">
                                        ...
                                        <div>
                                            <p class="font-medium text-slate-800">{{ $page['path'] }}</p>
                                            <p class="text-sm text-slate-600">Similarité: {{ $page['similarity'] }}%</p>
                                        </div>
                                    </a>
                                @endforeach
                            </div>
                    ...
                    @if(isset($popular_pages) && count($popular_pages) > 0)
                                    ...
                                    <a href="{{ $page['url'] }}"
                                       class="flex items-center p-3 bg-white/70 rounded-lg hover:bg-white/90 transition-all duration-200 hover:shadow-md group">
                                        <div class="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-500 rounded-lg flex items-center justify-center mr-3 group-hover:scale-110 transition-transform">
                                            <i class="{{ $page['icon'] }} text-white"></i>
                                        </div>
                                        <div>
