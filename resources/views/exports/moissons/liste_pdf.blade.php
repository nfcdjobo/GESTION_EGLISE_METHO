...
<head>
    ...
    <style>
        @page {
            ...
            @top-left {
                content: "{{ $donnees['meta']['eglise']['nom'] ?? 'Église' }}";
                font-size: 10px;
                color: #666;
            }
            @top-right {
            ...
            @bottom-center {
                content: "Généré le {{ $donnees['meta']['date_export'] }} - {{ $donnees['meta']['eglise']['nom'] ?? 'Système de Gestion des Moissons' }}";
                font-size: 9px;
                color: #888;
...
<body>
    ...
    <div class="header">
        <div class="logo-section">
            @if(!empty($donnees['meta']['eglise']['logo']) && file_exists(public_path($donnees['meta']['eglise']['logo'])))
                <img src="{{ public_path($donnees['meta']['eglise']['logo']) }}" alt="Logo" class="logo">
            @endif
            <div class="church-info">
                <div class="church-name">{{ $donnees['meta']['eglise']['nom'] }}</div>
                <div class="church-details">
                    {{ $donnees['meta']['eglise']['adresse'] }}<br>
                    Tél: {{ $donnees['meta']['eglise']['telephone'] }}<br>
                    Email: {{ $donnees['meta']['eglise']['email'] }}
                </div>
            </div>
        </div>
        <div class="export-info">
            <strong>Date d'export:</strong> {{ $donnees['meta']['date_export'] }}<br>
            <strong>Période:</strong> {{ $donnees['meta']['periode'] }}<br>
            <strong>Total moissons:</strong> {{ $donnees['meta']['nombre_total'] }}
        </div>
    </div>
    ...
    <div class="summary-stats">
        <div class="stat-item">
            <span class="stat-value">{{ $donnees['statistiques']['total_objectifs'] }} FCFA</span>
            <div class="stat-label">Objectifs Totaux</div>
        </div>
        <div class="stat-item">
            <span class="stat-value">{{ $donnees['statistiques']['total_collecte'] }} FCFA</span>
            <div class="stat-label">Montant Collecté</div>
        </div>
        <div class="stat-item">
            <span class="stat-value">{{ $donnees['statistiques']['objectifs_atteints'] }}</span>
            <div class="stat-label">Objectifs Atteints</div>
        </div>
        <div class="stat-item">
            <span class="stat-value">{{ $donnees['statistiques']['performance_moyenne'] }}</span>
            ...
            <tbody>
                @foreach($donnees['donnees'] as $moisson)
                    <tr class="{{
                        ...
                        <td style="text-align: left; padding-left: 6px;">{{ $moisson['theme'] }}</td>
                        <td>{{ $moisson['date'] }}</td>
                        <td>{{ $moisson['culte'] }}</td>
                        <td class="text-right">{{ $moisson['objectif'] }}</td>
                        <td class="text-right">{{ $moisson['collecte'] }}</td>
                        <td class="font-bold">{{ $moisson['pourcentage'] }}</td>
                        <td>{{ $moisson['statut'] }}</td>
                        <td>{{ $moisson['nb_passages'] }}</td>
                        <td>{{ $moisson['nb_ventes'] }}</td>
                        <td>{{ $moisson['nb_engagements'] }}</td>
                        <td style="text-align: left; padding-left: 4px;">{{ $moisson['createur'] }}</td>
                    </tr>
                @endforeach
                ...
                <tr class="totals-row">
                    ...
                    <td class="text-right font-bold">{{ $donnees['statistiques']['total_objectifs'] }}</td>
                    <td class="text-right font-bold">{{ $donnees['statistiques']['total_collecte'] }}</td>
                    <td class="font-bold">{{ $donnees['statistiques']['performance_moyenne'] }}</td>
                    <td colspan="5" class="text-center font-bold">{{ $donnees['statistiques']['objectifs_atteints'] }} objectifs atteints</td>
                </tr>
            </tbody>
        </table>
    @else
