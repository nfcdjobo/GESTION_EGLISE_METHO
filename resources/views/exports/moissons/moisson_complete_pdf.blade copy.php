...
<body>
    ...
    <!-- Vue d'ensemble sans tableau -->
    <div class="performance-overview">
        <div class="performance-title">Performance de la Moisson</div>

        <!-- Statistiques en ligne -->
        <div style="text-align: center; margin: 10px 0;">
            <span style="display: inline-block; margin: 0 15px; font-size: 11px;">
                <strong style="color: #1976D2; font-size: 13px;">{{ number_format($donnees['objectifs_et_realisations']['objectif_initial'], 0, ',', ' ') }} FCFA</strong><br>
                <span style="font-size: 8px; color: #666;">Objectif Initial</span>
            </span>
            <span style="display: inline-block; margin: 0 15px; font-size: 11px;">
                <strong style="color: #1976D2; font-size: 13px;">{{ number_format($donnees['objectifs_et_realisations']['montant_collecte'], 0, ',', ' ') }} FCFA</strong><br>
                <span style="font-size: 8px; color: #666;">Montant Collecté</span>
            </span>
            <span style="display: inline-block; margin: 0 15px; font-size: 11px;">
                <strong style="color: #1976D2; font-size: 13px;">{{ $donnees['objectifs_et_realisations']['pourcentage_realisation'] }}%</strong><br>
                <span style="font-size: 8px; color: #666;">Taux Réalisation</span>
            </span>
            <span style="display: inline-block; margin: 0 15px; font-size: 11px;">
                <strong style="color: #1976D2; font-size: 13px;">{{ $donnees['objectifs_et_realisations']['statut_progression'] }}</strong><br>
                <span style="font-size: 8px; color: #666;">Statut</span>
            </span>
        </div>
    ...
    <!-- Informations en format compact sans grille -->
    <div style="margin: 12px 0;">
        <!-- Informations Générales -->
        <div style="background: #f8f9fa; padding: 10px; border-radius: 6px; border-left: 3px solid #4472C4; margin-bottom: 8px;">
            <div style="font-weight: bold; color: #2E74B5; margin-bottom: 6px; font-size: 10px;">Informations Générales</div>
            <div style="font-size: 9px; line-height: 1.4;">
                <strong>Date:</strong> {{ $donnees['informations_generales']['date'] }} •
                <strong>Culte:</strong> {{ \Str::limit($donnees['informations_generales']['culte'], 35) }} •
                <strong>Créateur:</strong> {{ $donnees['informations_generales']['createur'] }} •
                <strong>Statut:</strong>
                <span style="padding: 2px 6px; border-radius: 3px; font-size: 8px; font-weight: bold; background-color: {{ $donnees['informations_generales']['statut'] === 'Actif' ? '#d4edda; color: #155724' : '#f8d7da; color: #721c24' }};">
                    {{ $donnees['informations_generales']['statut'] }}
                </span> •
                <strong>Créé le:</strong> {{ $donnees['informations_generales']['date_creation'] }}
            </div>
        </div>

        <!-- Résumé Financier -->
        <div style="background: #e8f5e8; padding: 10px; border-radius: 6px; border-left: 3px solid #4CAF50;">
            <div style="font-weight: bold; color: #2E7D32; margin-bottom: 6px; font-size: 10px;">Résumé Financier</div>
            <div style="font-size: 9px; line-height: 1.4;">
                <strong>Reste à collecter:</strong> <span style="font-family: 'Courier New', monospace; font-weight: bold; color: #d32f2f;">{{ number_format($donnees['objectifs_et_realisations']['reste_a_collecter'], 0, ',', ' ') }} FCFA</span> •
                <strong>Montant supplémentaire:</strong> <span style="font-family: 'Courier New', monospace; font-weight: bold; color: #388e3c;">{{ number_format($donnees['objectifs_et_realisations']['montant_supplementaire'], 0, ',', ' ') }} FCFA</span> •
                <strong>Dernière modification:</strong> {{ $donnees['informations_generales']['derniere_modification'] }}
            </div>
        </div>
    </div>
