            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('private.rapports-reunions.show', $rapport) }}" class="text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors">{{ $rapport->titre_rapport }}</a>
                    </div>
                </li>
                <li>
                    <div class="flex items-center">
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <select id="reunion_id" name="reunion_id" required
                                ...
                                @if($rapport->reunion)
                                    <option value="{{ $rapport->reunion->id }}" selected>
                                        {{ $rapport->reunion->titre }} - {{ \Carbon\Carbon::parse($rapport->reunion->date_reunion)->format('d/m/Y') }}
                                    </option>
                                @endif
                            </select>
                            @error('reunion_id')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="titre_rapport" name="titre_rapport" value="{{ old('titre_rapport', $rapport->titre_rapport) }}" required maxlength="200" placeholder="Ex: Rapport de la réunion mensuelle"
                            ...
                            <div>
                                ...
                                <select id="type_rapport" name="type_rapport" required
                                    ...
                                    @foreach(\App\Models\RapportReunion::TYPES_RAPPORT as $key => $value)
                                        <option value="{{ $value }}" {{ old('type_rapport', $rapport->type_rapport) == $value ? 'selected' : '' }}>
                                            @switch($value)
                            ...
                            <div>
                                ...
                                <select id="redacteur_id" name="redacteur_id"
                                    ...
                                    @foreach($redacteurs as $redacteur)
                                        <option value="{{ $redacteur->id }}" {{ old('redacteur_id', $rapport->redacteur_id) == $redacteur->id ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select id="validateur_id" name="validateur_id"
                                    ...
                                    @foreach($validateurs as $validateur)
                                        <option value="{{ $validateur->id }}" {{ old('validateur_id', $rapport->validateur_id) == $validateur->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="@error('resume') has-error @enderror">
                                <textarea id="resume" name="resume" rows="4" placeholder="Résumé du rapport"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none">{{ old('resume', $rapport->resume) }}</textarea>
                            </div>
                            @error('resume')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <div class="@error('decisions_prises') has-error @enderror">
                                <textarea id="decisions_prises" name="decisions_prises" rows="4" placeholder="Décisions importantes prises lors de la réunion"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none">{{ old('decisions_prises', $rapport->decisions_prises) }}</textarea>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="@error('actions_decidees') has-error @enderror">
                                <textarea id="actions_decidees" name="actions_decidees" rows="4" placeholder="Actions décidées et à mettre en œuvre"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none">{{ old('actions_decidees', $rapport->actions_decidees) }}</textarea>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="@error('recommandations') has-error @enderror">
                                <textarea id="recommandations" name="recommandations" rows="4" placeholder="Recommandations pour l'avenir"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none">{{ old('recommandations', $rapport->recommandations) }}</textarea>
                            </div>
                        ...
                        <div>
                            ...
                            <div class="@error('commentaires') has-error @enderror">
                                <textarea id="commentaires" name="commentaires" rows="3" placeholder="Commentaires généraux sur la réunion"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors resize-none">{{ old('commentaires', $rapport->commentaires) }}</textarea>
                            </div>
                            @error('commentaires')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                                ...
                                @if($rapport->points_traites && count($rapport->points_traites) > 0)
                                    @foreach($rapport->points_traites as $index => $point)
                                        <div class="flex items-center gap-2">
                                            <input type="text" name="points_traites[]" value="{{ is_array($point) ? $point['titre'] ?? $point : $point }}" placeholder="Titre du point traité"
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" id="nombre_presents" name="nombre_presents" value="{{ old('nombre_presents', $rapport->nombre_presents) }}" min="0"
                            ...
                            <div>
                                ...
                                <input type="number" id="montant_collecte" name="montant_collecte" value="{{ old('montant_collecte', $rapport->montant_collecte) }}" min="0" step="0.01"
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-3">
                                <label class="block text-sm font-medium text-slate-700">Liste des présences ({{ $rapport->presences ? count($rapport->presences) : 0 }})</label>
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                        @foreach($rapport->presences as $presence)
                                            <div class="flex items-center justify-between p-2 bg-white rounded border">
                                                <div>
                                                    <p class="font-medium text-sm">{{ is_array($presence) ? $presence['nom'] : $presence }}</p>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 text-center">
                        ...
                        @endphp
                        <span class="inline-flex items-center px-4 py-2 rounded-full text-sm font-medium {{ $statutColors[$rapport->statut] ?? 'bg-gray-100 text-gray-800' }}">
                            {{ $rapport->statut_traduit }}
                        </span>
                        <p class="text-sm text-slate-500 mt-2">Créé le {{ $rapport->created_at->format('d/m/Y à H:i') }}</p>
                        <p class="text-sm text-slate-500">Modifié le {{ $rapport->updated_at->format('d/m/Y à H:i') }}</p>
                        @if($rapport->valide_le)
                            <p class="text-sm text-slate-500">Validé le {{ $rapport->valide_le->format('d/m/Y à H:i') }}</p>
                        @endif
                        @if($rapport->publie_le)
                            <p class="text-sm text-slate-500">Publié le {{ $rapport->publie_le->format('d/m/Y à H:i') }}</p>
                        @endif

                        @if(!$rapport->est_modifiable)
                            <div class="mt-3 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                                <p class="text-sm text-orange-700">
                                    <i class="fas fa-exclamation-triangle mr-2"></i>
                                    Ce rapport ne peut plus être modifié car il est {{ $rapport->statut_traduit }}.
                                </p>
                            </div>
                        @endif
                    </div>
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span id="preview-titre" class="text-sm text-slate-900 font-semibold">{{ $rapport->titre_rapport }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Type:</span>
                            <span id="preview-type" class="text-sm text-slate-600">{{ $rapport->type_rapport_traduit }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Réunion:</span>
                            <span id="preview-reunion" class="text-sm text-slate-600">{{ $rapport->reunion->titre ?? 'N/A' }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Présents:</span>
                            <span id="preview-presents" class="text-sm text-slate-600">{{ $rapport->nombre_presents }}</span>
                        </div>
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    <div class="w-16 h-2 bg-slate-200 rounded-full mr-2">
                                        <div id="completion-bar" class="h-2 bg-gradient-to-r from-blue-500 to-green-500 rounded-full" style="width: {{ $rapport->pourcentage_completion }}%"></div>
                                    </div>
                                    <span id="completion-text" class="text-sm text-slate-900 font-semibold">{{ $rapport->pourcentage_completion }}%</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div>
                            ...
                            <select id="note_satisfaction" name="note_satisfaction"
                                ...
                                @for($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ old('note_satisfaction', $rapport->note_satisfaction) == $i ? 'selected' : '' }}>{{ $i }} - {{ ['Très insatisfaisante', 'Insatisfaisante', 'Correcte', 'Satisfaisante', 'Excellente'][$i-1] }}</option>
                                @endfor
                            </select>
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-3">
                        <a href="{{ route('private.rapports-reunions.show', $rapport) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-200">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
            <div class="p-6">
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    ...
                    <a href="{{ route('private.rapports-reunions.show', $rapport) }}" class="inline-flex items-center justify-center px-8 py-3 bg-slate-600 text-white font-medium rounded-xl hover:bg-slate-700 transition-colors">
        ...
        <div class="p-6">
            <div class="space-y-4">
                ...
                <div id="presences-modal-list" class="space-y-2">
                    @if($rapport->presences && count($rapport->presences) > 0)
                        @foreach($rapport->presences as $index => $presence)
                            <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg" data-presence-index="{{ $index }}">
                                <div>
                                    <p class="font-medium">{{ is_array($presence) ? $presence['nom'] : $presence }}</p>
                                    @if(is_array($presence) && isset($presence['role']))
                                        <p class="text-sm text-slate-500">{{ $presence['role'] }}</p>
                                    @endif
                                </div>
...
@push('scripts')
<script>
let pointsCount = {{ $rapport->points_traites ? count($rapport->points_traites) : 1 }};
let presencesData = @json($rapport->presences ?? []);

// Mise à jour de l'aperçu en temps réel
function updatePreview() {
    const titre = document.getElementById('titre_rapport').value || '{{ $rapport->titre_rapport }}';
    const typeSelect = document.getElementById('type_rapport');
    const type = typeSelect.options[typeSelect.selectedIndex]?.text || '{{ $rapport->type_rapport_traduit }}';
    const presents = document.getElementById('nombre_presents').value || '{{ $rapport->nombre_presents }}';
...
function supprimerPresence(userId) {
    ...
    fetch(`{{ route('private.rapports-reunions.presences.supprimer', $rapport->id) }}`, {
        method: 'DELETE',
        headers: {
