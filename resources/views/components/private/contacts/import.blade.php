    ...
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2">
                ...
                @can('contacts.import')
                    <div
                        class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300 mb-8">
                        <div class="p-6 border-b border-slate-200">
                            <h2 class="text-xl font-bold text-slate-800 flex items-center">
                                <span
                                    class="inline-flex items-center justify-center w-8 h-8 bg-blue-100 text-blue-600 rounded-full mr-3 text-sm font-bold">1</span>
                                Télécharger le Modèle
                            </h2>
                        </div>
                        <div class="p-6">
                            <p class="text-slate-600 mb-4">Téléchargez le fichier modèle pour connaître la structure exacte
                                attendue.</p>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <a href="{{ route('private.contacts.import-template') }}?format=csv"
                                    class="flex items-center justify-center p-4 border-2 border-dashed border-green-300 rounded-xl hover:border-green-400 hover:bg-green-50 transition-colors">
                                ...
                                <a href="{{ route('private.contacts.import-template') }}?format=excel"
                                    class="flex items-center justify-center p-4 border-2 border-dashed border-blue-300 rounded-xl hover:border-blue-400 hover:bg-blue-50 transition-colors">
                                    <div class="text-center">
                ...
                <div
                    ...
                    <div class="p-6 border-b border-slate-200">
                        <h2 class="text-xl font-bold text-slate-800 flex items-center">
                            <span
                                class="inline-flex items-center justify-center w-8 h-8 bg-amber-100 text-amber-600 rounded-full mr-3 text-sm font-bold">2</span>
                ...
                <div
                    ...
                    <div class="p-6 border-b border-slate-200">
                        <h2 class="text-xl font-bold text-slate-800 flex items-center">
                            <span
                                class="inline-flex items-center justify-center w-8 h-8 bg-green-100 text-green-600 rounded-full mr-3 text-sm font-bold">3</span>
                            Importer le Fichier
                        </h2>
                    </div>
                    <div class="p-6">
                        <form action="{{ route('private.contacts.import') }}" method="POST" enctype="multipart/form-data"
                            id="importForm">
                            @csrf
                            ...
                            <div class="mb-6">
                                <div id="dropZone"
                                    ...
                                    <input type="file" id="fileInput" name="import_file" accept=".csv,.xlsx,.xls"
                            ...
                            <div class="space-y-4 mb-6">
                                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                                    <div>
                                        <label for="skip_duplicates" class="text-sm font-medium text-slate-700">Ignorer
                                            les doublons</label>
                                        <p class="text-xs text-slate-500">Éviter d'importer les contacts existants</p>
                                    </div>
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="skip_duplicates" name="skip_duplicates"
                                ...
                                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                                    <div>
                                        <label for="validate_emails" class="text-sm font-medium text-slate-700">Valider
                                            les emails</label>
                                        <p class="text-xs text-slate-500">Vérifier le format des adresses email</p>
                                    </div>
                                    <label class="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" id="validate_emails" name="validate_emails"
                                            value="1" checked class="sr-only peer">
                                        <div
                                            class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600">
                                        </div>
                                ...
                                <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                                    <div>
                                        <label for="auto_verify" class="text-sm font-medium text-slate-700">Vérification
                                            automatique</label>
                            ...
                            <div class="flex flex-col sm:flex-row gap-4">
                                ...
                                <a href="{{ route('private.contacts.index') }}"
                                    class="flex-1 inline-flex items-center justify-center px-6 py-3 bg-slate-600 text-white font-medium rounded-xl hover:bg-slate-700 transition-colors">
                ...
                <div
                    ...
                    <div class="p-6 space-y-4">
                        <div class="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                            ...
                            <p class="text-sm text-amber-700">Vérifiez que votre fichier respecte le format du modèle
                                téléchargé.</p>
                        </div>

                        <div class="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                            <h4 class="font-medium text-blue-800 mb-1">Format de date</h4>
                            <p class="text-sm text-blue-700">Utilisez le format JJ/MM/AAAA ou AAAA-MM-JJ pour les dates.
                            </p>
                        </div>

                        <div class="p-3 bg-green-50 border border-green-200 rounded-lg">
                            <h4 class="font-medium text-green-800 mb-1">Encodage</h4>
                            <p class="text-sm text-green-700">Sauvegardez vos fichiers CSV en UTF-8 pour éviter les
                                problèmes d'accents.</p>
                        </div>

                        <button onclick="showHelp()"
                            class="w-full inline-flex items-center justify-center px-4 py-2 bg-amber-600 text-white text-sm font-medium rounded-xl hover:bg-amber-700 transition-colors">
        ...
        <script>
            ...
            function simulateImport() {
                ...
                const steps = [{
                    ...
                    {
                        progress: 25,
                        status: 'Validation des données...'
                    },
                    {
                        progress: 50,
                        status: 'Traitement des contacts...'
                    },
                    {
                        progress: 75,
