...
@section('title', 'Paramètres de l\'Église')
...
<div class="space-y-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Configuration générale de l'église - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
    </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        <div class="p-6">
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('private.parametres.show') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-emerald-600 to-green-600 text-white text-sm font-medium rounded-xl hover:from-emerald-700 hover:to-green-700 transition-all duration-200 shadow-md hover:shadow-lg">
                ...
                <a href="{{route('private.parametresdons.index')}}"  class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-200 shadow-md hover:shadow-lg">
            ...
            <div class="p-6 space-y-4">
                <div class="flex items-center space-x-4">
                    @if($parametres->logo_url)
                        <img src="{{ $parametres->logo_url }}" alt="Logo" class="w-16 h-16 object-cover rounded-xl shadow-md">
                    @else
                    ...
                    @endif
                    <div>
                        <h3 class="text-xl font-bold text-slate-900">{{ $parametres->nom_eglise }}</h3>
                        @if($parametres->date_fondation)
                            <p class="text-sm text-slate-500">Fondée en {{ $parametres->date_fondation->format('Y') }}</p>
                        @endif
                    </div>
                </div>
                ...
                @if($parametres->description_eglise)
                    <div class="p-4 bg-slate-50 rounded-xl">
                        <p class="text-slate-700">{{ Str::limit($parametres->description_eglise, 200) }}</p>
                    </div>
                @endif

                @if($parametres->verset_biblique)
                    <div class="p-4 bg-blue-50 rounded-xl border-l-4 border-blue-500">
                        <p class="text-blue-800 italic">"{{ $parametres->verset_biblique }}"</p>
                        @if($parametres->reference_verset)
                            <p class="text-blue-600 text-sm mt-2 font-medium">- {{ $parametres->reference_verset }}</p>
                        @endif
                    </div>
                @endif
            </div>
            ...
            <div class="p-6 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p class="font-medium text-slate-900">{{ $parametres->telephone_1 ?: 'Non défini' }}</p>
                        </div>
                    </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <div>
                        ...
                        <p class="font-medium text-slate-900">{{ $parametres->getAdresseComplete() }}</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $parametres->nombre_membres ?: '0' }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $parametres->date_fondation ? $parametres->date_fondation->diffInYears() : '0' }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ count(array_filter([$parametres->facebook_url, $parametres->instagram_url, $parametres->youtube_url, $parametres->twitter_url])) }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $parametres->getProgrammes() ? count($parametres->getProgrammes()) : '0' }}</p>
    ...
    <!-- Vision et Mission -->
    @if($parametres->mission_statement || $parametres->vision)
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            @if($parametres->mission_statement)
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6">
                        <p class="text-slate-700 leading-relaxed">{{ $parametres->mission_statement }}</p>
                    </div>
                </div>
            @endif
            ...
            @if($parametres->vision)
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6">
                        <p class="text-slate-700 leading-relaxed">{{ $parametres->vision }}</p>
                    </div>
                </div>
            @endif
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                ...
                @if(!$parametres->facebook_url && !$parametres->instagram_url && !$parametres->youtube_url && !$parametres->twitter_url)
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                @if($parametres->getProgrammesPublics() && count($parametres->getProgrammesPublics()) > 0)
                    <div class="space-y-3">
                        @foreach($parametres->getProgrammesPublics() as $programme)
                            <div class="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                                <div class="flex items-center space-x-3">
                                    <div class="w-10 h-10 bg-amber-100 rounded-lg flex items-center justify-center">
                                        <i class="{{ $programme['icone'] ?? 'fas fa-calendar-day' }} text-amber-600"></i>
                                    </div>
                                    <div>
                                        <p class="font-medium text-slate-900">{{ $programme['titre'] ?? 'N/A' }}</p>
                                        <p class="text-sm text-slate-500">{{ $programme['type_horaire'] ?? 'regulier' }}</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="font-medium text-slate-900">{{ $programme['horaire_texte'] ?? 'N/A' }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
...
function gererProgrammes() {
    ...
    window.location.href = "{{ route('private.parametres.edit') }}#programmes";
}
