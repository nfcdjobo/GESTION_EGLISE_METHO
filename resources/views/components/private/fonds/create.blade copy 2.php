...
@section('content')
    <div class="space-y-8">
        ...
        <div class="mb-8">
            <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Créer
                une Nouvelle Transaction</h1>
                    ...
                    {{-- <div
                        ...
                        <div class="p-6 space-y-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div class="space-y-4">
                                    <div class="flex items-center">
                                        <input type="checkbox" id="est_anonyme" name="est_anonyme" value="1" {{
                                            old('est_anonyme') ? 'checked' : '' }}
                                    ...
                                    <div class="flex items-center">
                                        <input type="checkbox" id="est_membre" name="est_membre" value="1" {{
                                            old('est_membre', true) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                ...
                                <div>
                                    ...
                                    <select id="collecteur_id" name="collecteur_id"
                                        ...
                                        <option value="{{ $collecteur->id }}" {{ old('collecteur_id', auth()->id()) ==
                                            $collecteur->id ? 'selected' : '' }}>
                            ...
                            <div id="donateur_section" class="space-y-4">
                                <div>
                                    ...
                                    <select id="donateur_id" name="donateur_id"
                                        ...
                                        <option value="{{ $donateur->id }}" {{ old('donateur_id')==$donateur->id ?
                                            'selected' : '' }}>
...
<!-- Informations du donateur - Section modifiée -->
<div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
    ...
    <div class="p-6 space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="relative" data-type="collecteur">
                    <input type="text"
                           id="collecteur_search"
                           name="collecteur_search"
                           placeholder="Rechercher un collecteur..."
                           class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('collecteur_id') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror"
                           required
                           autocomplete="off"
                           value="{{ old('collecteur_search', auth()->user()->nom . ' ' . auth()->user()->prenom) }}">
                    <input type="hidden" id="collecteur_id" name="collecteur_id" value="{{ old('collecteur_id', auth()->id()) }}">

                    <div class="autocomplete-dropdown absolute top-full left-0 right-0 bg-white border border-slate-200 rounded-lg shadow-xl max-h-72 overflow-y-auto z-50" id="collecteur_dropdown">
                        ...
                        <div class="add-new-item p-3 border-t border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors hidden">
        ...
        <div id="donateur_section" class="space-y-4">
            ...
            <div>
                <label for="donateur_search" class="block text-sm font-medium text-slate-700 mb-2">Donateur (membre)</label>
                <div class="relative" data-type="donateur">
                    <input type="text"
                           id="donateur_search"
                           name="donateur_search"
                           placeholder="Rechercher un donateur..."
                           class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('donateur_id') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror"
                           autocomplete="off"
                           value="{{ old('donateur_search') }}">
                    <input type="hidden" id="donateur_id" name="donateur_id" value="{{ old('donateur_id') }}">

                    <div class="autocomplete-dropdown absolute top-full left-0 right-0 bg-white border border-slate-200 rounded-lg shadow-xl max-h-72 overflow-y-auto z-50" id="donateur_dropdown">
                        <div class="loading-item p-3 text-center text-slate-500 hidden">
                            <i class="fas fa-spinner fa-spin mr-2"></i>Recherche en cours...
                        </div>
                        <div class="no-results p-3 text-center text-slate-500 hidden">
...
class AutoComplete {
    ...
    init() {
        ...
        this.addNewItem.addEventListener('click', this.handleAddNew.bind(this));
    ...
    destroy() {
        ...
        // Optionnel : nettoyer le sessionStorage si nécessaire
    ...
    handleInput() {
        ...
        // Si le champ est vide, cacher le dropdown et vider la sélection
        if (query.length === 0) {
            this.hideDropdown();
        ...
        // Si l'utilisateur tape quelque chose de différent de la sélection courante,
        // vider la valeur cachée car ce n'est plus valide
        const key = `autocomplete_${this.type}_selection`;
        const stored = sessionStorage.getItem(key);
        if (stored) {
            const selection = JSON.parse(stored);
            if (selection.display !== query) {
                this.hiddenInput.value = '';
                this.clearStoredSelection();
            }
        } else if (this.hiddenInput.value) {
            // Si pas de sélection stockée mais qu'il y a une valeur cachée,
            // la vider car l'utilisateur a modifié le texte
            this.hiddenInput.value = '';
        }

        // Si moins de 2 caractères, cacher le dropdown
        if (query.length < 2) {
            this.hideDropdown();
            return;
        }
        ...
        // Démarrer une nouvelle recherche avec délai
...
// alert()
            ...
            if (!this.currentRequest.signal.aborted) {
                // Adapter le format de réponse de l'API
    ...
    //     // Extraire le nom depuis le champ "text"
    //     // Format: "Nom PRENOM (email@example.com)"
    //     let displayName = user.text;
    //     let nameOnly = user.text;

    //     // Extraire le nom sans l'email entre parenthèses
    //     const emailMatch = user.text.match(/^(.+)\s+\([^)]+\)$/);
    //     if (emailMatch) {
    //         nameOnly = emailMatch[1].trim();
    //     }

    //     div.innerHTML = `
    //         <div class="font-medium text-slate-700">${nameOnly}</div>
    //         <div class="text-sm text-slate-500">sssss${user.email}</div>
    //     `;
    ...
    createUserItem(user, index) {
        ...
        div.addEventListener('click', (e) => {
            console.log("clicked:", nameOnly, user); // Debug
    ...
    selectItem(user, displayName = null) {
        // Utiliser le displayName si fourni, sinon extraire depuis le champ text
        ...
        // Stocker la sélection pour restauration ultérieure
    ...
    storeSelection(userId, displayName) {
        // Stocker dans sessionStorage pour persister durant la session
        const key = `autocomplete_${this.type}_selection`;
    ...
    async restoreSelection() {
        // Restaurer depuis sessionStorage ou depuis les valeurs du formulaire
        const key = `autocomplete_${this.type}_selection`;
        const stored = sessionStorage.getItem(key);

        // Si on a une valeur dans le champ caché, essayer de restaurer l'affichage
        if (this.hiddenInput.value) {
            // Si on a déjà le bon texte dans l'input, ne rien faire
            if (this.input.value && this.input.value.trim() !== '') {
                return;
            }

            // Sinon, essayer de récupérer les infos de l'utilisateur
            if (stored) {
                const selection = JSON.parse(stored);
                if (selection.id === this.hiddenInput.value) {
                    this.input.value = selection.display;
                    return;
                }
            }

            // En dernier recours, faire une requête pour récupérer les infos
            try {
    ...
    handleAddNew() {
        ...
        // Ouvrir un modal ou rediriger vers la création d'utilisateur
                ...
                <form id="addUserForm">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="telephone" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Rôle</label>
                            <select name="role" class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
    ...
    async createUser(formData, modal) {
        try {
            const response = await fetch('{{ route("private.users.store") }}', {
                method: 'POST',
                headers: {
...
// Modifier la fonction toggleDonateurSection existante
function toggleDonateurSection() {
    ...
    const donateurContainer = document.querySelector('[data-type="donateur"]').parentElement;
    ...
    if (estAnonyme) {
        donateurContainer.style.display = 'none';
        externeSection.classList.add('hidden');
        donateurSearch.value = '';
        donateurId.value = '';
    } else if (estMembre) {
        donateurContainer.style.display = 'block';
        externeSection.classList.add('hidden');
    } else {
        donateurContainer.style.display = 'none';
    ...
    if (collecteurContainer) {
        const collecteurAutocomplete = new AutoComplete(collecteurContainer, 'collecteur');
        // Restaurer l'affichage si une valeur est déjà sélectionnée
        collecteurAutocomplete.restoreSelection();
    }

    if (donateurContainer) {
        const donateurAutocomplete = new AutoComplete(donateurContainer, 'donateur');
        // Restaurer l'affichage si une valeur est déjà sélectionnée
        donateurAutocomplete.restoreSelection();
    }
