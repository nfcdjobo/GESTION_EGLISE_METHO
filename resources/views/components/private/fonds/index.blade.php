...
@section('content')
    <div class="space-y-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="p-6 border-b border-slate-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @can('fonds.create')
                            <a href="{{ route('private.fonds.create') }}"
                        ...
                        @endcan
                        @can('fonds.export')
                            <a href="{{ route('private.fonds.export') }}"
            ...
            <div class="p-6">
                <form method="GET" action="{{ route('private.fonds.index') }}"
                    ...
                    <div>
                        ...
                        <select name="statut"
                            ...
                            <option value="remboursee" {{ request('statut') == 'remboursee' ? 'selected' : '' }}>Remboursée
                            </option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="type_transaction"
                            ...
                            <option value="dime" {{ request('type_transaction') == 'dime' ? 'selected' : '' }}>Dîme
                            </option>
                            <option value="offrande_libre"
                                {{ request('type_transaction') == 'offrande_libre' ? 'selected' : '' }}>Offrande libre
                            </option>
                            <option value="offrande_ordinaire"
                                {{ request('type_transaction') == 'offrande_ordinaire' ? 'selected' : '' }}>Offrande
                                ordinaire</option>
                            <option value="offrande_speciale"
                                {{ request('type_transaction') == 'offrande_speciale' ? 'selected' : '' }}>Offrande
                                spéciale</option>
                            <option value="don_special"
                                {{ request('type_transaction') == 'don_special' ? 'selected' : '' }}>Don spécial</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="mode_paiement"
                            ...
                            <option value="mobile_money"
                                {{ request('mode_paiement') == 'mobile_money' ? 'selected' : '' }}>Mobile Money</option>
                            <option value="virement" {{ request('mode_paiement') == 'virement' ? 'selected' : '' }}>
                                Virement</option>
                            <option value="cheque" {{ request('mode_paiement') == 'cheque' ? 'selected' : '' }}>Chèque
                            </option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="donateur_id"
                            ...
                            @foreach ($filterData['donateurs'] as $donateur)
                                <option value="{{ $donateur->id }}"
                                    {{ request('donateur_id') == $donateur->id ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $fonds->where('statut', 'en_attente')->count() }}
                        </p>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ number_format($fonds->where('statut', 'validee')->sum('montant')) }}</p>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex items-center space-x-2">
                        <select id="perPage"
                            ...
                            <option value="100" {{ request('per_page') == 100 ? 'selected' : '' }}>100 par page
                            </option>
                        </select>
                    </div>
                </div>
                ...
                @if ($fonds->count() > 0)
                            ...
                            <tbody>
                                @foreach ($fonds as $fond)
                                    <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td class="py-4 px-4">
                                            <div class="text-slate-900">{{ $fond->date_transaction->format('d/m/Y') }}
                                            </div>
                                            @if ($fond->heure_transaction)
                                                <div class="text-xs text-slate-500">
                                                    {{ $fond->heure_transaction->format('H:i') }}</div>
                                            @endif
                                        </td>
                                        <td class="py-4 px-4">
                                            @if ($fond->est_anonyme)
                                            ...
                                            @elseif($fond->donateur)
                                                <div class="font-medium text-slate-900">{{ $fond->donateur->nom }}
                                                    {{ $fond->donateur->prenom }}</div>
                                        ...
                                        <td class="py-4 px-4">
                                            <div class="font-bold text-slate-900">
                                                {{ number_format($fond->montant, 0, ',', ' ') }} {{ $fond->devise }}</div>
                                        ...
                                        <td class="py-4 px-4">
                                            ...
                                            @elseif($fond->statut == 'remboursee') bg-purple-100 text-purple-800 @endif">
                                        ...
                                        <td class="py-4 px-4">
                                            <div class="flex items-center space-x-2">
                                                @can('fonds.read')
                                                    <a href="{{ route('private.fonds.show', $fond) }}"
                                                ...
                                                @can('fonds.update')
                                                    @if ($fond->peutEtreModifiee())
                                                        <a href="{{ route('private.fonds.edit', $fond) }}"
                                                ...
                                                @can('fonds.validate')
                                                    @if ($fond->peutEtreValidee())
                                                        <button type="button"
                                                            onclick="validateTransaction('{{ $fond->id }}')"
                                                ...
                                                @can('fonds.cancel')
                                                    @if ($fond->peutEtreAnnulee())
                                                        <button type="button"
                                                            onclick="openCancelModal('{{ $fond->id }}')"
                                                ...
                                                @can('fonds.generate-receipt')
                                                    @if ($fond->peutGenererRecu())
                                                        <button type="button"
                                                            onclick="generateReceipt('{{ $fond->id }}')"
                                                ...
                                                @can('fonds.delete')
                                                    @if ($fond->peutEtreModifiee())
                                                        <button type="button"
                                                            onclick="deleteTransaction('{{ $fond->id }}')"
                                                            class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors"
                                                            title="Supprimer">
                    ...
                    <div
                        ...
                        <div class="text-sm text-slate-700">
                            Affichage de <span class="font-medium">{{ $fonds->firstItem() }}</span> à <span
                                class="font-medium">{{ $fonds->lastItem() }}</span>
