...
@section('title', 'Gestion des Fonds')
...
<div class="space-y-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Suivi des transactions financières - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
    </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.fonds.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-2">Recherche</label>
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Numéro, donateur, référence..." class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div>
                    ...
                    <select name="type_transaction" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        <option value="dime" {{ request('type_transaction') == 'dime' ? 'selected' : '' }}>Dîme</option>
                        <option value="offrande_libre" {{ request('type_transaction') == 'offrande_libre' ? 'selected' : '' }}>Offrande libre</option>
                        <option value="offrande_ordinaire" {{ request('type_transaction') == 'offrande_ordinaire' ? 'selected' : '' }}>Offrande ordinaire</option>
                        <option value="offrande_speciale" {{ request('type_transaction') == 'offrande_speciale' ? 'selected' : '' }}>Offrande spéciale</option>
                        <option value="don_special" {{ request('type_transaction') == 'don_special' ? 'selected' : '' }}>Don spécial</option>
                    </select>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="donateur_id" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($filterData['donateurs'] as $donateur)
                            <option value="{{ $donateur->id }}" {{ request('donateur_id') == $donateur->id ? 'selected' : '' }}>
                ...
                <div class="lg:col-span-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="montant_min" value="{{ request('montant_min') }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Montant max</label>
                        <input type="number" name="montant_max" value="{{ request('montant_max') }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    </div>
                </div>
                <div class="lg:col-span-6 flex gap-2 pt-4">
                    ...
                    <a href="{{ route('private.fonds.index') }}" class="inline-flex items-center px-4 py-2 bg-slate-600 text-white text-sm font-medium rounded-xl hover:bg-slate-700 transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $fonds->total() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $fonds->where('statut', 'validee')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $fonds->where('statut', 'en_attente')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ number_format($fonds->where('statut', 'validee')->sum('montant')) }}</p>
        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h2 class="text-xl font-bold text-slate-800 flex items-center">
                    ...
                    Liste des Transactions ({{ $fonds->total() }})
                </h2>
            ...
            @if($fonds->count() > 0)
                        ...
                        <tbody>
                            @foreach($fonds as $fond)
                                <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                    <td class="py-4 px-4">
                                        <div class="font-medium text-slate-900">{{ $fond->numero_transaction }}</div>
                                        @if($fond->est_recurrente)
                                            <div class="text-xs text-purple-600"><i class="fas fa-sync-alt mr-1"></i>Récurrente</div>
                                        @endif
                                    </td>
                                    <td class="py-4 px-4">
                                        <div class="text-slate-900">{{ $fond->date_transaction->format('d/m/Y') }}</div>
                                        @if($fond->heure_transaction)
                                            <div class="text-xs text-slate-500">{{ $fond->heure_transaction->format('H:i') }}</div>
                                        @endif
                                    </td>
                                    <td class="py-4 px-4">
                                        @if($fond->est_anonyme)
                                            <span class="text-slate-500 italic">Donateur anonyme</span>
                                        @elseif($fond->donateur)
                                            <div class="font-medium text-slate-900">{{ $fond->donateur->nom }} {{ $fond->donateur->prenom }}</div>
                                            <div class="text-xs text-slate-500">{{ $fond->donateur->email }}</div>
                                        @else
                                            <span class="text-slate-700">{{ $fond->nom_donateur_anonyme }}</span>
                                        @endif
                                    </td>
                                    <td class="py-4 px-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            @if($fond->type_transaction == 'dime') bg-blue-100 text-blue-800
                                            @elseif(str_contains($fond->type_transaction, 'offrande')) bg-green-100 text-green-800
                                            @elseif(str_contains($fond->type_transaction, 'don')) bg-purple-100 text-purple-800
                                            @else bg-gray-100 text-gray-800
                                            @endif">
                                            {{ $fond->type_transaction_libelle ?? ucfirst(str_replace('_', ' ', $fond->type_transaction)) }}
                                        </span>
                                        @if($fond->est_flechee)
                                    ...
                                    <td class="py-4 px-4">
                                        <div class="font-bold text-slate-900">{{ number_format($fond->montant, 0, ',', ' ') }} {{ $fond->devise }}</div>
                                        @if($fond->type_transaction == 'don_materiel' && $fond->valeur_estimee)
                                            <div class="text-xs text-slate-500">Valeur estimée</div>
                                        @endif
                                    </td>
                                    <td class="py-4 px-4">
                                        <span class="text-slate-600">
                                            @if($fond->mode_paiement == 'especes')
                                                <i class="fas fa-money-bill text-green-600 mr-1"></i>Espèces
                                            @elseif($fond->mode_paiement == 'mobile_money')
                                                <i class="fas fa-mobile-alt text-blue-600 mr-1"></i>Mobile Money
                                            @elseif($fond->mode_paiement == 'virement')
                                                <i class="fas fa-university text-purple-600 mr-1"></i>Virement
                                            @elseif($fond->mode_paiement == 'cheque')
                                    ...
                                    <td class="py-4 px-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                            @if($fond->statut == 'validee') bg-green-100 text-green-800
                                            @elseif($fond->statut == 'en_attente') bg-yellow-100 text-yellow-800
                                            @elseif($fond->statut == 'annulee') bg-red-100 text-red-800
                                            @elseif($fond->statut == 'remboursee') bg-purple-100 text-purple-800
                                            @endif">
                                            {{ $fond->statut_libelle ?? ucfirst($fond->statut) }}
                                        </span>
                                        @if($fond->recu_emis)
                                    ...
                                    <td class="py-4 px-4">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @can('fonds.validate')
                                                ...
                                                @if($fond->peutEtreAnnulee())
                                                    <button type="button" onclick="openCancelModal('{{ $fond->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Annuler">
                                            ...
                                            @can('supprimer_fonds')
                                                @if($fond->peutEtreModifiee())
                                                    <button type="button" onclick="deleteTransaction('{{ $fond->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Supprimer">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6 pt-6 border-t border-slate-200">
                    <div class="text-sm text-slate-700">
                        Affichage de <span class="font-medium">{{ $fonds->firstItem() }}</span> à <span class="font-medium">{{ $fonds->lastItem() }}</span>
                        sur <span class="font-medium">{{ $fonds->total() }}</span> résultats
                    </div>
                    <div>
                        {{ $fonds->appends(request()->query())->links() }}
                    </div>
                </div>
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-slate-500 mb-6">
                        @if(request()->hasAny(['search', 'statut', 'type_transaction']))
...
function confirmValidation() {
    ...
    fetch(`{{ route('private.fonds.validate.strict', ':fond')}}`.replace(':fond', transactionId), {
        method: 'PATCH',
        headers: {
...
function confirmCancellation() {
    ...
    fetch(`{{ route('private.fonds.cancel.strict', ':fond')}}`.replace(':fond', transactionId), {
        method: 'PATCH',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json',
...
function deleteTransaction(transactionId) {
    if (confirm('Êtes-vous sûr de vouloir supprimer cette transaction ?')) {
        fetch(`{{route('private.fonds.destroy', ':fonds')}}`.replace(':fonds', transactionId), {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Accept': 'application/json'
