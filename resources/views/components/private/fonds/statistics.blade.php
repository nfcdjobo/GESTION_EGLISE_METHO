...
<div class="space-y-8">
    <!-- Page Title -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Statistiques Financières</h1>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        <div class="p-6">
            <form method="GET" action="{{ route('private.fonds.statistics') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                ...
                <div>
                    ...
                    <select name="annee" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div>
                    ...
                    <select name="mois" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div class="text-center">
                    <div class="text-3xl font-bold text-blue-600">{{ number_format($stats['resume_global']['total_collecte'] ?? 0, 0, ',', ' ') }}</div>
                    <div class="text-sm text-slate-500">Total collecté (XOF)</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-green-600">{{ number_format($stats['resume_global']['nombre_transactions'] ?? 0) }}</div>
                    <div class="text-sm text-slate-500">Transactions totales</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-purple-600">{{ number_format($stats['resume_global']['nombre_donateurs'] ?? 0) }}</div>
                    <div class="text-sm text-slate-500">Donateurs uniques</div>
                </div>
                <div class="text-center">
                    <div class="text-3xl font-bold text-amber-600">{{ number_format($stats['resume_global']['montant_moyen'] ?? 0, 0, ',', ' ') }}</div>
            ...
            <div class="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-6">
                <div class="bg-blue-50 rounded-xl p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-lg font-bold text-blue-900">{{ number_format($stats['resume_global']['dimes_total'] ?? 0, 0, ',', ' ') }}</div>
                ...
                <div class="bg-green-50 rounded-xl p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-lg font-bold text-green-900">{{ number_format($stats['resume_global']['offrandes_total'] ?? 0, 0, ',', ' ') }}</div>
                ...
                <div class="bg-purple-50 rounded-xl p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-lg font-bold text-purple-900">{{ number_format($stats['resume_global']['dons_total'] ?? 0, 0, ',', ' ') }}</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                @if(isset($stats['tendances_annuelles']['donnees_mensuelles']) && $stats['tendances_annuelles']['donnees_mensuelles']->count() > 0)
                    <div class="mb-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div class="bg-green-50 rounded-lg p-3">
                            ...
                            <div class="text-lg font-bold text-green-900">{{ number_format($stats['tendances_annuelles']['croissance_moyenne'] ?? 0, 1) }}%</div>
                        </div>
                    ...
                    <div class="space-y-3 max-h-64 overflow-y-auto">
                        @foreach($stats['comparaison_mensuelle'] as $mois => $data)
                            <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                <div>
                                    <div class="font-medium text-slate-900">{{ $data['nom_mois'] ?? 'Mois ' . $mois }}</div>
                                    <div class="text-sm text-slate-500">{{ number_format($data['actuel'], 0, ',', ' ') }} XOF</div>
                                </div>
                                    ...
                                    @if($data['variation'] != 0)
                                        <div class="flex items-center">
                                            @if($data['tendance'] == 'hausse')
                                                ...
                                                <span class="text-green-600 font-medium">+{{ number_format($data['variation'], 1) }}%</span>
                                            @elseif($data['tendance'] == 'baisse')
                                                <i class="fas fa-arrow-down text-red-500 mr-1"></i>
                                                <span class="text-red-600 font-medium">{{ number_format($data['variation'], 1) }}%</span>
                                            @else
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        @foreach($stats['performance_donateurs']['segmentation'] as $segment => $data)
                            <div class="bg-slate-50 rounded-lg p-3">
                                <div class="text-sm font-medium text-slate-600 capitalize">{{ str_replace('_', ' ', $segment) }}</div>
                                <div class="text-lg font-bold text-slate-900">{{ $data['count'] ?? 0 }}</div>
                                <div class="text-xs text-slate-500">{{ number_format($data['pourcentage_montant'] ?? 0, 1) }}% du total</div>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="bg-green-50 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="text-2xl font-bold text-green-900">{{ number_format($stats['performance_donateurs']['retention_rate'] ?? 0, 1) }}%</div>
                            </div>
                            <div class="w-10 h-10 bg-green-500 rounded-lg flex items-center justify-center">
                            ...
                            <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                <div>
                                    ...
                                    <div class="text-sm text-slate-500">
                                        {{ \Carbon\Carbon::parse($culte->date_culte)->format('d/m/Y') }} - {{ $culte->nombre_transactions }} transactions
                                    </div>
                                </div>
                                <div class="text-right">
                                    <div class="font-bold text-slate-900">{{ number_format($culte->total_montant, 0, ',', ' ') }}</div>
                                    <div class="text-sm text-slate-500">{{ number_format($culte->montant_moyen, 0, ',', ' ') }} moy.</div>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div class="text-center">
                        <div class="w-20 h-20 mx-auto mb-4 bg-blue-500 rounded-full flex items-center justify-center">
                            <span class="text-white font-bold text-lg">{{ number_format($stats['ratios_financiers']['ratio_dimes'] ?? 0, 0) }}%</span>
                        </div>
                    ...
                    <div class="text-center">
                        <div class="w-20 h-20 mx-auto mb-4 bg-green-500 rounded-full flex items-center justify-center">
                            <span class="text-white font-bold text-lg">{{ number_format($stats['ratios_financiers']['ratio_offrandes'] ?? 0, 0) }}%</span>
                        </div>
                    ...
                    <div class="text-center">
                        <div class="w-20 h-20 mx-auto mb-4 bg-purple-500 rounded-full flex items-center justify-center">
                            <span class="text-white font-bold text-lg">{{ number_format($stats['ratios_financiers']['ratio_dons'] ?? 0, 0) }}%</span>
                        </div>
                    ...
                    <div class="text-center">
                        <div class="w-20 h-20 mx-auto mb-4 bg-amber-500 rounded-full flex items-center justify-center">
                            <span class="text-white font-bold text-lg">{{ number_format($stats['ratios_financiers']['diversification_score'] ?? 0, 0) }}</span>
                        </div>
                    ...
                    <div class="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <div class="text-2xl font-bold text-slate-900">{{ number_format($stats['ratios_financiers']['stabilite_score'] ?? 0, 1) }}/100</div>
                            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    @if(isset($stats['saisonnalite']['mois_le_plus_fort']))
                        <div class="text-center p-4 bg-green-50 rounded-xl">
                            ...
                            <div class="font-bold text-green-900">{{ $stats['saisonnalite']['mois_le_plus_fort']['nom_mois'] ?? 'N/A' }}</div>
                            <div class="text-sm text-green-600">Meilleur mois</div>
                            <div class="text-lg font-semibold text-slate-900 mt-2">
                                {{ number_format($stats['saisonnalite']['mois_le_plus_fort']['total'] ?? 0, 0, ',', ' ') }} XOF
                            </div>
                        </div>
                    @endif
                    ...
                    @if(isset($stats['saisonnalite']['mois_le_plus_faible']))
                        <div class="text-center p-4 bg-red-50 rounded-xl">
                            <div class="w-16 h-16 mx-auto mb-3 bg-red-500 rounded-full flex items-center justify-center">
                                <i class="fas fa-arrow-down text-white text-xl"></i>
                            </div>
                            <div class="font-bold text-red-900">{{ $stats['saisonnalite']['mois_le_plus_faible']['nom_mois'] ?? 'N/A' }}</div>
                            <div class="text-sm text-red-600">Mois le plus faible</div>
                            <div class="text-lg font-semibold text-slate-900 mt-2">
                                {{ number_format($stats['saisonnalite']['mois_le_plus_faible']['total'] ?? 0, 0, ',', ' ') }} XOF
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="text-center p-4 bg-blue-50 rounded-xl">
                        ...
                        <div class="font-bold text-blue-900">{{ number_format($stats['saisonnalite']['coefficient_variation'] ?? 0, 1) }}%</div>
                                        ...
                                        <div class="mt-2 text-sm text-orange-700">
                                            @if(is_array($alerte) && isset($alerte['message']))
                                                ...
                                                @if(isset($alerte['recommandation']))
                                                    <br><strong>Recommandation:</strong> {{ $alerte['recommandation'] }}
                                                @endif
                                            @else
                                                {{-- {{ $alerte }} --}}
                                            @endif
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
        <div class="p-6">
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('private.fonds.export') }}?format=excel&periode={{ $periode }}&annee={{ $annee }}&mois={{ $mois }}" class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                    <i class="fas fa-file-excel mr-2"></i> Exporter Excel
                </a>
                <a href="{{ route('private.fonds.export') }}?format=pdf&periode={{ $periode }}&annee={{ $annee }}&mois={{ $mois }}" class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-red-600 to-pink-600 text-white font-medium rounded-xl hover:from-red-700 hover:to-pink-700 transition-all duration-200 shadow-md hover:shadow-lg">
                    <i class="fas fa-file-pdf mr-2"></i> Exporter PDF
                </a>
                <a href="{{ route('private.fonds.analytics') }}?annee={{ $annee }}" class="inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-medium rounded-xl hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 shadow-md hover:shadow-lg">
