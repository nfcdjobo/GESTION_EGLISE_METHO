...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Modifier l'Annonce</h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('private.annonces.show', $annonce) }}" class="text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors">
    ...
    <form action="{{ route('private.annonces.update', $annonce) }}" method="POST" id="annonceForm" class="space-y-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-yellow-700">
                                    <p>Cette annonce est encore en brouillon. N'oubliez pas de la publier une fois les modifications terminées.</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <input type="text" id="titre" name="titre" value="{{ old('titre', $annonce->titre) }}" required maxlength="200" placeholder="Titre accrocheur pour l'annonce"
                ...
                <div id="evenement_section" class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300 {{ $annonce->type_annonce !== 'evenement' ? 'hidden' : '' }}">
                    <div class="p-6 border-b border-slate-200">
                        <h2 class="text-xl font-bold text-slate-800 flex items-center">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" id="date_evenement" name="date_evenement" value="{{ old('date_evenement', $annonce->date_evenement?->format('Y-m-d')) }}"
                            ...
                            <div>
                                ...
                                <input type="text" id="lieu_evenement" name="lieu_evenement" value="{{ old('lieu_evenement', $annonce->lieu_evenement) }}" maxlength="255" placeholder="Lieu de l'événement"
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="datetime-local" id="expire_le" name="expire_le" value="{{ old('expire_le', $annonce->expire_le?->format('Y-m-d\TH:i')) }}"
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="text-center">
                            <span class="inline-flex items-center px-4 py-2 rounded-full text-lg font-medium {{ $annonce->badge_statut }}">
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span id="preview-titre" class="text-sm text-slate-900 font-semibold">{{ $annonce->titre }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Type:</span>
                            <span id="preview-type" class="text-sm text-slate-600">{{ $typesAnnonces[$annonce->type_annonce] ?? $annonce->type_annonce }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Priorité:</span>
                            <span id="preview-priorite" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $annonce->badge_priorite }}">{{ $niveauxPriorite[$annonce->niveau_priorite] ?? $annonce->niveau_priorite }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Audience:</span>
                            <span id="preview-audience" class="text-sm text-slate-600">{{ $audiencesCibles[$annonce->audience_cible] ?? $annonce->audience_cible }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            ...
                            <div id="preview-diffusion" class="flex flex-col items-end space-y-1">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 {{ $annonce->afficher_site_web ? '' : 'hidden' }}" id="badge-web">
                                    Site web
                                </span>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 {{ $annonce->annoncer_culte ? '' : 'hidden' }}" id="badge-culte">
                                    Culte
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="p-6 space-y-3 text-sm">
                        ...
                        @if($annonce->auteur)
                            <div class="pt-2 border-t border-slate-100">
                                ...
                                <span class="text-slate-600">{{ $annonce->auteur->nom }} {{ $annonce->auteur->prenom }}</span>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
            <div class="p-6">
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    ...
                    @if($annonce->statut === 'brouillon')
                        @can('publish', $annonce)
                            <button type="button" onclick="publierAnnonce('{{ $annonce->id }}')" class="inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                                <i class="fas fa-paper-plane mr-2"></i> Publier maintenant
                            </button>
                        @endcan
                    @endif

                    <a href="{{ route('private.annonces.show', $annonce) }}" class="inline-flex items-center justify-center px-8 py-3 bg-slate-600 text-white font-medium rounded-xl hover:bg-slate-700 transition-colors">
...
function publierAnnonce(annonceId) {
    if (confirm('Êtes-vous sûr de vouloir publier cette annonce ? Elle sera visible par le public.')) {
        ...
        .then(data => {
            if (data.success) {
                window.location.href = `{{ url('dashboard/annonces') }}/${annonceId}`;
            } else {
