...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">{{ $annonce->titre }}</h1>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="p-6">
                    <div class="flex flex-wrap gap-3">
                        @can('update', $annonce)
                            <a href="{{ route('private.annonces.edit', $annonce) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 text-white text-sm font-medium rounded-xl hover:from-yellow-700 hover:to-orange-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        ...
                        @can('publish', $annonce)
                            @if($annonce->statut === 'brouillon')
                                <button onclick="publierAnnonce('{{ $annonce->id }}')" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        ...
                        @can('archive', $annonce)
                            @if($annonce->statut === 'publiee')
                                <button onclick="archiverAnnonce('{{ $annonce->id }}')" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-orange-600 to-red-600 text-white text-sm font-medium rounded-xl hover:from-orange-700 hover:to-red-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        ...
                        @can('duplicate', $annonce)
                            <button onclick="dupliquerAnnonce('{{ $annonce->id }}')" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-medium rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all duration-200 shadow-md hover:shadow-lg">
                                <i class="fas fa-copy mr-2"></i> Dupliquer
                            </button>
                        @endcan

                        @can('delete', $annonce)
                            <button onclick="supprimerAnnonce('{{ $annonce->id }}')" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-red-600 to-red-700 text-white text-sm font-medium rounded-xl hover:from-red-700 hover:to-red-800 transition-all duration-200 shadow-md hover:shadow-lg">
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            <div class="flex flex-wrap items-center gap-3">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $annonce->badge_statut }}">
                                    {{ \App\Models\Annonce::getStatuts()[$annonce->statut] ?? $annonce->statut }}
                                </span>
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $annonce->badge_priorite }}">
                        ...
                        <div class="text-right">
                            ...
                            <p class="font-semibold text-slate-700">{{ \App\Models\Annonce::getAudiencesCibles()[$annonce->audience_cible] ?? $annonce->audience_cible }}</p>
                        </div>
                    </div>
                </div>
                <div class="p-6">
                    <div class="prose max-w-none text-slate-700">
                        <x-ckeditor-display :model="$annonce" field="contenu" show-meta="true" class="bg-slate-50 p-4 rounded-lg" />
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-center justify-between py-3 border-b border-slate-100">
                        ...
                        @if($annonce->auteur)
                            <span class="text-sm text-slate-600">{{ $annonce->auteur->nom }} {{ $annonce->auteur->prenom }}</span>
                        @endif
                    </div>
                        ...
                        <div class="flex items-center justify-between py-3 border-b border-slate-100">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-600">{{ $annonce->publie_le->format('d/m/Y à H:i') }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                        ...
                        <div class="flex items-center justify-between py-3">
                            <div class="flex items-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-sm text-slate-600">{{ $annonce->updated_at->format('d/m/Y à H:i') }}</p>
                                </div>
                            </div>
                        </div>
                    @endif
                            ...
                            @if($annonce->jours_restants !== null)
                                <div class="mt-2">
                                    @if($annonce->jours_restants > 0)
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $annonce->jours_restants <= 3 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                                            {{ $annonce->jours_restants }} jour{{ $annonce->jours_restants > 1 ? 's' : '' }} restant{{ $annonce->jours_restants > 1 ? 's' : '' }}
                                        </span>
                                    @else
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <p class="font-semibold text-slate-800">{{ $annonce->contactPrincipal->nom }} {{ $annonce->contactPrincipal->prenom }}</p>
                                <div class="space-y-1">
                                    <a href="mailto:{{ $annonce->contactPrincipal->email }}" class="flex items-center text-sm text-blue-600 hover:text-blue-800 transition-colors">
                                        <i class="fas fa-envelope w-4 mr-2"></i>
                                        {{ $annonce->contactPrincipal->email }}
                                    </a>
                                    @if($annonce->contactPrincipal->telephone)
                                        <a href="tel:{{ $annonce->contactPrincipal->telephone }}" class="flex items-center text-sm text-blue-600 hover:text-blue-800 transition-colors">
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="text-center">
                            <div class="text-2xl font-bold text-slate-800">{{ $annonce->created_at->diffForHumans() }}</div>
                            <div class="text-sm text-slate-600">Créée</div>
                        </div>

                        @if($annonce->publie_le)
                            <div class="text-center pt-3 border-t border-slate-100">
                                <div class="text-2xl font-bold text-green-600">{{ $annonce->publie_le->diffForHumans() }}</div>
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
                    <div class="p-6">
                        ...
                        <div class="flex flex-col space-y-3">
                            @can('update', $annonce)
                                <a href="{{ route('private.annonces.edit', $annonce->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-yellow-600 text-white text-sm font-medium rounded-xl hover:bg-yellow-700 transition-colors">
                                    <i class="fas fa-edit mr-2"></i> Modifier
                                </a>
                            @endcan
                            ...
                            @can('publish', $annonce)
                                @if($annonce->statut === 'brouillon')
                                    <button onclick="publierAnnonce('{{ $annonce->id }}')" class="inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-xl hover:bg-green-700 transition-colors">
                                        <i class="fas fa-paper-plane mr-2"></i> Publier
                                    </button>
                                @endif
                            @endcan
