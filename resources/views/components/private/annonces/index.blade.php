...
<div class="space-y-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Communication et annonces pour la communauté - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
    </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    @can('create', App\Models\Annonce::class)
                        <a href="{{ route('private.annonces.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                            <i class="fas fa-plus mr-2"></i> Nouvelle Annonce
                        </a>
                    @endcan
                    <a href="{{ route('private.annonces.actives') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.annonces.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-2">Recherche</label>
                    <div class="relative">
                        <input type="text" name="search" value="{{ $filtres['search'] ?? '' }}" placeholder="Titre, contenu..." class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div>
                    ...
                    <select name="statut" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($statuts as $key => $label)
                            <option value="{{ $key }}" {{ ($filtres['statut'] ?? '') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="type_annonce" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($typesAnnonces as $key => $label)
                            <option value="{{ $key }}" {{ ($filtres['type_annonce'] ?? '') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="niveau_priorite" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($niveauxPriorite as $key => $label)
                            <option value="{{ $key }}" {{ ($filtres['niveau_priorite'] ?? '') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="audience_cible" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($audiencesCibles as $key => $label)
                            <option value="{{ $key }}" {{ ($filtres['audience_cible'] ?? '') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="lg:col-span-6 flex gap-2 pt-4">
                    ...
                    <a href="{{ route('private.annonces.index') }}" class="inline-flex items-center px-4 py-2 bg-slate-600 text-white text-sm font-medium rounded-xl hover:bg-slate-700 transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $annonces->where('statut', 'publiee')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $annonces->where('statut', 'brouillon')->count() }}</p>
        ...
        <div class="p-6">
            @if($annonces->count() > 0)
                        ...
                        <div class="bg-gradient-to-br from-white to-slate-50 rounded-xl border border-slate-200 p-6 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="flex-1">
                                    ...
                                    <p class="text-sm text-slate-600">{{ $typesAnnonces[$annonce->type_annonce] ?? $annonce->type_annonce }}</p>
                                </div>
                                <div class="flex flex-col items-end space-y-2">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $annonce->badge_statut }}">
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-slate-200">
                                <div class="flex items-center space-x-2">
                                    @can('view', $annonce)
                                        <a href="{{ route('private.annonces.show', $annonce) }}" class="inline-flex items-center justify-center w-8 h-8 text-cyan-600 bg-cyan-100 rounded-lg hover:bg-cyan-200 transition-colors" title="Voir">
                                    ...
                                    @can('publish', $annonce)
                                        @if($annonce->statut === 'brouillon')
                                            <button type="button" onclick="publierAnnonce('{{ $annonce->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-100 rounded-lg hover:bg-green-200 transition-colors" title="Publier">
                                    ...
                                    @can('archive', $annonce)
                                        @if($annonce->statut === 'publiee')
                                            <button type="button" onclick="archiverAnnonce('{{ $annonce->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-orange-600 bg-orange-100 rounded-lg hover:bg-orange-200 transition-colors" title="Archiver">
                                                <i class="fas fa-archive text-sm"></i>
                                            </button>
                                        @endif
                                    @endcan

                                    @can('duplicate', $annonce)
                                        <button type="button" onclick="dupliquerAnnonce('{{ $annonce->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-purple-600 bg-purple-100 rounded-lg hover:bg-purple-200 transition-colors" title="Dupliquer">
                                ...
                                @can('delete', $annonce)
                                    <button type="button" onclick="supprimerAnnonce('{{ $annonce->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Supprimer">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6 pt-6 border-t border-slate-200">
                    <div class="text-sm text-slate-700">
                        Affichage de <span class="font-medium">{{ $annonces->firstItem() }}</span> à <span class="font-medium">{{ $annonces->lastItem() }}</span>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    @can('create', App\Models\Annonce::class)
                        <a href="{{ route('private.annonces.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
