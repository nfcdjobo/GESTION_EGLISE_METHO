...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="bg-white rounded-xl shadow-lg p-6 border border-slate-200">
            ...
            <p class="text-slate-500 mt-1">Planification et suivi des interventions - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('private.interventions.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-plus mr-2"></i> Nouvelle Intervention
                    </a>
                    <a href="{{ route('private.interventions.trash') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-gray-600 to-slate-600 text-white text-sm font-medium rounded-xl hover:from-gray-700 hover:to-slate-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-trash mr-2"></i> Corbeille
                    </a>
                    <a href="{{ route('private.interventions.par-evenement') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.interventions.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-2">Recherche</label>
                    <div class="relative">
                        <input type="text" name="search" value="{{ $currentFilters['search'] ?? '' }}" placeholder="Titre, description, passage biblique..." class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div>
                    ...
                    <select name="reunion_id" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($reunions as $reunion)
                            <option value="{{ $reunion->id }}" {{ ($currentFilters['reunion_id'] ?? '') == $reunion->id ? 'selected' : '' }}>
                                {{ $reunion->titre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="intervenant_id" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($intervenants as $intervenant)
                            <option value="{{ $intervenant->id }}" {{ ($currentFilters['intervenant_id'] ?? '') == $intervenant->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="type_intervention" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach($filters['types_intervention'] as $key => $label)
                            <option value="{{ $key }}" {{ ($currentFilters['type_intervention'] ?? '') == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="lg:col-span-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select name="statut" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            ...
                            @foreach($filters['statuts'] as $key => $label)
                                <option value="{{ $key }}" {{ ($currentFilters['statut'] ?? '') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center pt-6">
                        <input type="checkbox" name="ordre_passage" value="1" {{ request('ordre_passage') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $interventions->where('statut', 'terminee')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $interventions->where('statut', 'prevue')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $interventions->where('statut', 'annulee')->count() }}</p>
        ...
        <div class="p-6">
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($interventions as $intervention)
                                <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <div class="text-sm">
                                            @if($intervention->ordre_passage)
                                                <div class="flex items-center mb-1">
                                                    ...
                                                    <span class="font-medium">{{ $intervention->ordre_passage }}</span>
                                                </div>
                                            @endif
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('private.interventions.show', $intervention) }}" class="inline-flex items-center justify-center w-8 h-8 text-cyan-600 bg-cyan-100 rounded-lg hover:bg-cyan-200 transition-colors" title="Voir">
                                                <i class="fas fa-eye text-sm"></i>
                                            </a>
                                            <a href="{{ route('private.interventions.edit', $intervention) }}" class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors" title="Modifier">
                                                <i class="fas fa-edit text-sm"></i>
                                            </a>
                                            <form method="POST" action="{{ route('private.interventions.change-statut', $intervention) }}" class="inline-block">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <button type="button" onclick="deleteIntervention('{{ $intervention->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Supprimer">
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <a href="{{ route('private.interventions.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
