...
@section('title', 'Corbeille des Interventions')
...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="bg-white rounded-xl shadow-lg p-6 border border-slate-200">
            <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Corbeille des Interventions</h1>
            <p class="text-slate-500 mt-1">Interventions supprimées - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
        </div>
    </div>
        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('private.interventions.index') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-arrow-left mr-2"></i> Retour aux Interventions
                    </a>
                    @if($interventions->count() > 0)
                        <button type="button" onclick="restoreAll()" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                            <i class="fas fa-undo mr-2"></i> Tout Restaurer
                        </button>
                        <button type="button" onclick="deleteAllPermanently()" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-red-600 to-pink-600 text-white text-sm font-medium rounded-xl hover:from-red-700 hover:to-pink-700 transition-all duration-200 shadow-md hover:shadow-lg">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $interventions->where('type_intervention', 'predication')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $interventions->where('type_intervention', 'temoignage')->count() }}</p>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">
                        @if($interventions->count() > 0)
                            {{ $interventions->min('deleted_at')->diffInDays() }}
                        @else
                            0
                        @endif
                    </p>
        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <h2 class="text-xl font-bold text-slate-800 flex items-center">
                    ...
                    Liste des Interventions Supprimées ({{ $interventions->total() }})
                </h2>
                @if($interventions->count() > 0)
            ...
            @if($interventions->count() > 0)
                    ...
                    <table class="min-w-full">
                        <thead>
                            <tr class="border-b border-slate-200">
                                ...
                                <th class="px-4 py-3 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">Supprimée le</th>
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($interventions as $intervention)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-4 py-4">
                                        <input type="checkbox" name="selected_interventions[]" value="{{ $intervention->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 intervention-checkbox">
                                    </td>
                                    <td class="px-4 py-4">
                                        <div>
                                            <div class="font-semibold text-slate-900">{{ $intervention->titre }}</div>
                                            @if($intervention->description)
                                                <div class="text-sm text-slate-500">{{ Str::limit($intervention->description, 50) }}</div>
                                            @endif
                                            @if($intervention->passage_biblique)
                                                <div class="text-xs text-blue-600 font-medium">{{ $intervention->passage_biblique }}</div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            <span class="text-sm font-medium text-slate-900">{{ $intervention->intervenant->nom }}</span>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        @if($intervention->culte)
                                            <div class="flex items-center text-sm text-gray-600">
                                                ...
                                                {{ $intervention->culte->nom }}
                                            </div>
                                        @elseif($intervention->reunion)
                                            <div class="flex items-center text-sm text-gray-600">
                                                <i class="fas fa-users text-gray-400 mr-2"></i>
                                                {{ $intervention->reunion->nom }}
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="text-sm">
                                            <div class="text-slate-900 font-medium">{{ $intervention->deleted_at->format('d/m/Y') }}</div>
                                            <div class="text-slate-500">{{ $intervention->deleted_at->format('H:i') }}</div>
                                            <div class="text-xs text-slate-400">{{ $intervention->deleted_at->diffForHumans() }}</div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-2">
                                            <button type="button" onclick="restoreIntervention('{{ $intervention->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-100 rounded-lg hover:bg-green-200 transition-colors" title="Restaurer">
                                                <i class="fas fa-undo text-sm"></i>
                                            </button>
                                            <button type="button" onclick="deleteInterventionPermanently('{{ $intervention->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Supprimer définitivement">
                ...
                @if($interventions->count() > 0)
                    <div class="mt-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
                        <div class="flex flex-wrap items-center gap-3">
                            ...
                            <button type="button" onclick="restoreSelected()" class="inline-flex items-center px-3 py-1.5 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 transition-colors">
                                <i class="fas fa-undo mr-1"></i> Restaurer
                            </button>
                            <button type="button" onclick="deleteSelectedPermanently()" class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6 pt-6 border-t border-slate-200">
                    <div class="text-sm text-slate-700">
                        Affichage de <span class="font-medium">{{ $interventions->firstItem() }}</span> à <span class="font-medium">{{ $interventions->lastItem() }}</span>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-slate-500 mb-6">
                        Aucune intervention supprimée. Les interventions supprimées apparaîtront ici et pourront être restaurées.
                    </p>
                    <a href="{{ route('private.interventions.index') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-arrow-left mr-2"></i> Retour aux interventions
                    </a>
                </div>
            @endif
...
<div id="restoreModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl max-w-md w-full">
        <div class="p-6">
            ...
            <p class="text-slate-600 mb-2" id="restoreMessage">Êtes-vous sûr de vouloir restaurer cette intervention ?</p>
        </div>
        <div class="flex items-center justify-end space-x-3 p-6 border-t border-slate-200">
            <button type="button" onclick="closeRestoreModal()" class="px-4 py-2 text-slate-700 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">
                Annuler
            </button>
            <button type="button" id="confirmRestore" class="px-4 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-colors">
                Restaurer
            </button>
        </div>
    </div>
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl max-w-md w-full">
        <div class="p-6">
            ...
            <p class="text-slate-600 mb-2" id="deleteMessage">Êtes-vous sûr de vouloir supprimer définitivement cette intervention ?</p>
...
function restoreIntervention(interventionId) {
    ...
    document.getElementById('confirmRestore').onclick = function() {
        fetch(`{{ route('private.interventions.index') }}/${interventionId}/restore`, {
            method: 'PATCH',
            headers: {
...
function deleteInterventionPermanently(interventionId) {
    ...
    document.getElementById('confirmDelete').onclick = function() {
        ...
        fetch(`{{ route('private.interventions.index') }}/${interventionId}/force-delete`, {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
...
function restoreSelected() {
    ...
    document.getElementById('confirmRestore').onclick = function() {
        ...
        Promise.all(selected.map(id =>
            fetch(`{{ route('private.interventions.index') }}/${id}/restore`, {
                method: 'PATCH',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
...
function deleteSelectedPermanently() {
    ...
    document.getElementById('confirmDelete').onclick = function() {
        ...
        Promise.all(selected.map(id =>
            fetch(`{{ route('private.interventions.index') }}/${id}/force-delete`, {
                method: 'DELETE',
                headers: {
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                    'Content-Type': 'application/json',
