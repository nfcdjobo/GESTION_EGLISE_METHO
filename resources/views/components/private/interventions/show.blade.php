...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">{{ $intervention->titre }}</h1>
    ...
    <div class="flex flex-wrap gap-3 mb-6">
        <a href="{{ route('private.interventions.edit', $intervention) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-600 to-orange-600 text-white text-sm font-medium rounded-xl hover:from-yellow-700 hover:to-orange-700 transition-all duration-200 shadow-md hover:shadow-lg">
            <i class="fas fa-edit mr-2"></i> Modifier
        </a>
        <form method="POST" action="{{ route('private.interventions.change-statut', $intervention) }}" class="inline-block">
            @csrf
            @method('PATCH')
            <select name="statut" onchange="this.form.submit()" class="px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white border-0 rounded-xl text-sm font-medium hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                @foreach(\App\Models\Intervention::STATUTS as $key => $label)
                    <option value="{{ $key }}" {{ $intervention->statut == $key ? 'selected' : '' }} class="bg-white text-slate-900">
                        {{ $label }}
                    </option>
                @endforeach
            </select>
        </form>
        <button type="button" onclick="deleteIntervention()" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-red-600 to-pink-600 text-white text-sm font-medium rounded-xl hover:from-red-700 hover:to-pink-700 transition-all duration-200 shadow-md hover:shadow-lg">
                    ...
                    @if($intervention->passage_biblique)
                        <div class="mb-6">
                            ...
                            <div class="bg-gradient-to-r from-purple-50 to-blue-50 rounded-xl p-4 border border-purple-200">
                                <p class="text-purple-800 font-medium text-lg">{{ $intervention->passage_biblique }}</p>
                            </div>
                        </div>
                    @endif
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        @if($intervention->heure_debut)
                            <div class="text-center">
                                ...
                                <div class="text-2xl font-bold text-slate-800">{{ $intervention->heure_debut->format('H:i') }}</div>
                        ...
                        @if($intervention->heure_debut && $intervention->heure_fin)
                            <div class="text-center">
                                ...
                                <div class="text-2xl font-bold text-slate-800">{{ $intervention->heure_fin->format('H:i') }}</div>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                ...
                <div class="p-6">
                    @if($intervention->culte)
                        <div class="flex items-center p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl border border-blue-200">
                            ...
                            <div class="ml-4 flex-1">
                                ...
                                @if($intervention->culte->date_culte)
                                    <p class="text-blue-600">{{ $intervention->culte->date_culte->format('l d F Y') }}</p>
                                @endif
                    ...
                    @elseif($intervention->reunion)
                        <div class="flex items-center p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200">
                            ...
                            <div class="ml-4 flex-1">
                                <h3 class="text-lg font-semibold text-green-800">{{ $intervention->reunion->nom }}</h3>
                                @if($intervention->reunion->date_reunion)
                                    <p class="text-green-600">{{ $intervention->reunion->date_reunion->format('l d F Y') }}</p>
                                @endif
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                ...
                <div class="p-6">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-slate-800">{{ $intervention->intervenant->nom }}</h3>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                ...
                <div class="p-6 space-y-3">
                    <a href="{{ route('private.interventions.edit', $intervention) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors">
                        <i class="fas fa-edit mr-2"></i> Modifier
                    </a>

                    @if($intervention->culte)
                        <a href="{{ route('private.interventions.par-evenement', ['culte_id' => $intervention->culte->id]) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-cyan-600 text-white text-sm font-medium rounded-xl hover:bg-cyan-700 transition-colors">
                            <i class="fas fa-church mr-2"></i> Autres interventions du culte
                        </a>
                    @endif

                    @if($intervention->reunion)
                        <a href="{{ route('private.interventions.par-evenement', ['reunion_id' => $intervention->reunion->id]) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white text-sm font-medium rounded-xl hover:bg-green-700 transition-colors">
                            <i class="fas fa-users mr-2"></i> Autres interventions de la réunion
                        </a>
                    @endif

                    <a href="{{ route('private.interventions.index', ['intervenant_id' => $intervention->intervenant->id]) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-purple-600 text-white text-sm font-medium rounded-xl hover:bg-purple-700 transition-colors">
