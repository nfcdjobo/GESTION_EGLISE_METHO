...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="bg-white rounded-xl shadow-lg p-6 border border-slate-200">
            ...
            <p class="text-slate-500 mt-1">Gestion de la médiathèque de la communauté - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
        </div>
    </div>
        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('private.multimedia.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-upload mr-2"></i> Télécharger un Média
                    </a>
                    @can('moderate_media')
                        <a href="{{ route('private.multimedia.moderation.queue') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-orange-600 to-red-600 text-white text-sm font-medium rounded-xl hover:from-orange-700 hover:to-red-700 transition-all duration-200 shadow-md hover:shadow-lg">
                    ...
                    @endcan
                    <a href="{{ route('private.multimedia.galerie') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-eye mr-2"></i> Vue Publique
                    </a>
                    @can('manage_media')
                        <a href="{{ route('private.multimedia.statistiques') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white text-sm font-medium rounded-xl hover:from-purple-700 hover:to-indigo-700 transition-all duration-200 shadow-md hover:shadow-lg">
            ...
            <form method="GET" action="{{ route('private.multimedia.index') }}" class="space-y-6" id="filterForm">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
                    <div class="lg:col-span-2">
                        ...
                        <div class="relative">
                            <input type="text" name="search" value="{{ $currentFilters['search'] ?? '' }}" placeholder="Titre, description, photographe, lieu..." class="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                    ...
                    <div>
                        ...
                        <select name="culte_id" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            ...
                            @foreach($cultes as $culte)
                                <option value="{{ $culte->id }}" {{ ($currentFilters['culte_id'] ?? '') == $culte->id ? 'selected' : '' }}>
                                    {{ $culte->titre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="event_id" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            ...
                            @foreach($events as $event)
                                <option value="{{ $event->id }}" {{ ($currentFilters['event_id'] ?? '') == $event->id ? 'selected' : '' }}>
                                    {{ $event->titre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="statut_moderation" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            ...
                            @foreach($filters['statuts_moderation'] as $key => $label)
                                <option value="{{ $key }}" {{ ($currentFilters['statut_moderation'] ?? '') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="niveau_acces" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            ...
                            @foreach($filters['niveaux_acces'] as $key => $label)
                                <option value="{{ $key }}" {{ ($currentFilters['niveau_acces'] ?? '') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="telecharge_par" class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            ...
                            @foreach($uploaders as $uploader)
                                <option value="{{ $uploader->id }}" {{ ($currentFilters['telecharge_par'] ?? '') == $uploader->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <div class="space-y-2 pt-1">
                            <label class="flex items-center">
                                <input type="checkbox" name="featured_only" value="1" {{ request('featured_only') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                <span class="ml-2 text-sm text-slate-700">À la une uniquement</span>
                            </label>
                            <label class="flex items-center">
                                <input type="checkbox" name="visible_only" value="1" {{ request('visible_only') ? 'checked' : '' }} class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                ...
                <div class="flex gap-2 pt-4">
                    ...
                    <a href="{{ route('private.multimedia.index') }}" class="inline-flex items-center px-6 py-3 bg-slate-600 text-white text-sm font-medium rounded-xl hover:bg-slate-700 transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $multimedia->where('type_media', 'image')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $multimedia->where('type_media', 'video')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $multimedia->where('type_media', 'audio')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $multimedia->where('est_featured', true)->count() }}</p>
        ...
        <div class="p-6">
            @if($multimedia->count() > 0)
                        ...
                        <div class="group relative bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                            @can('moderate_media')
                                <div class="absolute top-2 left-2 z-10">
                                    <input type="checkbox" name="selected_media[]" value="{{ $media->id }}" class="w-4 h-4 text-blue-600 bg-white border-gray-300 rounded focus:ring-blue-500 media-checkbox">
                                </div>
                            @endcan
                            ...
                            <div class="aspect-square relative overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200">
                                @if($media->est_image && $media->url_miniature)
                                    <img src="{{ asset($media->url_miniature) }}" alt="{{ $media->alt_text ?? $media->titre }}" class="w-full h-full object-cover">
                                @elseif($media->est_image && $media->url_complete)
                                    <img src="{{ asset($media->url_complete) }}" alt="{{ $media->alt_text ?? $media->titre }}" class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full flex items-center justify-center">
                                        <div class="text-center">
                                            ...
                                            @endif
                                            <p class="text-xs text-slate-500 uppercase font-medium">{{ $media->extension }}</p>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                    <div class="flex flex-col space-y-1">
                                        <a href="{{ route('private.multimedia.show', $media) }}" class="inline-flex items-center justify-center w-8 h-8 bg-white/90 text-slate-700 rounded-lg hover:bg-white transition-colors" title="Voir">
                                            <i class="fas fa-eye text-sm"></i>
                                        </a>
                                        @can('update', $media)
                                            <a href="{{ route('private.multimedia.edit', $media) }}" class="inline-flex items-center justify-center w-8 h-8 bg-white/90 text-slate-700 rounded-lg hover:bg-white transition-colors" title="Modifier">
                                                <i class="fas fa-edit text-sm"></i>
                                            </a>
                                        @endcan
                                        <a href="{{ route('private.multimedia.download', $media) }}" class="inline-flex items-center justify-center w-8 h-8 bg-white/90 text-slate-700 rounded-lg hover:bg-white transition-colors" title="Télécharger">
                            ...
                            <div class="p-4">
                                <div class="mb-2">
                                    <h3 class="font-semibold text-slate-900 text-sm line-clamp-2 mb-1">{{ $media->titre }}</h3>
                                ...
                                <div class="mt-3 flex items-center justify-between">
                                    ...
                                    @can('moderate_media')
                                        <div class="flex items-center space-x-1">
                                            @if($media->statut_moderation == 'en_attente')
                                                <button type="button" onclick="moderateMedia('{{ $media->id }}', 'approve')" class="text-green-600 hover:text-green-700 transition-colors" title="Approuver">
                                                    <i class="fas fa-check text-sm"></i>
                                                </button>
                                                <button type="button" onclick="moderateMedia('{{ $media->id }}', 'reject')" class="text-red-600 hover:text-red-700 transition-colors" title="Rejeter">
                                                    <i class="fas fa-times text-sm"></i>
                                                </button>
                                            @endif
                                            @can('feature_media')
                                                <button type="button" onclick="toggleFeatured('{{ $media->id }}')" class="text-yellow-600 hover:text-yellow-700 transition-colors {{ $media->est_featured ? 'opacity-100' : 'opacity-50' }}" title="Mettre à la une">
                ...
                <div id="listView" class="hidden space-y-4">
                    @foreach($multimedia as $media)
                        <div class="flex items-center space-x-4 p-4 bg-white border border-slate-200 rounded-xl hover:shadow-md transition-all duration-300">
                            @can('moderate_media')
                                <div class="flex-shrink-0">
                                    <input type="checkbox" name="selected_media[]" value="{{ $media->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 media-checkbox">
                                </div>
                            @endcan
                            ...
                            <div class="flex-shrink-0 w-16 h-16 bg-gradient-to-br from-slate-100 to-slate-200 rounded-lg overflow-hidden">
                                @if($media->est_image && $media->url_miniature)
                                    <img src="{{ asset($media->url_miniature) }}" alt="{{ $media->titre }}" class="w-full h-full object-cover">
                                @else
                                    <div class="w-full h-full flex items-center justify-center">
                                        @if($media->type_media == 'video')
                            ...
                            <!-- Actions -->
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('private.multimedia.show', $media) }}" class="inline-flex items-center justify-center w-8 h-8 text-cyan-600 bg-cyan-100 rounded-lg hover:bg-cyan-200 transition-colors" title="Voir">
                                    <i class="fas fa-eye text-sm"></i>
                                </a>
                                @can('update', $media)
                                    <a href="{{ route('private.multimedia.edit', $media) }}" class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors" title="Modifier">
                                        <i class="fas fa-edit text-sm"></i>
                                    </a>
                                @endcan
                                <a href="{{ route('private.multimedia.download', $media) }}" class="inline-flex items-center justify-center w-8 h-8 text-blue-600 bg-blue-100 rounded-lg hover:bg-blue-200 transition-colors" title="Télécharger">
                                    <i class="fas fa-download text-sm"></i>
                                </a>
                                @can('moderate_media')
                                    @if($media->statut_moderation == 'en_attente')
                                        <button type="button" onclick="moderateMedia('{{ $media->id }}', 'approve')" class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-100 rounded-lg hover:bg-green-200 transition-colors" title="Approuver">
                                            <i class="fas fa-check text-sm"></i>
                                        </button>
                                        <button type="button" onclick="moderateMedia('{{ $media->id }}', 'reject')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Rejeter">
                                            <i class="fas fa-times text-sm"></i>
                                        </button>
                                    @endif
                                @endcan
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-8 pt-6 border-t border-slate-200">
                    <div class="text-sm text-slate-700">
                        Affichage de <span class="font-medium">{{ $multimedia->firstItem() }}</span> à <span class="font-medium">{{ $multimedia->lastItem() }}</span>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <a href="{{ route('private.multimedia.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-upload mr-2"></i> Télécharger un média
                    </a>
                </div>
            @endif
...
function toggleFeatured(mediaId) {
    fetch(`{{ route('private.multimedia.index') }}/${mediaId}/toggle-featured`, {
        method: 'PATCH',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json'
