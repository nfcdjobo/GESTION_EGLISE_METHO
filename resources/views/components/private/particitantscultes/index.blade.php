...
@section('content')
    <div class="space-y-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            @canany(['participants-cultes.nouveaux-visiteurs', 'participants-cultes.statistics'])
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @can('participants-cultes.nouveaux-visiteurs')
                                <a href="{{ route('private.participantscultes.nouveaux-visiteurs') }}"
                            ...
                            @can('participants-cultes.statistics')
                                <a href="{{ route('private.participantscultes.statistiques') }}"
            ...
            @endcanany
            <div class="p-6">
                <form method="GET" action="{{ route('private.participantscultes.index') }}"
                    ...
                    <div class="lg:col-span-2">
                        ...
                        <div class="relative">
                            <input type="text" name="participant_search" value="{{ request('participant_search') }}"
                        ...
                        <select name="culte_id"
                            ...
                            @foreach ($cultes as $key => $culte)
                                <option value="{{ $culte->id }}">
                                    {{ $culte->titre . ' (' . $culte->date_culte->format('Y-h-d') . ')' }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="statut_presence"
                            ...
                            <option value="present" {{ request('statut_presence') == 'present' ? 'selected' : '' }}>Présent
                            </option>
                            <option value="present_partiel"
                                {{ request('statut_presence') == 'present_partiel' ? 'selected' : '' }}>Présent Partiel
                            </option>
                            <option value="en_retard" {{ request('statut_presence') == 'en_retard' ? 'selected' : '' }}>En
                                Retard</option>
                            <option value="parti_tot" {{ request('statut_presence') == 'parti_tot' ? 'selected' : '' }}>
                                Parti Tôt</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="type_participation"
                            ...
                            <option value="physique" {{ request('type_participation') == 'physique' ? 'selected' : '' }}>
                                Physique</option>
                            <option value="en_ligne" {{ request('type_participation') == 'en_ligne' ? 'selected' : '' }}>En
                                Ligne</option>
                            <option value="hybride" {{ request('type_participation') == 'hybride' ? 'selected' : '' }}>
                                Hybride</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="role_culte"
                            ...
                            <option value="participant" {{ request('role_culte') == 'participant' ? 'selected' : '' }}>
                                Participant</option>
                            <option value="equipe_technique"
                                {{ request('role_culte') == 'equipe_technique' ? 'selected' : '' }}>Équipe Technique
                            </option>
                            <option value="equipe_louange"
                                {{ request('role_culte') == 'equipe_louange' ? 'selected' : '' }}>Équipe Louange</option>
                            <option value="equipe_accueil"
                                {{ request('role_culte') == 'equipe_accueil' ? 'selected' : '' }}>Équipe Accueil</option>
                            <option value="orateur" {{ request('role_culte') == 'orateur' ? 'selected' : '' }}>Orateur
                            </option>
                            <option value="dirigeant" {{ request('role_culte') == 'dirigeant' ? 'selected' : '' }}>
                                Dirigeant</option>
                            <option value="nouveau_visiteur"
                                {{ request('role_culte') == 'nouveau_visiteur' ? 'selected' : '' }}>Nouveau Visiteur
                            </option>
                        </select>
                    </div>
                            ...
                            <div class="w-full space-y-2">
                                <div class="flex items-center">
                                    <input type="checkbox" name="premiere_visite" value="1"
                                        {{ request('premiere_visite') ? 'checked' : '' }} id="premiere_visite"
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" name="necessite_suivi" value="1"
                                        {{ request('necessite_suivi') ? 'checked' : '' }} id="necessite_suivi"
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ $participations->where('statut_presence', 'present')->count() ?? 0 }}</p>
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ $participations->where('premiere_visite', true)->count() ?? 0 }}</p>
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ $participations->where('demande_contact_pastoral', true)->count() ?? 0 }}</p>
    ...
    function setView(view) {
        if (view === 'list') {
            ...
            function editParticipation(participantId, culteId) {
                ...
                return;
                window.location.href = `{{route('private.participantscultes.update', [':participant', ':culte'])}}`.replace(':participant', participantId).replace(':culte', culteId);
            }
</script>
    @endpush
@endsection
