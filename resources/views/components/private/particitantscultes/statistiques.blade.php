...
@section('title', 'Statistiques des Participations')
...
<div class="space-y-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-users text-slate-500">Analyse détaillée des participations aux cultes et tendances de fréquentation</span>
                    </div>
                </li>
            </ol>
        </nav>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.participantscultes.statistiques') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Date début</label>
                    <input type="date" name="date_debut" value="{{ request('date_debut', now()->subMonths(6)->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Date fin</label>
                    <input type="date" name="date_fin" value="{{ request('date_fin', now()->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                </div>
                <div>
                    ...
                    <select name="type_culte" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        <option value="dimanche_matin" {{ request('type_culte') == 'dimanche_matin' ? 'selected' : '' }}>Dimanche Matin</option>
                        <option value="dimanche_soir" {{ request('type_culte') == 'dimanche_soir' ? 'selected' : '' }}>Dimanche Soir</option>
                        <option value="mercredi" {{ request('type_culte') == 'mercredi' ? 'selected' : '' }}>Mercredi</option>
                        <option value="vendredi" {{ request('type_culte') == 'vendredi' ? 'selected' : '' }}>Vendredi</option>
                        <option value="special" {{ request('type_culte') == 'special' ? 'selected' : '' }}>Spécial</option>
                    </select>
                </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $stats['total_participations'] ?? 0 }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $stats['premieres_visites'] ?? 0 }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $stats['demandes_suivi'] ?? 0 }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $stats['participations_confirmees'] ?? 0 }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                @if(isset($stats['par_statut']) && count($stats['par_statut']) > 0)
                    <div class="space-y-4">
                        @foreach($stats['par_statut'] as $statut => $count)
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    <div class="w-4 h-4 rounded {{ $statutColors[$statut] ?? 'bg-gray-500' }}"></div>
                                    <span class="text-sm font-medium text-slate-700">{{ $statutLabels[$statut] ?? ucfirst($statut) }}</span>
                                </div>
                            ...
                            <div class="w-full bg-slate-200 rounded-full h-2">
                                <div class="h-2 rounded-full {{ $statutColors[$statut] ?? 'bg-gray-500' }}" style="width: {{ $percentage }}%"></div>
                            </div>
                        @endforeach
                    </div>
                @else
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                @if(isset($stats['par_type']) && count($stats['par_type']) > 0)
                    <div class="space-y-4">
                        @foreach($stats['par_type'] as $type => $count)
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-3">
                                    <div class="w-4 h-4 rounded {{ $typeColors[$type] ?? 'bg-gray-500' }}"></div>
                                    <span class="text-sm font-medium text-slate-700">{{ $typeLabels[$type] ?? ucfirst($type) }}</span>
                                </div>
                                <div class="flex items-center space-x-2">
                                    <span class="text-sm text-slate-500">{{ $count }}</span>
                                    <span class="text-sm font-bold text-slate-700">{{ $percentage }}%</span>
                            ...
                            <div class="w-full bg-slate-200 rounded-full h-2">
                                <div class="h-2 rounded-full {{ $typeColors[$type] ?? 'bg-gray-500' }}" style="width: {{ $percentage }}%"></div>
                            </div>
                        @endforeach
                    </div>
                @else
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-blue-800">
                                    {{ isset($stats['moyenne_par_culte']) ? round($stats['moyenne_par_culte'], 1) : '0' }}
                                </p>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-green-800">
                                    @if(isset($stats['par_statut']['present']) && $stats['total_participations'] > 0)
                                        {{ round(($stats['par_statut']['present'] / $stats['total_participations']) * 100, 1) }}%
                                    @else
                                        0%
                                    @endif
                                </p>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-purple-800">
                                    {{ isset($stats['croissance']) ? ($stats['croissance'] > 0 ? '+' : '') . $stats['croissance'] . '%' : '0%' }}
                                </p>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-2xl font-bold text-amber-800">
                                    {{ isset($stats['taux_fidelite']) ? round($stats['taux_fidelite'], 1) . '%' : '0%' }}
                                </p>
                            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                @if(isset($topParticipants) && count($topParticipants) > 0)
                    <div class="space-y-4">
                        @foreach($topParticipants as $index => $participant)
                            <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                                <div class="flex items-center space-x-3">
                                    <div class="w-8 h-8 rounded-full flex items-center justify-center {{ $index < 3 ? 'bg-gradient-to-r from-yellow-400 to-orange-500 text-white' : 'bg-slate-200 text-slate-600' }} font-bold text-sm">
                                        {{ $index + 1 }}
                                    </div>
                                    <div>
                                        <p class="font-medium text-slate-900">{{ $participant['nom'] ?? 'N/A' }} {{ $participant['prenom'] ?? '' }}</p>
                                        <p class="text-sm text-slate-500">{{ $participant['participations'] ?? 0 }} participations</p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-sm font-bold text-green-600">{{ $participant['taux_presence'] ?? 0 }}%</p>
                ...
                <div class="space-y-4">
                    @if(isset($insights) && count($insights) > 0)
                        @foreach($insights as $insight)
                            <div class="p-4 {{ $insight['type'] == 'positive' ? 'bg-green-50 border-green-200' : ($insight['type'] == 'warning' ? 'bg-yellow-50 border-yellow-200' : 'bg-blue-50 border-blue-200') }} border rounded-lg">
                                <div class="flex items-start space-x-3">
                                    <i class="fas {{ $insight['type'] == 'positive' ? 'fa-check-circle text-green-500' : ($insight['type'] == 'warning' ? 'fa-exclamation-triangle text-yellow-500' : 'fa-info-circle text-blue-500') }} mt-1"></i>
                                    <div>
                                        <p class="font-medium text-slate-900">{{ $insight['title'] ?? 'Insight' }}</p>
                                        <p class="text-sm text-slate-600 mt-1">{{ $insight['message'] ?? 'Message par défaut' }}</p>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                    @else
                        ...
                        @if(isset($stats['total_participations']) && $stats['total_participations'] > 0)
                                ...
                                <div class="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-slate-600 mt-1">
                                            Vous avez enregistré {{ $stats['total_participations'] }} participations sur la période sélectionnée.
                                            @if(isset($stats['premieres_visites']) && $stats['premieres_visites'] > 0)
                                                {{ $stats['premieres_visites'] }} nouvelles personnes ont visité l'église.
                                            @endif
                                        </p>
                                    </div>
                                </div>
                            ...
                            @if(isset($stats['par_statut']['present']) && $stats['total_participations'] > 0)
                                @php $tauxPresence = round(($stats['par_statut']['present'] / $stats['total_participations']) * 100, 1); @endphp
                                <div class="p-4 {{ $tauxPresence >= 80 ? 'bg-green-50 border-green-200' : ($tauxPresence >= 60 ? 'bg-yellow-50 border-yellow-200' : 'bg-red-50 border-red-200') }} border rounded-lg">
                                    <div class="flex items-start space-x-3">
                                        <i class="fas {{ $tauxPresence >= 80 ? 'fa-check-circle text-green-500' : ($tauxPresence >= 60 ? 'fa-exclamation-triangle text-yellow-500' : 'fa-times-circle text-red-500') }} mt-1"></i>
                                        <div>
                                            <p class="font-medium text-slate-900">Taux de présence</p>
                                            <p class="text-sm text-slate-600 mt-1">
                                                Le taux de présence complète est de {{ $tauxPresence }}%.
                                                @if($tauxPresence >= 80)
                                                    Excellent! Votre communauté est très assidue.
                                                @elseif($tauxPresence >= 60)
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
            <div class="flex flex-wrap gap-3">
                ...
                <a href="{{ route('private.participantscultes.nouveaux-visiteurs') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-medium rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all duration-200">
                    <i class="fas fa-user-plus mr-2"></i> Nouveaux visiteurs
                </a>
                @endcan
                <a href="{{ route('private.participantscultes.index') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-cyan-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-cyan-700 transition-all duration-200">
...
function exportStatistiques() {
    ...
    const exportUrl = new URL('{{ route("private.participantscultes.statistiques") }}', window.location.origin);
