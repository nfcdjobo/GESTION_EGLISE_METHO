...
<div class="space-y-8">
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <span class="text-sm font-medium text-slate-500">Paiement #{{ substr($payment['id'], 0, 8) }}</span>
                    </div>
                </li>
            </ol>
        </nav>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    @if($payment['statut'] === 'en_attente')
                        <a href="{{ route('private.paiements.edit', $payment['id']) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-yellow-500 to-orange-500 text-white text-sm font-medium rounded-xl hover:from-yellow-600 hover:to-orange-600 transition-all duration-200 shadow-md hover:shadow-lg">
                            <i class="fas fa-edit mr-2"></i> Modifier
                        </a>
                    @endif
                    <a href="{{ route('private.subscriptions.show', $payment['subscription']['id']) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                ...
                <div class="p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <p class="text-slate-900 text-lg">{{ \Carbon\Carbon::parse($payment['date_paiement'])->format('d/m/Y à H:i') }}</p>
                        </div>
                        <div>
                            <h4 class="font-medium text-slate-700 mb-2">Type de paiement</h4>
                            <p class="text-slate-900 text-lg capitalize">{{ str_replace('_', ' ', $payment['type_paiement']) }}</p>
                        </div>
                        @if($payment['reference_paiement'])
                            <div>
                                <h4 class="font-medium text-slate-700 mb-2">Référence</h4>
                                <code class="px-3 py-2 bg-slate-100 text-slate-800 rounded-lg text-sm">{{ $payment['reference_paiement'] }}</code>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="bg-gradient-to-r from-slate-50 to-blue-50 rounded-xl p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="text-center">
                                <div class="text-2xl font-bold text-green-600">{{ number_format($payment['montant'], 0, ',', ' ') }}</div>
                                <div class="text-sm text-green-700">Montant payé</div>
                            </div>
                            <div class="text-center">
                                <div class="text-lg font-semibold text-slate-600">{{ number_format($payment['ancien_reste'], 0, ',', ' ') }}</div>
                            ...
                            <div class="text-center">
    <div class="text-lg font-semibold text-orange-600">{{ ($payment['nouveau_reste'] < 0 ? '+' : '') . number_format(abs($payment['nouveau_reste']), 0, ',', ' ') }}</div>
    <div class="text-sm text-orange-700">{{ $payment['nouveau_reste'] >= 0 ? 'Reste après' : 'Montant supplementaire' }}</div>
</div>
                        </div>
                    </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                @if($payment['date_validation'])
                                    <div>
                                        ...
                                        <p class="text-slate-900">{{ \Carbon\Carbon::parse($payment['date_validation'])->format('d/m/Y à H:i') }}</p>
                                    </div>
                                @endif
                            </div>
                        @endif
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                ...
                <div class="p-6">
                    <div class="flex items-start justify-between mb-4">
                        <div>
                            <h3 class="text-lg font-bold text-slate-900 mb-1">{{ $payment['subscription']['fimeco']['nom'] }}</h3>
                            <p class="text-sm text-slate-600">Souscrit le {{ \Carbon\Carbon::parse($payment['subscription']['date_souscription'])->format('d/m/Y') }}</p>
                        </div>
                    ...
                    <div class="grid grid-cols-3 gap-4 mb-4">
                        <div class="text-center p-3 bg-blue-50 rounded-lg">
                            <div class="text-lg font-bold text-blue-600">{{ number_format($payment['subscription']['montant_souscrit'], 0, ',', ' ') }}</div>
                            <div class="text-sm text-blue-700">Souscrit</div>
                        </div>
                        <div class="text-center p-3 bg-green-50 rounded-lg">
                            <div class="text-lg font-bold text-green-600">{{ number_format($payment['subscription']['montant_paye'], 0, ',', ' ') }}</div>
                            <div class="text-sm text-green-700">Payé</div>
                        </div>
                        <div class="text-center p-3 bg-orange-50 rounded-lg">
                            <div class="text-lg font-bold text-orange-600">{{ number_format($payment['subscription']['reste_a_payer'], 0, ',', ' ') }}</div>
                    ...
                    <div class="w-full bg-gray-200 rounded-full h-3 mb-2">
                        <div class="bg-gradient-to-r from-blue-500 to-green-500 h-3 rounded-full" style="width: {{ $pourcentagePaye < 100 ? $pourcentagePaye : 100 }}%"></div>
                    </div>
    ...
    @if($payment['statut'] === 'en_attente')
        ...
        <a href="{{ route('private.paiements.edit', $payment['id']) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-200">
            <i class="fas fa-pen mr-2"></i> Modifier ce Paiement
        </a>
    @endif

    <!-- Bouton Voir Souscription - Orange pour visualisation -->
    <a href="{{ route('private.subscriptions.show', $payment['subscription']['id']) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-orange-600 to-amber-600 text-white text-sm font-medium rounded-xl hover:from-orange-700 hover:to-amber-700 transition-all duration-200">
        <i class="fas fa-file-contract mr-2"></i> Voir la souscription
    </a>

    <!-- Bouton Voir FIMECO - Violet -->
    <a href="{{ route('private.fimecos.show', $payment['subscription']['fimeco']['id']) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-purple-600 to-violet-600 text-white text-sm font-medium rounded-xl hover:from-purple-700 hover:to-violet-700 transition-all duration-200">
    ...
    @if($payment['subscription']['reste_a_payer'] > 0 && !in_array($payment['statut'], ['en_attente', 'annulee']))
        ...
        <a href="{{ route('private.paiements.create', $payment['subscription']['id']) }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-teal-600 to-cyan-600 text-white text-sm font-medium rounded-xl hover:from-teal-700 hover:to-cyan-700 transition-all duration-200">
            <i class="fas fa-credit-card mr-2"></i> Solder la souscription
        </a>
    @endif

    <!-- Bouton Retour - Gris pour navigation -->
    <a href="{{ route('private.paiements.index') }}" class="w-full inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-slate-600 to-gray-600 text-white text-sm font-medium rounded-xl hover:from-slate-700 hover:to-gray-700 transition-all duration-200">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-start space-x-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-slate-600">{{ \Carbon\Carbon::parse($payment['created_at'])->format('d/m/Y à H:i') }}</p>
                        </div>
                    </div>
                    ...
                    @if($payment['updated_at'] !== $payment['created_at'])
                        <div class="flex items-start space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-600">{{ \Carbon\Carbon::parse($payment['updated_at'])->format('d/m/Y à H:i') }}</p>
                            </div>
                        </div>
                    @endif
                    ...
                    @if($payment['date_validation'])
                        <div class="flex items-start space-x-3">
                            <div class="w-3 h-3 {{ $payment['statut'] === 'valide' ? 'bg-green-500' : 'bg-red-500' }} rounded-full mt-1.5"></div>
                            <div>
                                <h3 class="font-medium text-slate-900">
                                    {{ $payment['statut'] === 'valide' ? 'Paiement validé' : 'Paiement refusé' }}
                                </h3>
                                <p class="text-sm text-slate-600">{{ \Carbon\Carbon::parse($payment['date_validation'])->format('d/m/Y à H:i') }}</p>
                                @if($payment['validateur'])
