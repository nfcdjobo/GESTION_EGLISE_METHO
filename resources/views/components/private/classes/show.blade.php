        ...
        <div class="mb-8">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                ...
                <div class="flex flex-wrap gap-3">
                    @if (($classe->responsableSuperieur()?->id === Auth::user()->id) || auth()->user()->hasAnyRole(['super-admin', 'secretaire']) )
                        @can('classes.update')
                                ...
                                @foreach($classe->responsables_collection as $responsable)
                                    <div class="flex items-center justify-between p-4 bg-slate-50 rounded-xl">
                                        <div class="flex items-center space-x-4">
                                            ...
                                            <div>
                                                <h3 class="font-semibold text-slate-900">{{ $responsable->prenom }}
                                                    {{ $responsable->nom }}
                                                </h3>
                                ...
                                @foreach($classe->membres->take(6) as $membre)
                                    <div class="flex items-center space-x-3 p-3 bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <p class="font-medium text-slate-900 truncate">{{ $membre->prenom }} {{ $membre->nom }}
                                            </p>
                                            <p class="text-sm text-slate-500 truncate">{{ $membre->email }}</p>
                                        </div>
                                        @if (($classe->responsableSuperieur()?->id === Auth::user()->id) || auth()->user()->hasAnyRole(['super-admin', 'secretaire']) )
                                            @can('classes.manage-members')
                                                <button onclick="removeMember('{{ $membre->id }}')" class="text-red-600 hover:text-red-800 p-1">
                                                    <i class="fas fa-trash text-sm"></i>
                                                </button>
                                            @endcan
                                        @endif
                        ...
                        <div class="flex items-center space-x-2">
                            ${membre.is_responsable ? `
                                ...
                                <label class="flex items-center">
                                    @if(auth()->user()->hasAnyRole(['super-admin', 'secretaire']))
