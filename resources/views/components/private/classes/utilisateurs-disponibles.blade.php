                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-3 text-sm text-blue-700">
                                ...
                                @endif
                                <span>{{ $classe->nombre_inscrits + $classe->responsables()->count() }} membre(s)</span>
...
// Afficher les messages de statut
        @if(session('success'))
            showSuccessMessage('{{ addslashes(session('success')) }}');
        @endif

        @if(session('error'))
            showErrorMessage('{{ addslashes(session('error')) }}');
        @endif

        @if($errors->any())
            showErrorMessage('{{ addslashes($errors->first()) }}');
        @endif
