        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ number_format($stats['total_inscrits']) }}</p>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $stats['nombre_inscrits_moyen'] }}</p>
                            ...
                            @foreach($stats['tranches_age'] as $tranche)
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        <div class="w-4 h-4 {{ $colors[$colorIndex] }} rounded"></div>
                                        <span class="text-sm font-medium text-slate-700">{{ $tranche }}</span>
                                    </div>
                                    <div class="flex items-center space-x-3">
                                        <div class="w-32 bg-slate-200 rounded-full h-2">
                                            <div class="{{ $colors[$colorIndex] }} h-2 rounded-full transition-all duration-300"
                                                 style="width: {{ $percentage }}%"></div>
                                        </div>
                                        <span class="text-sm font-medium text-slate-600 w-12 text-right">{{ $count }}</span>
                                        <span class="text-xs text-slate-500 w-12 text-right">{{ $percentage }}%</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="flex items-center justify-between p-4 bg-green-50 rounded-xl">
                            ...
                            <div class="text-right">
                                <p class="text-2xl font-bold text-green-900">{{ $stats['classes_actives'] }}</p>
                                <p class="text-sm text-green-600">
                                    {{ $stats['total_classes'] > 0 ? round(($stats['classes_actives'] / $stats['total_classes']) * 100, 1) : 0 }}%
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-yellow-50 rounded-xl">
                            ...
                            <div class="text-right">
                                <p class="text-2xl font-bold text-yellow-900">{{ $classesEnAttente }}</p>
                                <p class="text-sm text-yellow-600">
                                    {{ $stats['total_classes'] > 0 ? round(($classesEnAttente / $stats['total_classes']) * 100, 1) : 0 }}%
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="p-4 bg-slate-50 rounded-xl">
                            ...
                            <div class="w-full bg-slate-200 rounded-full h-3">
                                <div class="bg-gradient-to-r from-green-500 to-emerald-500 h-3 rounded-full transition-all duration-500"
                                     style="width: {{ $stats['total_classes'] > 0 ? round(($stats['classes_actives'] / $stats['total_classes']) * 100, 1) : 0 }}%"></div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                @foreach($classesPopulaires as $classe)
                                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                        <div class="flex-1 min-w-0">
                                            <p class="font-medium text-slate-900 truncate">{{ $classe->nom }}</p>
                                            <p class="text-sm text-slate-500">{{ $classe->tranche_age ?? 'Tous âges' }}</p>
                                        </div>
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                            {{ $classe->nombre_inscrits }} membres
                                        </span>
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <div class="space-y-4">
                        ...
                        @endphp
                        @if($classesRecentes->count() > 0)
                            <div class="space-y-3">
                                @foreach($classesRecentes as $classe)
                                    <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                                        <div class="flex-1 min-w-0">
                                            <p class="font-medium text-slate-900 truncate">{{ $classe->nom }}</p>
                                            <p class="text-sm text-slate-500">{{ $classe->created_at->diffForHumans() }}</p>
                                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-3">
                            ...
                            @if($classesSansResponsables > 0)
                                <div class="p-3 bg-amber-50 rounded-lg border border-amber-200">
                                    <div class="flex items-start space-x-2">
                                        <i class="fas fa-exclamation-triangle text-amber-600 mt-0.5"></i>
                                        <div>
                                            <p class="text-sm font-medium text-amber-800">{{ $classesSansResponsables }} classe(s) sans responsable</p>
                            ...
                            @if($classesSansMembres > 0)
                                <div class="p-3 bg-blue-50 rounded-lg border border-blue-200">
                                    <div class="flex items-start space-x-2">
                                        <i class="fas fa-info-circle text-blue-600 mt-0.5"></i>
                                        <div>
                                            <p class="text-sm font-medium text-blue-800">{{ $classesSansMembres }} classe(s) sans membres</p>
                            ...
                            @if($classesSansResponsables == 0 && $classesSansMembres == 0)
                            ...
                            @if($stats['nombre_inscrits_moyen'] < 5)
