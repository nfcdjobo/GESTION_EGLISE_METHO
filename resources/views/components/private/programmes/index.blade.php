...
<div class="space-y-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-1">Administration des programmes d'église - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
    </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="{{ route('private.programmes.planning') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-calendar mr-2"></i> Planning
                    </a>
                    <a href="{{ route('private.programmes.statistiques') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.programmes.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
                <div class="lg:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-2">Recherche</label>
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Nom, code ou description..." class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div>
                    ...
                    <select name="audience" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        @foreach(\App\Models\Programme::AUDIENCES as $key => $label)
                            <option value="{{ $key }}" {{ request('audience') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="sort" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        <option value="nom_programme" {{ request('sort') == 'nom_programme' ? 'selected' : '' }}>Nom</option>
                        <option value="date_debut" {{ request('sort') == 'date_debut' ? 'selected' : '' }}>Date début</option>
                        <option value="type_programme" {{ request('sort') == 'type_programme' ? 'selected' : '' }}>Type</option>
                    </select>
                </div>
                <div class="lg:col-span-6 flex gap-2 pt-4">
                    ...
                    <a href="{{ route('private.programmes.index') }}" class="inline-flex items-center px-4 py-2 bg-slate-600 text-white text-sm font-medium rounded-xl hover:bg-slate-700 transition-colors">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $programmes->where('statut', 'actif')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $programmes->where('statut', 'planifie')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $programmes->whereIn('frequence', ['quotidien', 'hebdomadaire'])->count() }}</p>
        ...
        <div class="p-6">
            @if($programmes->count() > 0)
                        ...
                        <tbody class="divide-y divide-slate-200">
                            @foreach($programmes as $programme)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-4 py-4">
                                        <input type="checkbox" name="selected_programmes[]" value="{{ $programme->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 programme-checkbox">
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-3">
                                            <div>
                                                <div class="font-semibold text-slate-900">{{ $programme->nom_programme }}</div>
                                                <div class="text-sm text-slate-500">
                                                    <code class="px-2 py-1 text-xs bg-slate-100 text-slate-800 rounded">{{ $programme->code_programme }}</code>
                                                </div>
                                                @if($programme->description)
                                                    <div class="text-sm text-slate-600 mt-1">{{ Str::limit($programme->description, 50) }}</div>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="text-sm">
                                            @if($programme->heure_debut && $programme->heure_fin)
                                                <div class="font-medium text-slate-900">{{ $programme->horaires }}</div>
                                            @endif
                                            @if($programme->jours_semaine)
                                                <div class="text-slate-600">{{ $programme->jours_semaine_texte }}</div>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        @if($programme->responsablePrincipal)
                                            <div class="text-sm">
                                                <div class="font-medium text-slate-900">{{ $programme->responsablePrincipal->prenom }} {{ $programme->responsablePrincipal->nom }}</div>
                                            </div>
                                        @else
                                    ...
                                    <td class="px-4 py-4">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-{{ $programme->statut_badge }}-100 text-{{ $programme->statut_badge }}-800">
                                    ...
                                    <td class="px-4 py-4">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if($programme->statut === 'planifie')
                                                <button type="button" onclick="changerStatut('{{ $programme->id }}', 'activer')" class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-100 rounded-lg hover:bg-green-200 transition-colors" title="Activer">
                                                    <i class="fas fa-play text-sm"></i>
                                                </button>
                                            @endif

                                            @if($programme->statut === 'actif')
                                                <button type="button" onclick="changerStatut('{{ $programme->id }}', 'suspendre')" class="inline-flex items-center justify-center w-8 h-8 text-orange-600 bg-orange-100 rounded-lg hover:bg-orange-200 transition-colors" title="Suspendre">
                                                    <i class="fas fa-pause text-sm"></i>
                                                </button>
                                            @endif

                                            <button type="button" onclick="dupliquerProgramme('{{ $programme->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors" title="Dupliquer">
                                                <i class="fas fa-copy text-sm"></i>
                                            </button>

                                            @if($programme->peutEtreModifie())
                                                <button type="button" onclick="supprimerProgramme('{{ $programme->id }}')" class="inline-flex items-center justify-center w-8 h-8 text-red-600 bg-red-100 rounded-lg hover:bg-red-200 transition-colors" title="Supprimer">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6 pt-6 border-t border-slate-200">
                    <div class="text-sm text-slate-700">
                        Affichage de <span class="font-medium">{{ $programmes->firstItem() }}</span> à <span class="font-medium">{{ $programmes->lastItem() }}</span>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <a href="{{ route('private.programmes.create') }}" class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
...
function changerStatut(programmeId, action) {
    ...
    showModal(actionData.title, actionData.message, function() {
        fetch(`{{route('private.programmes.activer', ':programmeid')}}`.replace(':programmeid', programmeId).replace('activer', action), {
            method: 'POST',
            headers: {
...
function dupliquerProgramme(programmeId) {
    showModal('Dupliquer le programme', 'Voulez-vous créer une copie de ce programme ?', function() {
        ...
        fetch(`{{route('private.programmes.dupliquer', ':programmeid')}}`.replace(':programmeid', programmeId), {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                // location.reload();
                window.location.href = `{{route('private.programmes.edit', ':programmeid')}}`.replace(':programmeid', data.data.id);
            } else {
                alert(data.message);
            }
        })
...
function supprimerProgramme(programmeId) {
    showModal('Supprimer le programme', 'Cette action est irréversible. Voulez-vous vraiment supprimer ce programme ?', function() {
        fetch(`{{route('private.programmes.destroy', ':programmeid')}}`.replace(':programmeid', programmeId), {
            method: 'DELETE',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}',
                'Content-Type': 'application/json',
