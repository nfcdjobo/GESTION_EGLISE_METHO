...
@section('title', 'Hiérarchie des Rôles')
...
<div class="space-y-8">
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div class="bg-gradient-to-br from-yellow-500 to-orange-500 text-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-3xl font-bold">{{ $systemRoles }}</p>
                ...
                <div class="bg-gradient-to-br from-purple-500 to-pink-500 text-white rounded-2xl p-6 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-3xl font-bold">{{ $customRoles }}</p>
    ...
    <div id="hierarchy-view" class="space-y-6">
        @foreach($hierarchy as $levelName => $levelRoles)
            @php
                $levelRange = '';
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                                @if(strpos($levelName, '100') !== false) bg-red-100 text-red-800
                                @elseif(strpos($levelName, '80-99') !== false) bg-yellow-100 text-yellow-800
                                @elseif(strpos($levelName, '60-79') !== false) bg-blue-100 text-blue-800
                                @elseif(strpos($levelName, '40-59') !== false) bg-purple-100 text-purple-800
                                @elseif(strpos($levelName, '20-39') !== false) bg-green-100 text-green-800
                                @elseif(strpos($levelName, '10-19') !== false) bg-gray-100 text-gray-800
                                @else bg-slate-100 text-slate-800
                                @endif">
                                {{ $levelRange }}
                            </span>
                            <h2 class="text-xl font-bold text-slate-800">{{ $levelName }}</h2>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-200 text-slate-700">
                                {{ $levelRoles->count() }} rôle(s)
                            </span>
                        </div>
                        <button type="button" onclick="toggleLevelSection('{{ $levelRange }}')" id="toggle-{{ $levelRange }}" class="p-2 text-slate-600 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors">
                            <i class="fas fa-chevron-down"></i>
                        </button>
                    </div>
                </div>
                <div class="level-content p-6" id="level-{{ $levelRange }}">
                    @if($levelRoles->count() > 0)
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            @foreach($levelRoles as $role)
                                <div class="role-item group" data-system="{{ $role->is_system_role ? 'true' : 'false' }}" data-level="{{ $role->level }}">
                                    <div class="h-full bg-white border-2 rounded-xl p-6 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 group-hover:border-blue-300 {{ $role->is_system_role ? 'border-yellow-200 bg-yellow-50' : 'border-slate-200' }}">
                                        <!-- En-tête du rôle -->
                                        ...
                                        <div class="grid grid-cols-3 gap-4 mb-4">
                                            ...
                                            <div class="text-center">
                                                <div class="text-xl font-bold text-green-600">{{ $role->users()->wherePivot('actif', true)->count() }}</div>
                                        ...
                                        <div class="flex flex-wrap gap-2 mb-4">
                                            ...
                                            @if($role->users()->wherePivot('actif', true)->count() > 0)
                                        ...
                                        <div class="flex gap-2 mb-4">
                                            @can('roles.read')
                                                <a href="{{ route('private.roles.show', $role) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-cyan-100 text-cyan-700 text-sm font-medium rounded-lg hover:bg-cyan-200 transition-colors">
                                                    <i class="fas fa-eye mr-1"></i> Voir
                                                </a>
                                            @endcan

                                            @can('roles.update')
                                                <a href="{{ route('private.roles.edit', $role) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-yellow-100 text-yellow-700 text-sm font-medium rounded-lg hover:bg-yellow-200 transition-colors">
                                                    <i class="fas fa-edit mr-1"></i> Modifier
                                                </a>
                                            @endcan

                                            @can('roles.manage')
                                                <a href="{{ route('private.roles.permissions', $role) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-blue-100 text-blue-700 text-sm font-medium rounded-lg hover:bg-blue-200 transition-colors">
                                                ...
                                                <div class="flex items-center space-x-2">
                                                    @foreach($role->users()->take(3)->get() as $user)
                                                        <div class="relative group">
                                                            @if($user->avatar)
                                                                <img src="{{ $user->avatar }}" class="w-8 h-8 rounded-full object-cover ring-2 ring-blue-500" alt="{{ $user->nom_complet }}">
                                                            @else
                                                    ...
                                                    @if($role->users()->count() > 3)
                                                        <span class="text-xs text-slate-500">+{{ $role->users()->count() - 3 }} autres</span>
                                                    @endif
                                                </div>
                                            </div>
                                        @endif
                    ...
                    @else
                        <div class="text-center py-12">
                            ...
                            @can('roles.create')
                                <a href="{{ route('private.roles.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
