...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Comparaison de Rôles</h1>
    ...
    <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 p-6">
        <div class="flex flex-wrap justify-center gap-3">
            <a href="{{ route('private.roles.index') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-slate-600 to-gray-600 text-white text-sm font-medium rounded-xl hover:from-slate-700 hover:to-gray-700 transition-all duration-200 shadow-md hover:shadow-lg">
                <i class="fas fa-arrow-left mr-2"></i> Retour à la liste
            </a>
            <button type="button" onclick="exportComparison()" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-purple-600 text-white text-sm font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                <i class="fas fa-download mr-2"></i> Exporter
            </button>
            <button type="button" onclick="printComparison()" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-200 shadow-md hover:shadow-lg">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-{{ min($roles->count(), 4) }} gap-6">
                @foreach($roles as $role)
                    <div class="bg-gradient-to-br from-{{ $loop->index % 2 == 0 ? 'slate' : 'blue' }}-50 to-{{ $loop->index % 2 == 0 ? 'blue' : 'purple' }}-50 rounded-xl p-6 border border-{{ $loop->index % 2 == 0 ? 'slate' : 'blue' }}-200 hover:shadow-lg transition-all duration-300">
                        <div class="text-center">
                            <div class="mb-4">
                                ...
                                <code class="text-xs bg-slate-200 text-slate-700 px-2 py-1 rounded">{{ $role->slug }}</code>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-4 mt-4">
                                    <div class="text-center">
                                        <div class="text-xl font-bold text-blue-600">{{ $role->permissions->count() }}</div>
                            ...
                            <div class="mt-4">
                                <a href="{{ route('private.roles.show', $role) }}" class="inline-flex items-center px-3 py-1.5 bg-white/50 text-blue-600 text-sm font-medium rounded-lg hover:bg-white/80 transition-colors border border-blue-200">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ collect($comparison)->filter(function($item) { return collect($item['roles'])->contains(true); })->count() }}</p>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ collect($comparison)->filter(function($item) {
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ collect($comparison)->filter(function($item) { return collect($item['roles'])->filter()->count() === $roles->count(); })->count() }}</p>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    ...
                    <div class="relative">
                        <input type="text" id="search-permissions" placeholder="Nom ou slug de permission..." onkeyup="filterComparison()" class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        <i class="fas fa-search absolute left-3 top-1/2 transform -translate-y-1/2 text-slate-400"></i>
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Catégorie</label>
                    <select id="filter-category" onchange="filterComparison()" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div>
                    ...
                    <select id="filter-status" onchange="filterComparison()" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        ...
                        <tr class="bg-gradient-to-r from-slate-800 to-slate-700 text-white">
                            <th class="px-4 py-4 text-left permission-column" style="min-width: 300px;">
                                <div class="flex items-center">
                                    ...
                                    <button type="button" onclick="sortTable()" class="ml-2 p-1 text-slate-300 hover:text-white transition-colors rounded">
                                ...
                                <tr class="category-separator bg-gradient-to-r from-blue-600 to-purple-600 text-white">
                                    <td colspan="{{ $roles->count() + 2 }}" class="px-4 py-3">
                                        <div class="flex items-center justify-between">
                                            ...
                                            <button type="button" onclick="toggleCategory('{{ $currentCategory }}')" id="toggle-{{ $currentCategory }}" class="p-1 text-blue-200 hover:text-white transition-colors rounded">
                            ...
                            <tr class="permission-row category-{{ $currentCategory }} hover:bg-slate-50 transition-colors"
                                ...
                                <td class="px-4 py-4 permission-info">
                                    <div class="permission-details">
                                        <div class="font-semibold text-slate-900 mb-1">{{ $item['permission']->name }}</div>
                                        @if($item['permission']->description)
                                            <div class="text-sm text-slate-600 mb-2">{{ $item['permission']->description }}</div>
                                        @endif
                                        <code class="text-xs bg-slate-100 text-slate-700 px-2 py-1 rounded">{{ $item['permission']->slug }}</code>
                                    </div>
                                </td>
                                @foreach($roles as $role)
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-3">
                        <li class="flex items-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 mr-3">X/X</span>
                            <span class="text-slate-700">Tous les rôles ont cette permission</span>
                        </li>
                        <li class="flex items-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 mr-3">X/X</span>
                            <span class="text-slate-700">Quelques rôles ont cette permission</span>
                        </li>
                        <li class="flex items-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 mr-3">0/X</span>
...
function filterComparison() {
    ...
    document.querySelectorAll('.category-separator').forEach(separator => {
        ...
        const visibleRows = document.querySelectorAll(`.category-${categoryName.replace(/\s+/g, '')}:not([style*="display: none"])`);
...
function sortTable() {
    ...
    rows.forEach(row => {
        ...
        if (currentCategory !== category) {
            ...
            separator.className = 'category-separator bg-gradient-to-r from-blue-600 to-purple-600 text-white';
            separator.innerHTML = `
                <td colspan="{{ $roles->count() + 2 }}" class="px-4 py-3">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center">
...
function exportComparison() {
    ...
    comparison.forEach(item => {
        ...
        const roleValues = roles.map(roleName => {
            const role = {{ $roles->toJson() }}.find(r => r.name === roleName);
            return item.roles[role.id] ? 'Oui' : 'Non';
        });
        const total = Object.values(item.roles).filter(v => v).length;

        csv += `"${permission.name}","${permission.category}","${permission.description || ''}","${permission.slug}",${roleValues.join(',')},${total}\n`;
    });
