...
@section('title', 'Tableau de Bord - Cultes')
...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-2">
                ...
                @can('cultes.create')
                    <a href="{{ route('private.cultes.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white text-sm font-medium rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-md hover:shadow-lg">
                        <i class="fas fa-plus mr-2"></i> Nouveau Culte
                    </a>
                @endcan
            </div>
        </div>
        <p class="text-slate-500 mt-2">Vue d'ensemble de l'activité religieuse - {{ \Carbon\Carbon::now()->format('l d F Y à H:i') }}</p>
    </div>
            ...
            <div class="p-6 border-b border-blue-200">
                <h2 class="text-xl font-bold text-blue-800 flex items-center">
                    ...
                    <span class="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-600 text-white">
                        {{ $dashboard['aujourd_hui']['nombre'] }}
                    </span>
                </h2>
            </div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($dashboard['aujourd_hui']['cultes'] as $culte)
                        <div class="bg-white rounded-xl p-4 border border-blue-200 hover:shadow-md transition-all duration-200 cursor-pointer"
                             onclick="window.location.href='{{ route('private.cultes.show', $culte) }}'">
                            <div class="flex items-start justify-between mb-3">
                                <div class="flex-1">
                                    <h3 class="font-semibold text-slate-900 truncate">{{ $culte->titre }}</h3>
                                ...
                                @endphp
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ $statutColors[$culte->statut] ?? 'bg-gray-100 text-gray-800' }}">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $dashboard['cette_semaine']['cultes_a_venir'] }}</p>
                    <p class="text-sm text-slate-500">À venir cette semaine</p>
                    <p class="text-xs text-green-600">{{ $dashboard['cette_semaine']['cultes_termines'] }} terminés</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $dashboard['ce_mois']['total_cultes'] }}</p>
                    <p class="text-sm text-slate-500">Total ce mois</p>
                    <p class="text-xs text-blue-600">{{ number_format($dashboard['ce_mois']['total_participants']) }} participants</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $dashboard['ce_mois']['total_conversions'] }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                @if($dashboard['prochains_cultes']->count() > 0)
                    <div class="space-y-4">
                        @foreach($dashboard['prochains_cultes'] as $culte)
                            @php
                            ...
                            <div class="flex items-center space-x-4 p-4 rounded-xl hover:bg-slate-50 transition-colors cursor-pointer"
                                ...
                                <div class="flex-shrink-0">
                                    <div class="w-12 h-12 rounded-xl flex flex-col items-center justify-center text-xs font-bold
                                        {{ $isToday ? 'bg-orange-100 text-orange-800' : ($isTomorrow ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-800') }}">
                                        <div>{{ \Carbon\Carbon::parse($culte->date_culte)->format('j') }}</div>
                                        <div>{{ \Carbon\Carbon::parse($culte->date_culte)->format('M') }}</div>
                                    </div>
                                </div>
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center space-x-4 text-sm text-slate-600">
                                        ...
                                        <span>{{ $culte->type_culte_libelle }}</span>
                                        @if($culte->pasteurPrincipal)
                                            <span>{{ $culte->pasteurPrincipal->nom }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="flex-shrink-0 text-right">
                                    ...
                                    @else
                                        <span class="text-xs text-slate-500">Dans {{ $daysUntil }} jour{{ $daysUntil > 1 ? 's' : '' }}</span>
                                    @endif
                                </div>
                            </div>
                        @endforeach
                ...
                @else
                    <div class="text-center py-8">
                        ...
                        @can('cultes.create')
                            <a href="{{ route('private.cultes.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6 space-y-6">
                <!-- Note moyenne -->
                @if($dashboard['statistiques_rapides']['note_moyenne_mois'])
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-slate-700">Note moyenne ce mois</span>
                            <span class="text-lg font-bold text-amber-600">{{ round($dashboard['statistiques_rapides']['note_moyenne_mois'], 1) }}/10</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-gradient-to-r from-amber-500 to-orange-500 h-2 rounded-full transition-all duration-300"
                                 style="width: {{ ($dashboard['statistiques_rapides']['note_moyenne_mois'] / 10) * 100 }}%"></div>
                        </div>
                        <div class="flex mt-2">
                            @for($i = 1; $i <= 10; $i++)
                                <i class="fas fa-star text-sm {{ $i <= round($dashboard['statistiques_rapides']['note_moyenne_mois']) ? 'text-amber-400' : 'text-slate-300' }}"></i>
                            @endfor
                        </div>
                    </div>
                @endif
                ...
                @if($dashboard['statistiques_rapides']['taux_participation'])
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-medium text-slate-700">Taux de participation</span>
                            <span class="text-lg font-bold text-green-600">{{ round($dashboard['statistiques_rapides']['taux_participation'], 1) }}%</span>
                        </div>
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div class="bg-gradient-to-r from-green-500 to-emerald-500 h-2 rounded-full transition-all duration-300"
                                 style="width: {{ min($dashboard['statistiques_rapides']['taux_participation'], 100) }}%"></div>
                        </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="text-center p-4 bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl">
                        <div class="text-2xl font-bold text-blue-600">{{ $dashboard['ce_mois']['total_cultes'] }}</div>
                    ...
                    <div class="text-center p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl">
                        <div class="text-2xl font-bold text-purple-600">{{ round($dashboard['ce_mois']['total_participants'] / max($dashboard['ce_mois']['total_cultes'], 1), 0) }}</div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                @can('cultes.create')
                    <a href="{{ route('private.cultes.create') }}" class="flex items-center p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 hover:shadow-md transition-all duration-200 group">
    ...
    @if($dashboard['prochains_cultes']->where('statut', 'en_preparation')->count() > 0 || $dashboard['aujourd_hui']['cultes']->where('statut', 'planifie')->count() > 0)
        <div class="bg-gradient-to-r from-yellow-50 to-amber-50 rounded-2xl shadow-lg border border-yellow-200">
            ...
            <div class="p-6 space-y-4">
                @if($dashboard['aujourd_hui']['cultes']->where('statut', 'planifie')->count() > 0)
                    <div class="flex items-center p-4 bg-orange-100 rounded-xl border border-orange-200">
                        ...
                        <div>
                            ...
                            <div class="text-sm text-orange-700">
                                {{ $dashboard['aujourd_hui']['cultes']->where('statut', 'planifie')->count() }} culte(s) planifié(s) pour aujourd'hui
                            </div>
                        </div>
                    </div>
                @endif

                @if($dashboard['prochains_cultes']->where('statut', 'en_preparation')->count() > 0)
                    <div class="flex items-center p-4 bg-blue-100 rounded-xl border border-blue-200">
                        ...
                        <div>
                            ...
                            <div class="text-sm text-blue-700">
                                {{ $dashboard['prochains_cultes']->where('statut', 'en_preparation')->count() }} culte(s) nécessitent votre attention
                            </div>
                        </div>
                    </div>
                @endif
...
function checkNotifications() {
    ...
    const now = new Date();
    const cultes = @json($dashboard['aujourd_hui']['cultes']);
    ...
    if (e.ctrlKey || e.metaKey) {
        switch(e.key) {
            ...
            case 'n':
                e.preventDefault();
                @can('cultes.create')
                    window.location.href = '{{ route("private.cultes.create") }}';
                @endcan
                break;
            case 'p':
                e.preventDefault();
