...
@section('title', 'Participants du Culte')
...
@section('content')
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 p-6">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">{{ \Carbon\Carbon::parse($culte->date_culte)->format('d/m/Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Heure:</span>
                        <span class="font-medium">{{ \Carbon\Carbon::parse($culte->heure_debut)->format('H:i') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Type:</span>
                        <span class="font-medium">{{ $culte->type_culte_libelle }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-500">Statut:</span>
                        <span class="font-medium">{{ $culte->statut_libelle }}</span>
                    </div>
                </div>
            </div>
            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-6 gap-4">
                ...
                <div>
                    ...
                    <select name="statut_presence"
                        ...
                        @foreach (App\Models\ParticipantCulte::STATUT_PRESENCE as $key => $label)
                            <option value="{{ $key }}"
                                {{ request('statut_presence') === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="type_participation"
                        class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm">
                        <option value="">Tous</option>
                        @foreach (App\Models\ParticipantCulte::TYPE_PARTICIPATION as $key => $label)
                            <option value="{{ $key }}"
                                {{ request('type_participation') === $key ? 'selected' : '' }}>{{ $label }}
                            </option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select name="role_culte"
                        class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors text-sm">
                        <option value="">Tous</option>
                        @foreach (App\Models\ParticipantCulte::ROLE_CULTE as $key => $label)
                            <option value="{{ $key }}" {{ request('role_culte') === $key ? 'selected' : '' }}>
                                {{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex items-end space-x-2">
                    ...
                    <a href="{{ route('private.cultes.participants', $culte) }}"
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
            <div class="p-6 border-b border-slate-200">
                <h2 class="text-xl font-bold text-slate-800 flex items-center">
                    ...
                    Liste des Participants ({{ $participants->total() }})
                </h2>
            </div>
                            ...
                            @foreach ($participants as $participation)
                                <tr class="hover:bg-slate-50 transition-colors">
                                        ...
                                        <div class="flex items-center">
                                            <div class="flex-shrink-0 h-10 w-10">
                                                <div
                                                    ...
                                                    {{ strtoupper(substr($participation->participant->prenom, 0, 1) . substr($participation->participant->nom, 0, 1)) }}
                                                </div>
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-slate-900">
                                                    {{ $participation->participant->nom }}
                                                    {{ $participation->participant->prenom }}
                                                </div>
                                                <div class="text-sm text-slate-500">
                                                    {{ $participation->participant->email }}</div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                        {{ $participation->type_participation_libelle }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                        {{ $participation->role_culte_libelle }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                                        ...
                                        @if ($participation->heure_depart)
                                            <div>Départ:
                                                {{ \Carbon\Carbon::parse($participation->heure_depart)->format('H:i') }}
                                            </div>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <button type="button"
                                                onclick="editParticipant('{{ $participation->participant_id }}', '{{ $participation->culte_id }}')"
                                                class="text-blue-600 hover:text-blue-900">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button type="button"
                                                onclick="removeParticipant('{{ $participation->participant_id }}', '{{ $participation->culte_id }}')"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="type_participation" required id="typeParticipation"
                                ...
                                @foreach (App\Models\ParticipantCulte::TYPE_PARTICIPATION as $key => $label)
                                    <option value="{{ $key }}" {{ $key === 'physique' ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="role_culte" required id="roleCulte"
                                class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                                @foreach (App\Models\ParticipantCulte::ROLE_CULTE as $key => $label)
                                    <option value="{{ $key }}" {{ $key === 'participant' ? 'selected' : '' }}>
                                        {{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
    ...
    {{-- @push('scripts')
        <script>
            ...
            async function searchParticipants(query) {
                try {
                    const response = await fetch(`{{route('private.participantscultes.search', ':culteid')}}`.replace(':culteid', '{{$culte->id}}')+`?q=${encodeURIComponent(query)}`, {
                        method: 'GET',
                        headers: {
    ...
    // Si tout est valide, on peut ajouter un indicateur de chargement
    ...
    <script>
        ...
        async function submitParticipantForm() {
            ...
            try {
                ...
                // Faire l'appel API
                const response = await fetch('{{ route("private.participantscultes.store-with-user-creation") }}', {
                    method: 'POST',
                    headers: {
