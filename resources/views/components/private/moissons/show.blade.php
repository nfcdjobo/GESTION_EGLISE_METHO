...
@section('title', 'Détails Moisson - ' . $moisson->theme)
    ...
    <div class="space-y-8">
        ...
        <div class="mb-8">
            <div class="flex items-center gap-2 text-sm text-slate-600 mb-4">
                ...
                <span class="text-slate-800 font-medium">{{ Str::limit($moisson->theme, 30) }}</span>
            </div>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-500 mt-1 flex items-center gap-4">
                        <span><i class="fas fa-calendar mr-1"></i>{{ $moisson->date->format('d F Y') }}</span>
                        <span><i class="fas fa-church mr-1"></i>{{ $moisson->culte->titre ?? 'N/A' }}</span>
                        <span><i class="fas fa-user mr-1"></i>{{ $moisson->createur->nom_complet ?? 'N/A' }}</span>
                    </p>
                </div>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($moisson->cible, 0, ',', ' ') }}</p>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($moisson->montant_solde, 0, ',', ' ') }}</p>
                        <p class="text-xs text-slate-500">FCFA</p>
                    </div>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-slate-600">
                            @if($moisson->reste > 0) Reste à collecter @else Supplément @endif
                        </p>
                        <p class="text-2xl font-bold {{ $moisson->reste > 0 ? 'text-orange-600' : 'text-green-600' }}">
                            {{ number_format($moisson->reste > 0 ? $moisson->reste : $moisson->montant_supplementaire, 0, ',', ' ') }}
                        </p>
                        <p class="text-xs text-slate-500">FCFA</p>
                    </div>
                    <div class="w-12 h-12 bg-gradient-to-br {{ $moisson->reste > 0 ? 'from-orange-500 to-red-500' : 'from-purple-500 to-pink-500' }} rounded-xl flex items-center justify-center">
                        <i class="fas {{ $moisson->reste > 0 ? 'fa-exclamation-triangle' : 'fa-plus-circle' }} text-white text-xl"></i>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($moisson->pourcentage_realise, 1) }}%</p>
                        <p class="text-xs {{ $moisson->pourcentage_realise >= 100 ? 'text-green-600' : ($moisson->pourcentage_realise >= 70 ? 'text-blue-600' : 'text-orange-600') }}">
        ...
        <!-- Passages bibliques -->
        @if($moisson->passages_bibliques && count($moisson->passages_bibliques) > 0)
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="p-6">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach($moisson->passages_bibliques_formatted as $passage)
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
            <div class="border-b border-slate-200">
                <nav class="flex space-x-8 px-6" aria-label="Tabs">
                    <button onclick="switchTab('passages')" id="tab-passages"
                        ...
                        Passages ({{ $moisson->passageMoissons->count() }})
                    </button>
                    <button onclick="switchTab('ventes')" id="tab-ventes"
                        ...
                        Ventes ({{ $moisson->venteMoissons->count() }})
                    </button>
                    <button onclick="switchTab('engagements')" id="tab-engagements"
                        class="tab-button border-b-2 border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300 py-4 px-1 text-sm font-medium">
                        <i class="fas fa-handshake mr-2"></i>
                        Engagements ({{ $moisson->engagementMoissons->count() }})
                    </button>
                </nav>
            </div>
            ...
            <div class="p-6">
                ...
                <div id="content-passages" class="tab-content">
                    ...
                    @if($moisson->passageMoissons->count() > 0)
                        <div class="grid gap-4">
                            @foreach($moisson->passageMoissons as $passage)
                                <div class="border border-slate-200 rounded-xl p-4 hover:border-blue-300 transition-colors">
                                    <div class="flex items-center justify-between mb-3">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            @endif
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $passage->status ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                {{ $passage->status ? 'Validé' : 'En attente' }}
                                            </span>
                                        </div>
                                        <div class="flex items-center space-x-2">
                                            @can('passages.read')
                                                <button onclick="detailPassage('{{ $passage->id }}')"
                                            ...
                                            @endcan
                                            @can('passages.update')
                                                <button onclick="modifierPassage('{{ $passage->id }}')"
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div>
                                            ...
                                            <p class="font-medium">{{ number_format($passage->cible, 0, ',', ' ') }} FCFA</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Collecté</p>
                                            <p class="font-medium text-green-600">{{ number_format($passage->montant_solde, 0, ',', ' ') }} FCFA</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Reste/Supplément</p>
                                            <p class="font-medium {{ $passage->reste > 0 ? 'text-orange-600' : 'text-green-600' }}">
                                                {{ number_format($passage->reste > 0 ? $passage->reste : $passage->montant_supplementaire, 0, ',', ' ') }} FCFA
                                            </p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Performance</p>
                                            <p class="font-medium">{{ number_format($passage->pourcentage_realise, 1) }}%</p>
                                            <div class="w-full bg-slate-200 rounded-full h-1.5 mt-1">
                                                <div class="bg-blue-600 h-1.5 rounded-full" style="width: {{ min($passage->pourcentage_realise, 100) }}%"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div id="content-ventes" class="tab-content hidden">
                    ...
                    @if($moisson->venteMoissons->count() > 0)
                        <div class="grid gap-4">
                            @foreach($moisson->venteMoissons as $vente)
                                <div class="border border-slate-200 rounded-xl p-4 hover:border-green-300 transition-colors">
                                    <div class="flex items-center justify-between mb-3">
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $vente->status ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                {{ $vente->status ? 'Validé' : 'En attente' }}
                                            </span>
                                        </div>

                                        <div class="flex items-center space-x-2">
                                            @can('ventes.read')
                                                <button onclick="detailVente('{{ $vente->id }}')"
                                                class="inline-flex items-center justify-center w-8 h-8 text-cyan-600 bg-cyan-100 rounded-lg hover:bg-cyan-200 transition-colors"
                                                     title="Voir détail">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                            @endcan
                                            @can('ventes.update')
                                                <button onclick="modifierVente('{{ $vente->id }}')"
                                                    class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors" title="Modifier">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                            @endcan
                                    ...
                                    @if($vente->description)
                                        <p class="text-sm text-slate-600 mb-3">{{ $vente->description }}</p>
                                    @endif

                                    <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
                                        <div>
                                            <p class="text-xs text-slate-500">Cible</p>
                                            <p class="font-medium">{{ number_format($vente->cible, 0, ',', ' ') }} FCFA</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Collecté</p>
                                            <p class="font-medium text-green-600">{{ number_format($vente->montant_solde, 0, ',', ' ') }} FCFA</p>
                                        </div>

                                        <div>
                                            <p class="text-xs text-slate-500">Performance</p>
                                            <p class="font-medium">{{ number_format($vente->pourcentage_realise, 1) }}%</p>
                                            <div class="w-full bg-slate-200 rounded-full h-1.5 mt-1">
                                                <div class="bg-green-600 h-1.5 rounded-full" style="width: {{ min($vente->pourcentage_realise, 100) }}%"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div id="content-engagements" class="tab-content hidden">
                    ...
                    @if($moisson->engagementMoissons->count() > 0)
                        <div class="grid gap-4">
                            @foreach($moisson->engagementMoissons as $engagement)
                                <div class="border border-slate-200 rounded-xl p-4 hover:border-purple-300 transition-colors">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            @if($engagement->est_en_retard)
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                                    ...
                                                    En retard ({{ $engagement->jours_retard }}j)
                                                </span>
                                            @endif
                                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $engagement->status ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                                {{ $engagement->status ? 'Validé' : 'En attente' }}
                                            </span>
                                        </div>

                                        <div class="flex items-center space-x-2">
                                            @can('engagements.read')
                                                <button onclick="detailEngagement('{{ $engagement->id }}')"
                                                class="inline-flex items-center justify-center w-8 h-8 text-cyan-600 bg-cyan-100 rounded-lg hover:bg-cyan-200 transition-colors"
                                                     title="Voir détail">
                                                    <i class="fas fa-eye"></i>
                                                </button>
                                            ...
                                            @can('engagements.update')
                                                <button onclick="modifierEngagement('{{ $engagement->id }}')"
                                                    class="inline-flex items-center justify-center w-8 h-8 text-yellow-600 bg-yellow-100 rounded-lg hover:bg-yellow-200 transition-colors" title="Modifier">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                            @endcan
                                            @can('engagements.delete')
                                    ...
                                    <div class="mb-3">
                                        <p class="font-medium text-slate-800">{{ $engagement->nom_donateur }}</p>
                                        @if($engagement->description)
                                            <p class="text-sm text-slate-600">{{ $engagement->description }}</p>
                                        @endif
                                        @if($engagement->date_echeance)
                                            <p class="text-xs text-slate-500">
                                                <i class="fas fa-calendar mr-1"></i>
                                                Échéance: {{ $engagement->date_echeance->format('d/m/Y') }}
                                            </p>
                                        @endif
                                    </div>
                                    ...
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        <div>
                                            ...
                                            <p class="font-medium">{{ number_format($engagement->cible, 0, ',', ' ') }} FCFA</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Versé</p>
                                            <p class="font-medium text-green-600">{{ number_format($engagement->montant_solde, 0, ',', ' ') }} FCFA</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Reste</p>
                                            <p class="font-medium {{ $engagement->reste > 0 ? 'text-orange-600' : 'text-green-600' }}">
                                                {{ number_format($engagement->reste, 0, ',', ' ') }} FCFA
                                            </p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-slate-500">Progression</p>
                                            <p class="font-medium">{{ number_format($engagement->pourcentage_realise, 1) }}%</p>
                                            <div class="w-full bg-slate-200 rounded-full h-1.5 mt-1">
                                                <div class="bg-purple-600 h-1.5 rounded-full" style="width: {{ min($engagement->pourcentage_realise, 100) }}%"></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        <script>
            ...
            function recalculerTotaux() {
                if (confirm('Recalculer les totaux de cette moisson ?')) {
                    fetch("{{ route('private.moissons.recalculer-totaux', $moisson) }}", {
                        method: 'POST',
                        headers: {
    ...
    document.getElementById('exportModal')?.addEventListener('click', function(event) {
        if (event.target === this) {
            ...
            function ajouterPassage() {
                ...
                window.location.href = `{{ route('private.moissons.passages.create', $moisson->id) }}`;
            }

            function modifierPassage(passageId) {
                window.location.href = `{{ route('private.moissons.passages.edit', [$moisson->id,':id']) }}`.replace(':id', passageId);
            }

            function detailPassage(passageId){
                window.location.href = `{{ route('private.moissons.passages.show', [$moisson,':id']) }}`.replace(':id', passageId);
            }

            function supprimerPassage(passageId) {
                if (confirm('Êtes-vous sûr de vouloir supprimer ce passage ?')) {
                    fetch(`{{ route('private.moissons.passages.destroy', [$moisson->id,':id']) }}`.replace(':id', passageId), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': "{{ csrf_token() }}",
                            'Accept': 'application/json',
            ...
            function ajouterVente() {
                window.location.href = `{{ route('private.moissons.ventes.create', $moisson->id) }}`;
            }

            function detailVente(venteId){
                window.location.href = `{{ route('private.moissons.ventes.show', [$moisson,':id']) }}`.replace(':id', venteId);
            }

            function modifierVente(venteId) {
                window.location.href = `{{ route('private.moissons.ventes.edit', [$moisson->id, ':id']) }}`.replace(':id', venteId);
            }

            function supprimerVente(venteId) {
                if (confirm('Êtes-vous sûr de vouloir supprimer cette vente ?')) {
                    fetch(`{{ route('private.moissons.ventes.destroy', [$moisson->id, ':id']) }}`.replace(':id', venteId), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': "{{ csrf_token() }}",
                            'Accept': 'application/json',
            ...
            function ajouterEngagement() {
                window.location.href = `{{ route('private.moissons.engagements.create', $moisson->id) }}`;
            }

            function modifierEngagement(engagementId) {
                window.location.href = `{{ route('private.moissons.engagements.edit', [$moisson->id, ':id']) }}`.replace(':id', engagementId);
            }

            function detailEngagement(engagementId){
                window.location.href = `{{ route('private.moissons.engagements.show', [$moisson,':id']) }}`.replace(':id', engagementId);
            }

            function supprimerEngagement(engagementId) {
                if (confirm('Êtes-vous sûr de vouloir supprimer cet engagement ?')) {
                    fetch(`{{ route('private.moissons.engagements.destroy', [$moisson->id, ':id']) }}`.replace(':id', engagementId), {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': "{{ csrf_token() }}",
                            'Accept': 'application/json',
