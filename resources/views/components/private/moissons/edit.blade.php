...
@section('title', 'Modifier - ' . $moisson->theme)
...
@section('content')
    <div class="space-y-8">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-500 mt-1 flex items-center gap-4">
                        ...
                        <span><i class="fas fa-coins mr-1"></i>{{ number_format($moisson->cible, 0, ',', ' ') }} FCFA</span>
                        <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full {{ $moisson->status ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
        ...
        <form id="moisson-edit-form" action="{{ route('private.moissons.update', $moisson) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                ...
                <div class="p-6 border-b border-slate-200">
                    <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-2 text-sm text-slate-600">
                            ...
                            Créé par {{ $moisson->createur->nom_complet ?? 'N/A' }}
                            <span class="mx-2">•</span>
                            <i class="fas fa-clock text-xs"></i>
                            {{ $moisson->created_at->format('d/m/Y à H:i') }}
                        </div>
                    </div>
                </div>
                <div class="p-6">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="date" id="date" required
                                value="{{ old('date', $moisson->date->format('Y-m-d')) }}"
                                class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            @error('date')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="culte_id" id="culte_id" required
                                ...
                                @foreach($cultes as $culte)
                                    <option value="{{ $culte->id }}" {{ old('culte_id', $moisson->culte_id) == $culte->id ? 'selected' : '' }}>
                                        {{ $culte->titre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <div class="flex items-center space-x-4">
                                <label class="flex items-center">
                                    <input type="radio" name="status" value="1" {{ old('status', $moisson->status) == '1' ? 'checked' : '' }}
                                        class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300">
                                    <span class="ml-2 text-sm text-slate-700">Active</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="radio" name="status" value="0" {{ old('status', $moisson->status) == '0' ? 'checked' : '' }}
                                        class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-slate-300">
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                            <div class="bg-slate-50 rounded-lg p-4">
                                ...
                                <p class="font-semibold text-green-600">{{ number_format($moisson->montant_solde, 0, ',', ' ') }} FCFA</p>
                            </div>
                            <div class="bg-slate-50 rounded-lg p-4">
                                <p class="text-xs text-slate-500 mb-1">
                                    @if($moisson->reste > 0) Reste à collecter @else Supplément @endif
                                </p>
                                <p class="font-semibold {{ $moisson->reste > 0 ? 'text-orange-600' : 'text-green-600' }}">
                                    {{ number_format($moisson->reste > 0 ? $moisson->reste : $moisson->montant_supplementaire, 0, ',', ' ') }} FCFA
                                </p>
                            </div>
                            <div class="bg-slate-50 rounded-lg p-4">
                                <p class="text-xs text-slate-500 mb-1">Performance</p>
                                <p class="font-semibold text-blue-600">{{ number_format($moisson->pourcentage_realise, 1) }}%</p>
                            </div>
                            <div class="bg-slate-50 rounded-lg p-4">
                                <p class="text-xs text-slate-500 mb-1">Statut progression</p>
                                <p class="font-semibold text-slate-700">{{ $moisson->statut_progression }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6">
                    <div id="passages-container" class="space-y-4">
                        @if($moisson->passages_bibliques && count($moisson->passages_bibliques) > 0)
                            @foreach($moisson->passages_bibliques as $index => $passage)
                                <div class="passage-item flex items-center gap-4">
                                    <input type="text" name="passages_bibliques[]" value="{{ $passage }}"
                                    ...
                                    <button type="button" onclick="removePassage(this)"
                                        class="w-10 h-10 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors {{ $index === 0 && count($moisson->passages_bibliques) === 1 ? 'opacity-50 cursor-not-allowed' : '' }}">
...
<div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
    ...
    <div class="p-6">
                ...
                @foreach($moisson->passages_bibliques as $index => $passage)
                    <div class="passage-item flex items-center gap-4">
                        <input type="text" name="passages_bibliques[]"
                               value="{{ is_array($passage) ? ($passage['reference'] ?? $passage['livre'] . ' ' . $passage['chapitre'] . ':' . $passage['verset_debut'] . (isset($passage['verset_fin']) ? '-' . $passage['verset_fin'] : '')) : $passage }}"
                               placeholder="Ex: Matthieu 9:37-38"
                               class="flex-1 px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        <button type="button" onclick="removePassage(this)"
                                class="w-10 h-10 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-colors {{ $index === 0 && count($moisson->passages_bibliques) === 1 ? 'opacity-50 cursor-not-allowed' : '' }}">
        ...
        <div class="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-3xl font-bold text-blue-600 mb-1">{{ $moisson->passageMoissons->count() }}</p>
                            <p class="text-sm text-slate-500">Total: {{ number_format($moisson->passageMoissons->sum('montant_solde'), 0, ',', ' ') }} FCFA</p>
                        </div>
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-3xl font-bold text-green-600 mb-1">{{ $moisson->venteMoissons->count() }}</p>
                            <p class="text-sm text-slate-500">Total: {{ number_format($moisson->venteMoissons->sum('montant_solde'), 0, ',', ' ') }} FCFA</p>
                        </div>
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-3xl font-bold text-purple-600 mb-1">{{ $moisson->engagementMoissons->count() }}</p>
                            <p class="text-sm text-slate-500">Total: {{ number_format($moisson->engagementMoissons->sum('montant_solde'), 0, ',', ' ') }} FCFA</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            @if($moisson->editeurs && count($moisson->editeurs) > 0)
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    <div class="p-6 border-b border-slate-200">
                        <h3 class="text-lg font-bold text-slate-800 flex items-center">
                    ...
                    <div class="p-6">
                        <div class="space-y-3 max-h-64 overflow-y-auto">
                            @foreach(array_reverse($moisson->editeurs) as $edit)
                                <div class="flex items-center space-x-3 text-sm">
                                    <div class="w-2 h-2 bg-blue-500 rounded-full flex-shrink-0"></div>
                                    <div class="flex-1">
                                        <span class="font-medium">{{ $edit['action'] ?? 'modification' }}</span>
                                        @if(isset($edit['user_id']))
                                            par <span class="text-blue-600">{{ \App\Models\User::find($edit['user_id'])->nom_complet ?? 'Utilisateur inconnu' }}</span>
                                        @endif
                                        <span class="text-slate-500">
                                            le {{ \Carbon\Carbon::parse($edit['date'])->format('d/m/Y à H:i') }}
                                        </span>
                                    </div>
                                </div>
                            @endforeach
            ...
            <div class="flex justify-between items-center pt-6">
                <div class="flex gap-4">
                    ...
                    @endcan
                    @can('moissons.close')
                        @if($moisson->status && !$moisson->est_cloturee)
        ...
        <script>
            ...
            function cloturerMoisson() {
                ...
                if (motif !== null) { // L'utilisateur n'a pas annulé
                    fetch("{{ route('private.moissons.cloturer', $moisson) }}", {
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': "{{ csrf_token() }}",
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const originalCible = {{ $moisson->cible }};
                ...
                cibleInput.addEventListener('blur', function() {
                    ...
                    if (newCible !== originalCible && {{ $moisson->montant_solde }} > 0) {
