        ...
        {{-- <script>
            ...
            function initCharts() {
                ...
                evolutionChart = new Chart(evolutionCtx, {
                    type: 'line',
                    data: {
                        labels: @json($donneesGraphiques['evolution']['labels'] ?? []),
                        datasets: [{
                            label: 'Montant collecté',
                            data: @json($donneesGraphiques['evolution']['data'] ?? []),
                ...
                repartitionChart = new Chart(repartitionCtx, {
                    type: 'doughnut',
                    data: {
                        labels: @json($donneesGraphiques['repartition']['labels'] ?? []),
                        datasets: [{
                            data: @json($donneesGraphiques['repartition']['data'] ?? []),
                            backgroundColor: [
                                chartColors.primary,
            ...
            function mettreAJourMetriques(stats) {
                document.getElementById('total-moissons').textContent = stats.financier.objectif_total || 0;
                document.getElementById('total-collecte').textContent = new Intl.NumberFormat('fr-FR').format(stats.financier.collecte_totale || 0);
                document.getElementById('objectifs-atteints').textContent = stats.financier.objectif_total || 0;
                document.getElementById('performance-moyenne').textContent = (stats.financier.pourcentage_realisation || 0).toFixed(1) + '%';
            }
