...
@section('content')
    <div class="space-y-8">
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    @can('rapports-reunions.create')
                        @if($reunion->statut === 'terminee')
                            <a href="{{route('private.rapports-reunions.create', ['reunion' => $reunion->id])}}"
        ...
        {{-- Évaluer la réunion --}}
        @can('reunions.evaluate')
            @if($reunion->statut === 'Terminee' && !$reunion->note_globale)
                <button onclick="openEvaluationModal('{{$reunion->id}}')"
        ...
        {{-- Actions pendant ou après la réunion --}}
        @if(in_array($reunion->statut, ['en_cours', 'terminee']))
            {{-- Ajouter témoignage --}}
            @can('reunions.add-testimonies')
                <button onclick="openTemoignageModal('{{$reunion->id}}')"
            ...
            {{-- Demande de prière --}}
            @can('reunions.add-prayer-requests')
                <button onclick="openPriereModal('{{$reunion->id}}')"
        ...
        {{-- Envoyer rappel --}}
        @can('reunions.send-reminders')
            <button onclick="openRappelModal('{{$reunion->id}}', '{{$reunion}}')"
        ...
        {{-- Créer récurrence --}}
        @can('reunions.create-recurrence')
            @if(!$reunion->est_recurrente)
                <button onclick="openRecurrenceModal('{{$reunion->id}}', '{{$reunion}}')"
    ...
    @can('reunions.register-participant')
