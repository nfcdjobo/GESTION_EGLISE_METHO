...
@section('title', 'Statistiques des Réunions')
...
@section('content')
    <div class="space-y-8">
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6">
                <form method="GET" action="{{ route('private.reunions.statistiques') }}"
                    class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                        <label class="block text-sm font-medium text-slate-700 mb-2">Date de début</label>
                        <input type="date" name="date_debut"
                            value="{{ request('date_debut', now()->subMonth()->format('Y-m-d')) }}"
                    ...
                    <div>
                        ...
                        <select name="type_reunion_id"
                            class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                            <option value="">Tous les types</option>
                            @if (isset($typesReunions))
                                @foreach ($typesReunions as $type)
                                    <option value="{{ $type->id }}"
                                        {{ request('type_reunion_id') == $type->id ? 'selected' : '' }}>
                                        {{ $type->nom }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div class="flex gap-2">
                        <button type="submit"
                            class="flex-1 inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-xl hover:bg-blue-700 transition-colors">
                            <i class="fas fa-chart-bar mr-2"></i> Analyser
                        </button>
                        <div class="relative">
                            <button type="button" id="presetButton"
                                class="inline-flex items-center justify-center px-3 py-2 bg-slate-600 text-white text-sm font-medium rounded-xl hover:bg-slate-700 transition-colors">
                                <i class="fas fa-clock mr-2"></i>
                            </button>
                            <div id="presetMenu"
                                class="hidden absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-slate-200 z-10">
                                <div class="p-2">
                                    <button type="button" onclick="setPreset('cette_semaine')"
                                        class="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg">Cette
                                        semaine</button>
                                    <button type="button" onclick="setPreset('ce_mois')"
                                        class="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg">Ce
                                        mois</button>
                                    <button type="button" onclick="setPreset('trimestre')"
                                        class="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg">Ce
                                        trimestre</button>
                                    <button type="button" onclick="setPreset('semestre')"
                                        class="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg">Ce
                                        semestre</button>
                                    <button type="button" onclick="setPreset('annee')"
                                        class="w-full text-left px-3 py-2 text-sm text-slate-700 hover:bg-slate-100 rounded-lg">Cette
                                        année</button>
                                </div>
                            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $statistiques['globales']['total_reunions'] ?? 0 }}
                        </p>
                ...
                <div class="mt-4 pt-4 border-t border-slate-200">
                    <div class="flex justify-between text-sm">
                        ...
                        <span
                            class="font-medium text-green-600">{{ $statistiques['globales']['reunions_terminees'] ?? 0 }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600">Annulées</span>
                        <span
                            class="font-medium text-red-600">{{ $statistiques['globales']['reunions_annulees'] ?? 0 }}</span>
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ number_format($statistiques['globales']['total_participants'] ?? 0) }}</p>
                ...
                <div class="mt-4 pt-4 border-t border-slate-200">
                    <div class="flex justify-between text-sm">
                        ...
                        <span
                            class="font-medium text-slate-900">{{ number_format($statistiques['globales']['moyenne_participants'] ?? 0, 1) }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-600">Nouveaux</span>
                        <span
                            class="font-medium text-blue-600">{{ $statistiques['globales']['total_nouveaux'] ?? 0 }}</span>
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ $statistiques['globales']['total_decisions'] ?? 0 }}</p>
                ...
                <div class="mt-4 pt-4 border-t border-slate-200">
                    <div class="flex justify-between text-sm">
                        ...
                        @php
                            ...
                            $tauxConversion =
                                $totalParticipants > 0 ? round(($totalDecisions / $totalParticipants) * 100, 1) : 0;
                        @endphp
                        <span class="font-medium text-purple-600">{{ $tauxConversion }}%</span>
                    </div>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ number_format($statistiques['globales']['satisfaction_moyenne'] ?? 0, 1) }}</p>
                ...
                <div class="mt-4 pt-4 border-t border-slate-200">
                    <div class="flex justify-between text-sm">
                        ...
                        <span
                            class="font-medium text-amber-600">{{ number_format($statistiques['globales']['note_moyenne'] ?? 0, 1) }}/10</span>
                    </div>
                </div>
            </div>
        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-3 h-3 rounded-full mr-3"
                                                style="background-color: {{ $type->couleur ?? '#6B7280' }}"></div>
                                            <div>
                                                <div class="text-sm font-medium text-slate-900">{{ $type->nom_type }}
                                                </div>
                                                <div class="text-sm text-slate-500">
                                                    {{ $type->categorie ?? 'Non définie' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="text-sm font-medium text-slate-900">{{ $type->nombre_reunions }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="text-sm text-slate-900">{{ number_format($type->total_participants) }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="text-sm font-medium text-green-600">{{ number_format($type->moyenne_participants, 1) }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span class="text-sm text-slate-600">{{ $type->max_participants }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="text-sm font-medium text-purple-600">{{ $type->total_decisions ?? 0 }}</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <span
                                            class="text-sm text-slate-900">{{ number_format($type->satisfaction_moyenne ?? 0, 1) }}%</span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center">
                                            <span
                                                class="text-sm font-medium text-amber-600">{{ number_format($type->note_moyenne ?? 0, 1) }}</span>
                                            <span class="text-xs text-slate-500 ml-1">/10</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        @if ($type->nombre_annulations > 0)
                                            <span
                                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                                {{ $type->nombre_annulations }}
                                            </span>
                                        @else
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div
                ...
                <div class="p-6 space-y-4">
                    @if (isset($topReunions) && count($topReunions) > 0)
                        @foreach ($topReunions->take(5) as $reunion)
                            <div class="flex items-center justify-between">
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-slate-900 truncate">{{ $reunion->titre }}</p>
                                    <p class="text-xs text-slate-500">
                                        {{ \Carbon\Carbon::parse($reunion->date_reunion)->format('d/m/Y') }}</p>
                                </div>
                                <div class="ml-2 flex-shrink-0 flex items-center">
                                    <span class="text-sm font-bold text-amber-600">{{ $reunion->note_globale }}/10</span>
                                    <div class="ml-2 flex">
                                        @for ($i = 1; $i <= 5; $i++)
                                            @if ($i <= ceil($reunion->note_globale / 2))
            ...
            <div
                ...
                <div class="p-6 space-y-4">
                    @if (isset($reunionsFrequentees) && count($reunionsFrequentees) > 0)
                        @foreach ($reunionsFrequentees->take(5) as $reunion)
                            <div class="flex items-center justify-between">
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-slate-900 truncate">{{ $reunion->titre }}</p>
                                    <p class="text-xs text-slate-500">
                                ...
                                <div class="ml-2 flex-shrink-0">
                                    <span
                                        ...
                                        {{ $reunion->nombre_participants_reel }} participants
                                    </span>
                                </div>
                            </div>
                        @endforeach
            ...
            <div
                ...
                <div class="p-6 space-y-4">
                    @if (isset($topOrganisateurs) && count($topOrganisateurs) > 0)
                        @foreach ($topOrganisateurs->take(5) as $organisateur)
                            <div class="flex items-center justify-between">
                                <div class="flex items-center flex-1 min-w-0">
                                    <div
                                        class="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white text-xs font-medium mr-3">
                                        {{ substr($organisateur->nom, 0, 1) }}{{ substr($organisateur->prenom, 0, 1) }}
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-sm font-medium text-slate-900 truncate">{{ $organisateur->nom }}
                                            {{ $organisateur->prenom }}</p>
                                        <p class="text-xs text-slate-500">{{ $organisateur->nombre_reunions }} réunion(s)
                                        </p>
                                    </div>
                                </div>
                                <div class="ml-2 flex-shrink-0">
                                    <span
                                        class="text-xs text-slate-600">{{ number_format($organisateur->moyenne_participants, 1) }}
                                        moy.</span>
                                </div>
                            </div>
                        @endforeach
        ...
        <!-- Actions d'export -->
        @can('reunions.upload-documents')
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row gap-4 justify-center">
                        <button onclick="exportStatistiques('pdf')"
                            class="inline-flex items-center justify-center px-6 py-3 bg-red-600 text-white font-medium rounded-xl hover:bg-red-700 transition-colors">
                            <i class="fas fa-file-pdf mr-2"></i> Exporter en PDF
                        </button>
                        <button onclick="exportStatistiques('excel')"
                        ...
                        <button onclick="exportStatistiques('csv')"
                        ...
                        <button onclick="imprimerStatistiques()"
    ...
    @push('scripts')
        ...
        <script>
            ...
            function exportStatistiques(format) {
                ...
                window.open(`{{ route('private.reunions.statistiques') }}?${params.toString()}`, '_blank');
            }
