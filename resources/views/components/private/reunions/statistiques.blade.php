...
<div class="space-y-8">
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.reunions.statistiques') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Date de début</label>
                    <input type="date" name="date_debut" value="{{ request('date_debut', now()->subMonth()->format('Y-m-d')) }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                </div>
                <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $statistiques['globales']['total_reunions'] ?? 0 }}</p>
            ...
            <div class="mt-4 pt-4 border-t border-slate-200">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium text-green-600">{{ $statistiques['globales']['reunions_terminees'] ?? 0 }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-slate-600">Annulées</span>
                    <span class="font-medium text-red-600">{{ $statistiques['globales']['reunions_annulees'] ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ number_format($statistiques['globales']['total_participants'] ?? 0) }}</p>
            ...
            <div class="mt-4 pt-4 border-t border-slate-200">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium text-slate-900">{{ number_format($statistiques['globales']['moyenne_participants'] ?? 0, 1) }}</span>
                </div>
                <div class="flex justify-between text-sm">
                    <span class="text-slate-600">Nouveaux</span>
                    <span class="font-medium text-blue-600">{{ $statistiques['globales']['total_nouveaux'] ?? 0 }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $statistiques['globales']['total_decisions'] ?? 0 }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ number_format($statistiques['globales']['satisfaction_moyenne'] ?? 0, 1) }}</p>
            ...
            <div class="mt-4 pt-4 border-t border-slate-200">
                <div class="flex justify-between text-sm">
                    ...
                    <span class="font-medium text-amber-600">{{ number_format(($statistiques['globales']['note_moyenne'] ?? 0), 1) }}/10</span>
                </div>
            </div>
        </div>
    </div>
                        ...
                        @foreach($statistiques['par_type'] as $type)
                            <tr class="hover:bg-slate-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="w-3 h-3 rounded-full mr-3" style="background-color: {{ $type->couleur ?? '#6B7280' }}"></div>
                                        <div>
                                            <div class="text-sm font-medium text-slate-900">{{ $type->nom_type }}</div>
                                            <div class="text-sm text-slate-500">{{ $type->categorie ?? 'Non définie' }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-medium text-slate-900">{{ $type->nombre_reunions }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm text-slate-900">{{ number_format($type->total_participants) }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-medium text-green-600">{{ number_format($type->moyenne_participants, 1) }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm text-slate-600">{{ $type->max_participants }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-medium text-purple-600">{{ $type->total_decisions ?? 0 }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm text-slate-900">{{ number_format($type->satisfaction_moyenne ?? 0, 1) }}%</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center">
                                        <span class="text-sm font-medium text-amber-600">{{ number_format($type->note_moyenne ?? 0, 1) }}</span>
                    ...
                    @foreach($topReunions->take(5) as $reunion)
                        <div class="flex items-center justify-between">
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-slate-500">{{ \Carbon\Carbon::parse($reunion->date_reunion)->format('d/m/Y') }}</p>
                            </div>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center flex-1 min-w-0">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="text-sm font-medium text-slate-900 truncate">{{ $organisateur->nom }} {{ $organisateur->prenom }}</p>
                                    <p class="text-xs text-slate-500">{{ $organisateur->nombre_reunions }} réunion(s)</p>
                                </div>
                            </div>
                            <div class="ml-2 flex-shrink-0">
                                <span class="text-xs text-slate-600">{{ number_format($organisateur->moyenne_participants, 1) }} moy.</span>
                            </div>
                        </div>
                    @endforeach
                @else
