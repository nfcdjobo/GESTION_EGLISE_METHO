        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @endcan
                    <a href="{{ route('private.reunions.index') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-slate-600 to-gray-600 text-white text-sm font-medium rounded-xl hover:from-slate-700 hover:to-gray-700 transition-all duration-200 shadow-md hover:shadow-lg">
...
function createCalendarCell(date, currentMonth) {
    ...
    cell.addEventListener('click', () => {
        if (dayReunions.length > 0) {
            ...
            window.location.href = `{{ route('private.reunions.create') }}?date=${date.toISOString().split('T')[0]}`;
...
function showReunionModal(reunion) {
    ...
    voirPlus.href = `{{route('private.reunions.show', ':reunion')}}`.replace(':reunion', reunion.id);
...
function exportCalendar(format) {
    ...
    window.open(`{{ route('private.reunions.calendrier') }}?${params.toString()}`, '_blank');
}
...
function syncWithServer() {
    fetch(`{{ route('private.reunions.calendrier') }}?api=1&annee=${currentDate.getFullYear()}&mois=${currentDate.getMonth() + 1}`)
