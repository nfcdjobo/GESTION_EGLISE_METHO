    ...
    <div class="space-y-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-slate-500 mt-1">
                Suivi et gestion des souscriptions FIMECO - {{ \Carbon\Carbon::now()->locale('fr')->format('l d F Y') }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $subscriptions->total() }}</p>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $subscriptions->where('statut', 'completement_payee')->count() }}</p>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $subscriptions->where('statut', 'partiellement_payee')->count() }}</p>
            ...
            <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $subscriptions->filter(function($s) { return $s->en_retard; })->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="p-6 border-b border-slate-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @endcan
                        @can('subscriptions.dashboard')
                            <a href="{{ route('private.subscriptions.dashboard') }}"
            ...
            @can('subscriptions.search')
                <div class="p-6">
                    <form method="GET" action="{{ route('private.subscriptions.index') }}"
                        ...
                        <div>
                            ...
                            <select name="fimeco_id"
                                ...
                                @foreach(\App\Models\Fimeco::orderBy('nom')->get() as $fimeco)
                                    <option value="{{ $fimeco->id }}" {{ request('fimeco_id') === $fimeco->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="en_retard"
                                ...
                                <option value="1" {{ request('en_retard') === '1' ? 'selected' : '' }}>En retard uniquement</option>
                                <option value="0" {{ request('en_retard') === '0' ? 'selected' : '' }}>À jour uniquement</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-2">Date souscription</label>
                            <input type="date" name="date_souscription_debut" value="{{ request('date_souscription_debut') }}"
                                class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                        </div>
            ...
            <div class="p-6 border-b border-slate-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <h2 class="text-xl font-bold text-slate-800 flex items-center">
                        ...
                        Liste des Souscriptions ({{ $subscriptions->total() }})
                    </h2>
                    <div class="flex items-center space-x-2">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                            {{ $subscriptions->total() }} résultats
                        </span>
                        @if ($subscriptions->hasPages())
                            <span class="text-sm text-slate-600">
                                Page {{ $subscriptions->currentPage() }} sur {{ $subscriptions->lastPage() }}
                            </span>
                        @endif
                    </div>
                </div>
                ...
                @if ($subscriptions->count() > 0)
                        ...
                        <table class="min-w-full">
                            <thead>
                                <tr class="border-b border-slate-200">
                                    ...
                                    <th class="px-4 py-3 text-left text-xs font-bold text-slate-700 uppercase tracking-wider">
                                        <a href="{{ request()->fullUrlWithQuery(['sort_by' => 'date_souscription', 'sort_direction' => request('sort_direction') === 'asc' ? 'desc' : 'asc']) }}"
                                            class="group inline-flex items-center hover:text-blue-600 transition-colors">
                                            Dates
                                            <span class="ml-2 flex-none rounded text-slate-400 group-hover:text-blue-500">
                                                <i class="fas fa-sort"></i>
                                ...
                                @foreach ($subscriptions as $subscription)
                                    <tr class="hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-all duration-200">
                                            ...
                                            <div class="flex items-center">
                                                ...
                                                <div class="ml-4">
                                                    <div class="text-sm font-bold text-slate-900">
                                                        {{ $subscription->souscripteur?->nom ?? 'Utilisateur supprimé' }}
                                                    </div>
                                                    @if($subscription->souscripteur?->email)
                                                        <div class="text-xs text-slate-500">
                                                            {{ $subscription->souscripteur->email }}
                                                        </div>
                                                    @endif
                                                    @if($subscription->souscripteur?->telephone_1)
                                        ...
                                        <td class="px-4 py-4">
                                            <div>
                                                <div class="text-sm font-medium text-slate-900">
                                                    {{ Str::limit($subscription->fimeco->nom, 25) }}
                                                </div>
                                                <div class="text-xs text-slate-500">
                                                    Progression: {{ number_format($subscription->fimeco->progression, 1) }}%
                                                </div>
                                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium
                                                    @if($subscription->fimeco->statut === 'active') bg-green-100 text-green-800
                                                    @elseif($subscription->fimeco->statut === 'cloturee') bg-gray-100 text-gray-800
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="space-y-1">
                                                <div class="text-sm text-slate-900">
                                                    ...
                                                    {{ $subscription->date_souscription->format('d/m/Y') }}
                                                </div>
                                                @if($subscription->date_echeance)
                                                    <div class="text-sm {{ $subscription->en_retard ? 'text-red-600' : 'text-slate-600' }}">
                                                        <i class="fas fa-calendar-times {{ $subscription->en_retard ? 'text-red-600' : 'text-orange-600' }} mr-1"></i>
                                                        {{ $subscription->date_echeance->format('d/m/Y') }}
                                                    </div>
                                                    @if($subscription->en_retard)
                                                        <div class="text-xs text-red-600 font-medium">
                                                            <i class="fas fa-exclamation-triangle mr-1"></i>
                                                            Retard: {{ $subscription->jours_retard }} jours
                                                        </div>
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="space-y-1">
                                                <div class="text-sm font-medium text-slate-900">
                                                    Souscrit: {{ number_format($subscription->montant_souscrit, 0, ',', ' ') }} FCFA
                                                </div>
                                                <div class="text-sm text-green-600">
                                                    Payé: {{ number_format($subscription->montant_paye, 0, ',', ' ') }} FCFA
                                                </div>
                                                @if($subscription->reste_a_payer > 0)
                                                    <div class="text-sm text-orange-600">
                                                        Reste: {{ number_format($subscription->reste_a_payer, 0, ',', ' ') }} FCFA
                                                    </div>
                                                @endif
                                            </div>
                                        </td>
                                            ...
                                            <div class="space-y-2">
                                                <div class="flex items-center space-x-2">
                                                    <div class="flex-1 bg-slate-200 rounded-full h-2">
                                                        <div class="h-2 rounded-full {{ $subscription->progression >= 100 ? 'bg-green-500' : ($subscription->progression >= 75 ? 'bg-blue-500' : ($subscription->progression >= 50 ? 'bg-yellow-500' : 'bg-red-500')) }}"
                                                             style="width: {{ min($subscription->progression, 100) }}%"></div>
                                                    </div>
                                                    <span class="text-xs font-medium text-slate-700">
                                                        {{ number_format($subscription->progression, 1) }}%
                                                    </span>
                                                </div>
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                    @if($subscription->statut_global === 'objectif_atteint') bg-green-100 text-green-800
                                                    @elseif($subscription->statut_global === 'presque_atteint') bg-blue-100 text-blue-800
                                                    @elseif($subscription->statut_global === 'en_cours') bg-yellow-100 text-yellow-800
                                                    @else bg-gray-100 text-gray-800 @endif">
                                                    {{ ucfirst(str_replace('_', ' ', $subscription->statut_global)) }}
                                                </span>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4">
                                            <div class="space-y-1">
                                                ...
                                                @if($subscription->payments->where('statut', 'en_attente')->count() > 0)
                                                    <div class="text-xs text-orange-600">
                                                        <i class="fas fa-clock mr-1"></i>
                                                        {{ $subscription->payments->where('statut', 'en_attente')->count() }} en attente
                                                    </div>
                                                @endif
                                                @if($subscription->payments->where('statut', 'valide')->count() > 0)
                                                    <div class="text-xs text-green-600">
                                                        <i class="fas fa-check mr-1"></i>
                                                        {{ $subscription->payments->where('statut', 'valide')->count() }} validé(s)
                                                    </div>
                                                @endif
                                                @if($subscription->dernierPaiement)
                                                    <div class="text-xs text-slate-500">
                                                        Dernier: {{ $subscription->dernierPaiement->date_paiement->format('d/m/Y') }}
                                                    </div>
                                                @endif
                                            </div>
                                        </td>
                                            ...
                                            <div class="flex items-center justify-end space-x-2">
                                                ...
                                                @if($subscription->statut === 'partiellement_payee')
                                                    @can('subscriptions.deactivate')
                                                        <button onclick="desactiverSouscription('{{ $subscription->id }}')"
                                                ...
                                                @elseif($subscription->statut === 'inactive')
                                                    @can('subscriptions.reactivate')
                                                        <button onclick="reactiverSouscription('{{ $subscription->id }}')"
                                                            class="inline-flex items-center justify-center w-8 h-8 text-green-600 bg-green-100 rounded-lg hover:bg-green-200 transition-colors"
                                                            title="Réactiver">
                                                ...
                                                @can('subscriptions.delete')
                                                    <button onclick="deleteSouscription('{{ $subscription->id }}')"
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mt-6 pt-6 border-t border-slate-200">
                        <div class="text-sm text-slate-700">
                            Affichage de <span class="font-medium">{{ $subscriptions->firstItem() }}</span> à <span
                                class="font-medium">{{ $subscriptions->lastItem() }}</span> sur <span
                                class="font-medium">{{ $subscriptions->total() }}</span> résultats
                        </div>
                        <div>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="text-slate-500 mb-6">
                            @if (request()->hasAny(['search', 'statut', 'fimeco_id', 'en_retard', 'date_souscription_debut']))
                        ...
                        @can('subscriptions.create')
                            <a href="{{ route('private.subscriptions.create') }}"
                                ...
                                <i class="fas fa-plus mr-2"></i> Créer une souscription
                            </a>
                        @endcan
                    </div>
                @endif
    ...
    @push('scripts')
        <script>
            ...
            function confirmExport() {
                if (selectedFormat) {
                    window.location.href = `{{ route('private.subscriptions.export') }}?format=${selectedFormat}`;
                    closeExportModal();
                } else {
            ...
            function deleteSouscription(subscriptionId) {
                if (confirm('Êtes-vous sûr de vouloir supprimer cette souscription ?')) {
                    const url = "{{ route('private.subscriptions.destroy', ':id') }}".replace(':id', subscriptionId);
            ...
            function desactiverSouscription(subscriptionId) {
                if (confirm('Désactiver cette souscription ?')) {
                    const url = "{{ route('private.subscriptions.desactiver', ':id') }}".replace(':id', subscriptionId);
            ...
            function reactiverSouscription(subscriptionId) {
                if (confirm('Réactiver cette souscription ?')) {
                    const url = "{{ route('private.subscriptions.reactiver', ':id') }}".replace(':id', subscriptionId);
