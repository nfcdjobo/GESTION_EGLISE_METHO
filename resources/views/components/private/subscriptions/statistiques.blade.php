...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="bg-white rounded-xl shadow-lg p-6 border border-slate-200">
            ...
            <p class="text-slate-500 mt-1">Analyse de votre activité et performance - {{ \Carbon\Carbon::now()->format('l d F Y') }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ number_format($statistiques['total_souscrit'] ?? 0, 0, ',', ' ') }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ number_format($statistiques['total_reste'] ?? 0, 0, ',', ' ') }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-2xl font-bold text-slate-800">{{ $statistiques['nombre_souscriptions'] ?? 0 }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="text-center">
                    <div class="text-4xl font-bold text-blue-600 mb-2">{{ $tauxPaiementGlobal }}%</div>
                ...
                <div class="w-full bg-gray-200 rounded-full h-4">
                    <div class="bg-gradient-to-r from-blue-500 via-purple-500 to-green-500 h-4 rounded-full transition-all duration-500"
                         style="width: {{ $tauxPaiementGlobal }}%"></div>
                </div>

                <!-- Détails financiers -->
                <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200">
                    <div class="text-center">
                        <div class="text-lg font-bold text-green-600">{{ number_format($statistiques['total_paye'] ?? 0, 0, ',', ' ') }}</div>
                        <div class="text-sm text-green-700">Payé</div>
                    </div>
                    <div class="text-center">
                        <div class="text-lg font-bold text-orange-600">{{ number_format($statistiques['total_reste'] ?? 0, 0, ',', ' ') }}</div>
                        <div class="text-sm text-orange-700">À payer</div>
                    </div>
                </div>

                <!-- Moyennes -->
                @if(($statistiques['nombre_souscriptions'] ?? 0) > 0)
                    <div class="pt-4 border-t border-slate-200 space-y-3">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Souscription moyenne:</span>
                            <span class="text-sm font-bold text-slate-900">
                                {{ number_format(($statistiques['total_souscrit'] ?? 0) / ($statistiques['nombre_souscriptions'] ?? 1), 0, ',', ' ') }} FCFA
                            </span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-slate-700">Paiement moyen:</span>
                            <span class="text-sm font-bold text-slate-900">
                                {{ number_format(($statistiques['total_paye'] ?? 0) / ($statistiques['nombre_souscriptions'] ?? 1), 0, ',', ' ') }} FCFA
                            </span>
                        </div>
                    </div>
                @endif
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            ...
            <div class="p-6 space-y-4">
                @if(isset($statistiques['repartition_statuts']))
                    @foreach($statistiques['repartition_statuts'] as $statut => $data)
                        <div class="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                            <div class="flex items-center space-x-3">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    @if($statut === 'completement_payee') bg-green-100 text-green-800
                                    @elseif($statut === 'partiellement_payee') bg-yellow-100 text-yellow-800
                                    @elseif($statut === 'active') bg-blue-100 text-blue-800
                                    @else bg-red-100 text-red-800
                                    @endif">
                                    {{ ucfirst(str_replace('_', ' ', $statut)) }}
                                </span>
                                <span class="text-sm text-slate-700">{{ $data['nombre'] ?? 0 }} souscriptions</span>
                            </div>
                            <div class="text-right">
                                <div class="text-sm font-bold text-slate-900">{{ number_format($data['montant'] ?? 0, 0, ',', ' ') }} FCFA</div>
                                <div class="text-xs text-slate-500">
                                    {{ ($statistiques['total_souscrit'] ?? 0) > 0 ? round((($data['montant'] ?? 0) / ($statistiques['total_souscrit'] ?? 1)) * 100, 1) : 0 }}%
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <!-- Historique mensuel -->
    @if(isset($statistiques['evolution_mensuelle']) && count($statistiques['evolution_mensuelle']) > 0)
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="p-6 border-b border-slate-200">
                <h2 class="text-xl font-bold text-slate-800 flex items-center">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach($statistiques['evolution_mensuelle'] as $periode => $donnees)
                        <div class="p-4 bg-gradient-to-r from-slate-50 to-blue-50 rounded-lg">
                            <div class="font-medium text-slate-900 mb-2">{{ $periode }}</div>
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-slate-600">Souscriptions:</span>
                                    <span class="font-medium">{{ $donnees['souscriptions'] ?? 0 }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-slate-600">Paiements:</span>
                                    <span class="font-medium">{{ $donnees['paiements'] ?? 0 }}</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-slate-600">Montant:</span>
                                    <span class="font-medium text-green-600">{{ number_format($donnees['montant'] ?? 0, 0, ',', ' ') }} FCFA</span>
                                </div>
                            </div>
                        </div>
                    @endforeach
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        @if(isset($statistiques['performance_par_fimeco']) && count($statistiques['performance_par_fimeco']) > 0)
            <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                <div class="p-6 border-b border-slate-200">
                    <h2 class="text-xl font-bold text-slate-800 flex items-center">
                ...
                <div class="p-6 space-y-4">
                    @foreach($statistiques['performance_par_fimeco'] as $fimeco)
                        <div class="p-4 border border-slate-200 rounded-lg">
                            <div class="flex items-start justify-between mb-3">
                                <h3 class="font-medium text-slate-900">{{ $fimeco['nom'] }}</h3>
                                <span class="text-sm font-bold text-green-600">{{ $fimeco['taux_paiement'] }}%</span>
                            </div>
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-slate-600">Souscrit:</span>
                                    <span>{{ number_format($fimeco['montant_souscrit'], 0, ',', ' ') }} FCFA</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-slate-600">Payé:</span>
                                    <span class="text-green-600">{{ number_format($fimeco['montant_paye'], 0, ',', ' ') }} FCFA</span>
                                </div>
                            </div>
                            <div class="w-full bg-gray-200 rounded-full h-2 mt-3">
                                <div class="bg-gradient-to-r from-green-400 to-green-600 h-2 rounded-full"
                                     style="width: {{ $fimeco['taux_paiement'] }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                @if(count($recommendations) > 0)
                    @foreach($recommendations as $rec)
                        <div class="p-4 rounded-lg border
                            @if($rec['type'] === 'success') bg-green-50 border-green-200
                            @elseif($rec['type'] === 'warning') bg-yellow-50 border-yellow-200
                            @else bg-blue-50 border-blue-200
                            @endif">
                            <div class="flex">
                                <i class="{{ $rec['icon'] }}
                                    @if($rec['type'] === 'success') text-green-400
                                    @elseif($rec['type'] === 'warning') text-yellow-400
                                    @else text-blue-400
                                    @endif mt-0.5 mr-3"></i>
                                <div class="text-sm">
                                    <p class="font-medium
                                        ...
                                        @endif">{{ $rec['title'] }}</p>
                                    <p class="mt-1
                                        @if($rec['type'] === 'success') text-green-700
                                        @elseif($rec['type'] === 'warning') text-yellow-700
                                        @else text-blue-700
                                        @endif">{{ $rec['message'] }}</p>
                                </div>
                            </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
        <div class="p-6">
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                {{-- <a href="" class="inline-flex items-center justify-center px-8 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium rounded-xl hover:from-blue-700 hover:to-purple-700 transition-all duration-200 shadow-md hover:shadow-lg">
                ...
                <a href="{{ route('private.paiements.index') }}" class="inline-flex items-center justify-center px-8 py-3 bg-green-600 text-white font-medium rounded-xl hover:bg-green-700 transition-colors">
