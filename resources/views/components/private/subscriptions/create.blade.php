                        ...
                        <div class="p-6 space-y-6">
                            ...
                            <div>
                                ...
                                <div class="p-4 bg-blue-50 border border-blue-200 rounded-xl">
                                    ...
                                    <div class="flex items-center gap-4 mt-3 text-sm text-blue-600">
                                        <span><i
                                                class="fas fa-calendar mr-1"></i>{{ \Carbon\Carbon::parse($fimecoActive->debut)->format('d/m/Y') }}
                                            - {{ \Carbon\Carbon::parse($fimecoActive->fin)->format('d/m/Y') }}</span>
                                        <span><i
                                                class="fas fa-bullseye mr-1"></i>{{ number_format($fimecoActive->cible, 0, ',', ' ') }}
                                            FCFA</span>
                                    </div>
                                </div>
                                {{-- @else
                                <select id="fimeco_id" name="fimeco_id" required onchange="updateFimecoInfo()"
                                    ...
                                    <option value="{{ $fimeco->id }}" data-nom="{{ $fimeco->nom }}"
                                        data-description="{{ $fimeco->description }}" data-debut="{{ $fimeco->debut }}"
                                        data-fin="{{ $fimeco->fin }}" data-cible="{{ $fimeco->cible }}" {{
                                ...
                                <div id="fimeco-info" class="hidden mt-4 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                                    ...
                                    <div class="flex items-center gap-4 mt-3 text-sm text-blue-600">
                                        ...
                                        <span id="fimeco-cible"><i class="fas fa-bullseye mr-1"></i></span>
                                    </div>
                                </div>
                                @endif --}}
                            </div>
                            ...
                            {{-- <div>
                                ...
                                <select id="souscripteur_id" name="souscripteur_id" required onchange="() => fetchUsers()"
                                    ...
                                    @foreach($membresDisponibles as $user)
                                    <option value="{{ $user->id }}" data-nom="{{ $user->nom }}"
                                        data-telephone="{{ $user->telephone_1 }}" data-name="{{ $user->nom }}"
                            ...
                            <div>
                                ...
                                <div class="relative" data-type="souscripteur">
                                    <input type="text" id="souscripteur_search" name="souscripteur_search"
                                        ...
                                        class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors @error('souscripteur_id') border-red-500 focus:ring-red-500 focus:border-red-500 @enderror"
                                        autocomplete="off" value="{{ old('souscripteur_search') }}">

                                    <input type="hidden" id="souscripteur_id" name="souscripteur_id"
                                        value="{{ old('souscripteur_id') }}">
                            ...
                            <div>
                                ...
                                <input type="number" id="montant_souscrit" name="montant_souscrit"
                                    value="{{ old('montant_souscrit') }}" required min="10" step="0.01" placeholder="50000"
            ...
            class AutoComplete {
                ...
                async search(query) {
                    ...
                    try {
                        ...
                        const response = await fetch(`{{ route('private.users.index') }}?search=${encodeURIComponent(query)}`, {
                            headers: {
                                'Accept': 'application/json',
...
showAddUserModal(name) {
    ...
    if (this.hiddenInput.value) {
        ...
        try {
            const response = await fetch(`{{ route('private.users.index') }}?search=${this.hiddenInput.value}`, {
                headers: {
                    'Accept': 'application/json',
                    'X-Requested-With': 'XMLHttpRequest',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
