...
@section('content')
    <div class="space-y-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">{{ $users->where('statut_membre', 'actif')->count() }}
                        </p>
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ $users->where('statut_membre', 'nouveau_converti')->count() }}</p>
            ...
            <div
                ...
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <p class="text-2xl font-bold text-slate-800">
                            {{ $users->where('statut_membre', 'visiteur')->count() }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="p-6 border-b border-slate-200">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @can('users.create')
                            <a href="{{ route('private.users.create') }}"
                        ...
                        @endcan
                        @can('users.import')
                            <a href="{{ route('private.users.import') }}"
                        ...
                        @can('export-data', 'users')
                            <a href="{{ route('private.users.export') }}"
            ...
            @can('users.search')
                <div class="p-6">
                    <form method="GET" action="{{ route('private.users.index') }}"
                        ...
                        <div>
                            ...
                            <select name="statut_membre"
                                ...
                                <option value="actif" {{ request('statut_membre') === 'actif' ? 'selected' : '' }}>Actif
                                </option>
                                <option value="inactif" {{ request('statut_membre') === 'inactif' ? 'selected' : '' }}>Inactif
                                </option>
                                <option value="visiteur" {{ request('statut_membre') === 'visiteur' ? 'selected' : '' }}>
                                    Visiteur</option>
                                <option value="nouveau_converti"
                                    {{ request('statut_membre') === 'nouveau_converti' ? 'selected' : '' }}>Nouveau converti
                                </option>
                            </select>
                        </div>
                ...
                @if ($users->count() > 0)
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-12 w-12">
                                                    ...
                                                    @else
                                                        <div
                                                            class="h-12 w-12 rounded-full bg-gradient-to-br from-{{ $user->sexe === 'feminin' ? 'pink' : 'blue' }}-400 to-{{ $user->sexe === 'feminin' ? 'purple' : 'indigo' }}-500 flex items-center justify-center shadow-lg ring-4 ring-white">
                                        ...
                                        <td class="px-4 py-4">
                                            <div class="flex items-center justify-end space-x-2">
                                                ...
                                                @can('users.toggle-status')
                                                    @if (Auth::id() !== $user->id)
                                                        <button onclick="toggleStatus('{{ $user->id }}', '{{$user->nom.' '.$user->prenom}}', `{{$user->actif ? 'désactiver' : 'réactiver'}}`)" class="inline-flex items-center justify-center w-8 h-8 {{ $user->actif ? 'text-orange-600 bg-orange-100 hover:bg-orange-200' : 'text-green-600 bg-green-100 hover:bg-green-200' }} rounded-lg transition-colors" title="{{ $user->actif ? 'Désactiver' : 'Activer' }}">
                    ...
                    <div
                        ...
                        <div class="text-sm text-slate-700">
                            Affichage de <span class="font-medium">{{ $users->firstItem() }}</span> à <span
                                class="font-medium">{{ $users->lastItem() }}</span>sur <span
                                class="font-medium">{{ $users->total() }}</span> résultats
                        </div>
                        <div>
