...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="bg-white rounded-xl shadow-lg p-6 border border-slate-200">
            ...
            <p class="text-slate-500 mt-1">Analyse et métriques des événements - Période du {{ \Carbon\Carbon::parse($statistiques['periode']['debut'])->format('d/m/Y') }} au {{ \Carbon\Carbon::parse($statistiques['periode']['fin'])->format('d/m/Y') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
        ...
        <div class="p-6">
            <form method="GET" action="{{ route('private.events.statistiques') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Date de début</label>
                    <input type="date" name="date_debut" value="{{ request('date_debut', $statistiques['periode']['debut']) }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-2">Date de fin</label>
                    <input type="date" name="date_fin" value="{{ request('date_fin', $statistiques['periode']['fin']) }}" class="w-full px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                </div>
                <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-3xl font-bold text-slate-800">{{ $statistiques['totaux']['nombre_events'] }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-3xl font-bold text-slate-800">{{ $statistiques['totaux']['events_termines'] }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-3xl font-bold text-slate-800">{{ number_format($statistiques['totaux']['total_participants']) }}</p>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-3xl font-bold text-slate-800">{{ number_format($statistiques['totaux']['total_recettes']) }}</p>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="text-center">
                ...
                <div class="text-2xl font-bold text-slate-800">{{ $statistiques['moyennes']['participants_par_event'] }}</div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="text-center">
                ...
                <div class="text-2xl font-bold text-slate-800">{{ $statistiques['moyennes']['note_globale'] }}<span class="text-lg">/10</span></div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="text-center">
                ...
                <div class="text-2xl font-bold text-slate-800">{{ $statistiques['moyennes']['note_organisation'] }}<span class="text-lg">/10</span></div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="text-center">
                ...
                <div class="text-2xl font-bold text-slate-800">{{ $statistiques['moyennes']['note_contenu'] }}<span class="text-lg">/10</span></div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg p-6 border border-white/20 hover:shadow-xl transition-all duration-300">
            <div class="text-center">
                ...
                <div class="text-2xl font-bold text-slate-800">{{ $statistiques['moyennes']['taux_satisfaction'] }}<span class="text-lg">%</span></div>
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex-1">
                                    <div class="flex items-center justify-between mb-1">
                                        <span class="text-sm font-medium text-slate-700">{{ ucfirst(str_replace('_', ' ', $type->type_evenement)) }}</span>
                                        <span class="text-sm text-slate-500">{{ $type->nombre }} événements</span>
                                    </div>
                                    <div class="w-full bg-slate-200 rounded-full h-2">
                                        <div class="bg-gradient-to-r from-blue-400 to-purple-500 h-2 rounded-full" style="width: {{ $statistiques['totaux']['nombre_events'] > 0 ? ($type->nombre / $statistiques['totaux']['nombre_events']) * 100 : 0 }}%"></div>
                                    </div>
                                    <div class="flex items-center justify-between mt-1 text-xs text-slate-500">
                                        <span>{{ $type->total_participants }} participants</span>
                                        <span>Moyenne: {{ $type->moyenne_participants }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
                            ...
                            <div class="border border-slate-200 rounded-lg p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <h3 class="font-semibold text-slate-900">
                                        {{ \Carbon\Carbon::createFromDate($mois->annee, $mois->mois, 1)->format('F Y') }}
                                    </h3>
                                    <span class="text-sm text-slate-500">{{ $mois->nombre_events }} événements</span>
                                </div>
                                <div class="grid grid-cols-2 gap-4 text-sm">
                                    <div class="text-center p-2 bg-blue-50 rounded">
                                        <div class="font-bold text-blue-600">{{ $mois->total_participants }}</div>
                                        <div class="text-blue-800">Participants</div>
                                    </div>
                                    <div class="text-center p-2 bg-green-50 rounded">
                                        <div class="font-bold text-green-600">{{ number_format($mois->total_recettes) }} FCFA</div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
                            ...
                            <span class="font-bold text-green-600">
                                {{ $statistiques['totaux']['nombre_events'] > 0 ? round(($statistiques['totaux']['events_termines'] / $statistiques['totaux']['nombre_events']) * 100, 1) : 0 }}%
                            </span>
                        </div>
                        <div class="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
                            <span class="text-slate-700">Taux d'annulation</span>
                            <span class="font-bold text-red-600">
                                {{ $statistiques['totaux']['nombre_events'] > 0 ? round(($statistiques['totaux']['events_annules'] / $statistiques['totaux']['nombre_events']) * 100, 1) : 0 }}%
                            </span>
                        </div>
                        <div class="flex justify-between items-center p-3 bg-slate-50 rounded-lg">
                            <span class="text-slate-700">Budget moyen</span>
                            <span class="font-bold text-blue-600">{{ number_format($statistiques['totaux']['budget_total'] / max($statistiques['totaux']['nombre_events'], 1)) }} FCFA</span>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="space-y-3">
                        <div class="p-3 bg-blue-50 rounded-lg">
                            ...
                            <div class="text-2xl font-bold text-blue-600 mt-1">
                                {{ $statistiques['par_mois']->count() > 0 ? round($statistiques['totaux']['nombre_events'] / $statistiques['par_mois']->count(), 1) : 0 }}
                            </div>
                        </div>
                        ...
                        <div class="p-3 bg-green-50 rounded-lg">
                            ...
                            <div class="text-2xl font-bold text-green-600 mt-1">{{ $statistiques['moyennes']['taux_satisfaction'] }}%</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-3">
                        @if($statistiques['par_type']->isNotEmpty())
                            @php
                            ...
                            <div class="p-3 bg-yellow-50 rounded-lg">
                                ...
                                <div class="text-lg font-bold text-yellow-600 mt-1">{{ ucfirst($topType->type_evenement ?? 'N/A') }}</div>
                                <div class="text-sm text-yellow-700">{{ $topType->total_participants ?? 0 }} participants</div>
                            </div>
                        @endif
                        ...
                        <div class="p-3 bg-indigo-50 rounded-lg">
                            ...
                            @if($statistiques['par_mois']->isNotEmpty())
                                @php
                                    $bestMonth = $statistiques['par_mois']->sortByDesc('nombre_events')->first();
                                @endphp
                                <div class="text-lg font-bold text-indigo-600 mt-1">
                                    {{ \Carbon\Carbon::createFromDate($bestMonth->annee, $bestMonth->mois, 1)->format('F') }}
                                </div>
                                <div class="text-sm text-indigo-700">{{ $bestMonth->nombre_events }} événements</div>
                            @else
