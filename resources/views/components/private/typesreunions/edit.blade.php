...
@section('title', 'Modifier ' . $typeReunion->nom)

@section('content')
<div class="space-y-8">
    <!-- Page Title & Breadcrumb -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Modifier {{ $typeReunion->nom }}</h1>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('private.types-reunions.show', $typeReunion) }}" class="text-sm font-medium text-slate-700 hover:text-blue-600 transition-colors">{{ $typeReunion->code }}</a>
                    </div>
                </li>
                <li>
                    <div class="flex items-center">
    ...
    <form action="{{ route('private.types-reunions.update', $typeReunion) }}" method="POST" id="typeReunionForm" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    <div class="flex flex-col p-6 border-b border-slate-200 sm:flex-row sm:items-center sm:justify-between gap-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('private.types-reunions.show', $typeReunion) }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-cyan-600 to-blue-600 text-white text-sm font-medium rounded-xl hover:from-cyan-700 hover:to-blue-700 transition-all duration-200 shadow-md hover:shadow-lg">
                                <i class="fas fa-eye mr-2"></i> Voir
                            </a>
                            <button type="button" onclick="duplicateType('{{ $typeReunion->id }}')" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-medium rounded-xl hover:from-purple-700 hover:to-pink-700 transition-all duration-200 shadow-md hover:shadow-lg">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="text" id="nom" name="nom" value="{{ old('nom', $typeReunion->nom) }}" required maxlength="150"
                            ...
                            <div>
                                ...
                                <input type="text" id="code" name="code" value="{{ old('code', $typeReunion->code) }}" required maxlength="50"
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4"
                                ...
                                placeholder="Description détaillée du type de réunion...">{{ old('description', $typeReunion->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div>
                                ...
                                <select id="categorie" name="categorie" required
                                    ...
                                    @foreach($categories as $key => $label)
                                        <option value="{{ $key }}" {{ old('categorie', $typeReunion->categorie) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('categorie')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            ...
                            <div>
                                ...
                                <select id="niveau_acces" name="niveau_acces" required
                                    ...
                                    @foreach($niveauxAcces as $key => $label)
                                        <option value="{{ $key }}" {{ old('niveau_acces', $typeReunion->niveau_acces) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
                                @error('niveau_acces')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="text" id="icone" name="icone" value="{{ old('icone', $typeReunion->icone) }}" maxlength="100"
                            ...
                            <div>
                                ...
                                <div class="flex items-center space-x-2">
                                    <input type="color" id="couleur" name="couleur" value="{{ old('couleur', $typeReunion->couleur ?? '#3498db') }}"
                                        class="w-12 h-12 border border-slate-300 rounded-xl @error('couleur') border-red-500 @enderror">
                                    <input type="text" id="couleur_text" value="{{ old('couleur', $typeReunion->couleur ?? '#3498db') }}"
                            ...
                            <div>
                                ...
                                <select id="priorite" name="priorite"
                                    ...
                                    @for($i = 1; $i <= 10; $i++)
                                        <option value="{{ $i }}" {{ old('priorite', $typeReunion->priorite) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                        ...
                        @if($typeReunion->nombre_utilisations > 0)
                            <div class="p-4 bg-amber-50 border border-amber-200 rounded-xl">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-amber-700 mt-1">Ce type a été utilisé {{ $typeReunion->nombre_utilisations }} fois. Les modifications importantes peuvent affecter les réunions existantes.</p>
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    <div class="p-6 space-y-6">
                        <div>
                            ...
                            <select id="frequence_type" name="frequence_type" required
                                ...
                                @foreach($frequences as $key => $label)
                                    <option value="{{ $key }}" {{ old('frequence_type', $typeReunion->frequence_type) == $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                <input type="time" id="duree_standard" name="duree_standard" value="{{ old('duree_standard', $typeReunion->duree_standard?->format('H:i')) }}"
                            ...
                            <div>
                                ...
                                <input type="time" id="duree_min" name="duree_min" value="{{ old('duree_min', $typeReunion->duree_min?->format('H:i')) }}"
                            ...
                            <div>
                                ...
                                <input type="time" id="duree_max" name="duree_max" value="{{ old('duree_max', $typeReunion->duree_max?->format('H:i')) }}"
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                ...
                                <div class="space-y-3">
                                    <label class="flex items-center">
                                        <input type="checkbox" name="necessite_preparation" value="1" {{ old('necessite_preparation', $typeReunion->necessite_preparation) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="ml-2 text-sm text-slate-700">Nécessite une préparation spéciale</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="necessite_inscription" value="1" {{ old('necessite_inscription', $typeReunion->necessite_inscription) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="ml-2 text-sm text-slate-700">Inscription obligatoire</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="a_limite_participants" value="1" {{ old('a_limite_participants', $typeReunion->a_limite_participants) ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center">
                                        <input type="checkbox" name="permet_enfants" value="1" {{ old('permet_enfants', $typeReunion->permet_enfants) ? 'checked' : '' }}
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="space-y-3">
                                    <label class="flex items-center">
                                        <input type="checkbox" name="inclut_louange" value="1" {{ old('inclut_louange', $typeReunion->inclut_louange) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="ml-2 text-sm text-slate-700">Inclut un temps de louange</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="inclut_message" value="1" {{ old('inclut_message', $typeReunion->inclut_message) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="ml-2 text-sm text-slate-700">Inclut un message/enseignement</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="inclut_priere" value="1" {{ old('inclut_priere', $typeReunion->inclut_priere) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="ml-2 text-sm text-slate-700">Inclut un temps de prière</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="inclut_communion" value="1" {{ old('inclut_communion', $typeReunion->inclut_communion) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                        <span class="ml-2 text-sm text-slate-700">Peut inclure la communion</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="checkbox" name="permet_temoignages" value="1" {{ old('permet_temoignages', $typeReunion->permet_temoignages) ? 'checked' : '' }}
                                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                        ...
                        <div id="section_limite_participants" class="{{ $typeReunion->a_limite_participants ? '' : 'hidden' }}">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label for="limite_participants" class="block text-sm font-medium text-slate-700 mb-2">Limite de participants</label>
                                    <input type="number" id="limite_participants" name="limite_participants" value="{{ old('limite_participants', $typeReunion->limite_participants) }}" min="1"
                                        class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                                </div>
                            </div>
                        </div>

                        <div id="section_age_enfants" class="{{ $typeReunion->permet_enfants ? '' : 'hidden' }}">
                            <div>
                                <label for="age_minimum" class="block text-sm font-medium text-slate-700 mb-2">Âge minimum</label>
                                <input type="number" id="age_minimum" name="age_minimum" value="{{ old('age_minimum', $typeReunion->age_minimum) }}" min="0" max="99"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <label class="flex items-center">
                                    <input type="checkbox" name="collecte_offrandes" value="1" {{ old('collecte_offrandes', $typeReunion->collecte_offrandes) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                    <span class="ml-2 text-sm text-slate-700">Collecte d'offrandes</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="a_frais_participation" value="1" {{ old('a_frais_participation', $typeReunion->a_frais_participation) ? 'checked' : '' }}
                            ...
                            <div id="section_frais_participation" class="{{ $typeReunion->a_frais_participation ? '' : 'hidden' }}">
                                <div>
                                    <label for="frais_standard" class="block text-sm font-medium text-slate-700 mb-2">Frais standard (XOF)</label>
                                    <input type="number" id="frais_standard" name="frais_standard" value="{{ old('frais_standard', $typeReunion->frais_standard) }}" min="0" step="0.01"
                                        class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="space-y-4">
                                <label class="flex items-center">
                                    <input type="checkbox" name="afficher_calendrier_public" value="1" {{ old('afficher_calendrier_public', $typeReunion->afficher_calendrier_public) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                    <span class="ml-2 text-sm text-slate-700">Afficher sur le calendrier public</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="afficher_site_web" value="1" {{ old('afficher_site_web', $typeReunion->afficher_site_web) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                                    <span class="ml-2 text-sm text-slate-700">Afficher sur le site web</span>
                                </label>
                                <label class="flex items-center">
                                    <input type="checkbox" name="actif" value="1" {{ old('actif', $typeReunion->actif) ? 'checked' : '' }}
                                        class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500">
                            ...
                            <div>
                                ...
                                <input type="number" id="ordre_affichage" name="ordre_affichage" value="{{ old('ordre_affichage', $typeReunion->ordre_affichage) }}" min="0"
                                    class="w-full px-4 py-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors">
                ...
                <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20 hover:shadow-xl transition-all duration-300">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="text-center mb-4">
                            <div id="preview-icon" class="w-16 h-16 rounded-xl flex items-center justify-center text-white shadow-lg mx-auto mb-2" style="background-color: {{ $typeReunion->couleur ?? '#3498db' }}">
                                <i class="fas fa-{{ $typeReunion->icone ?? 'calendar-alt' }} text-2xl"></i>
                            </div>
                            <h3 id="preview-name" class="font-semibold text-slate-800">{{ $typeReunion->nom }}</h3>
                            <p id="preview-code" class="text-sm text-slate-500">{{ $typeReunion->code }}</p>
                        </div>

                        <div class="space-y-2 text-sm">
                            <div class="flex items-center justify-between">
                                <span class="text-slate-700">Catégorie:</span>
                                <span id="preview-categorie" class="text-slate-600">{{ ucfirst($typeReunion->categorie) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-700">Accès:</span>
                                <span id="preview-acces" class="text-slate-600">{{ ucfirst(str_replace('_', ' ', $typeReunion->niveau_acces)) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-700">Fréquence:</span>
                                <span id="preview-frequence" class="text-slate-600">{{ ucfirst(str_replace('_', ' ', $typeReunion->frequence_type)) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-slate-700">Durée:</span>
                                <span id="preview-duree" class="text-slate-600">{{ $typeReunion->duree_standard?->format('H:i') ?? '-' }}</span>
                            </div>
                        </div>
                    ...
                    <div class="p-6 space-y-3 text-sm">
                        ...
                        @if($typeReunion->derniere_utilisation)
                            <div class="flex justify-between">
                                ...
                                <span class="font-medium text-slate-800">{{ $typeReunion->derniere_utilisation->format('d/m/Y') }}</span>
                            </div>
                        @endif
                        <div class="flex justify-between">
                            <span class="text-slate-600">Créé le:</span>
                            <span class="font-medium text-slate-800">{{ $typeReunion->created_at->format('d/m/Y') }}</span>
                        </div>
                        @if($typeReunion->updated_at->gt($typeReunion->created_at))
                            <div class="flex justify-between">
                                <span class="text-slate-600">Modifié le:</span>
                                <span class="font-medium text-slate-800">{{ $typeReunion->updated_at->format('d/m/Y') }}</span>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="bg-white/80 rounded-2xl shadow-lg border border-white/20">
            <div class="p-6">
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    ...
                    <a href="{{ route('private.types-reunions.show', $typeReunion) }}" class="inline-flex items-center justify-center px-8 py-3 bg-slate-600 text-white font-medium rounded-xl hover:bg-slate-700 transition-colors">
...
function updatePreview() {
    const nom = document.getElementById('nom').value || '{{ $typeReunion->nom }}';
    const code = document.getElementById('code').value || '{{ $typeReunion->code }}';
    const icone = document.getElementById('icone').value || '{{ $typeReunion->icone ?? 'calendar-alt' }}';
    const couleur = document.getElementById('couleur').value || '{{ $typeReunion->couleur ?? '#3498db' }}';
    ...
    document.getElementById('preview-categorie').textContent = categorieSelect.options[categorieSelect.selectedIndex]?.text || '{{ ucfirst($typeReunion->categorie) }}';
    document.getElementById('preview-acces').textContent = accesSelect.options[accesSelect.selectedIndex]?.text || '{{ ucfirst(str_replace("_", " ", $typeReunion->niveau_acces)) }}';
    document.getElementById('preview-frequence').textContent = frequenceSelect.options[frequenceSelect.selectedIndex]?.text || '{{ ucfirst(str_replace("_", " ", $typeReunion->frequence_type)) }}';
    document.getElementById('preview-duree').textContent = dureeStandard || '{{ $typeReunion->duree_standard?->format("H:i") ?? "-" }}';
