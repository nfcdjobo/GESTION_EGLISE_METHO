<?php $__env->startSection('title', 'Membres - ' . $classe->nom); ?>
...
<div class="space-y-8">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-slate-600 bg-clip-text text-transparent">Membres de <?php echo e($classe->nom); ?></h1>
        ...
        <p class="text-slate-500 mt-2">Gestion des membres de la classe - <?php echo e($classe->membres->count()); ?> membre(s) inscrit(s)</p>
    </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div class="text-center p-4 bg-blue-50 rounded-xl">
                    <div class="text-2xl font-bold text-blue-600"><?php echo e($classe->membres->count()); ?></div>
                    <div class="text-sm text-blue-700 font-medium">Membres inscrits</div>
                </div>
                <div class="text-center p-4 bg-green-50 rounded-xl">
                    <div class="text-2xl font-bold text-green-600"><?php echo e(50 - $classe->membres->count()); ?></div>
                    <div class="text-sm text-green-700 font-medium">Places disponibles</div>
                </div>
                <div class="text-center p-4 bg-purple-50 rounded-xl">
                    <div class="text-2xl font-bold text-purple-600"><?php echo e(round(($classe->membres->count() / 50) * 100, 1)); ?>%</div>
                    <div class="text-sm text-purple-700 font-medium">Taux de remplissage</div>
                </div>
                <div class="text-center p-4 bg-amber-50 rounded-xl">
                    <div class="text-2xl font-bold text-amber-600"><?php echo e($classe->tranche_age ?: 'Non définie'); ?></div>
        ...
        <div class="p-6 border-b border-slate-200">
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-bold text-slate-800 flex items-center">
                    ...
                    Liste des Membres (<span id="memberCount"><?php echo e($classe->membres->count()); ?></span>)
                </h2>
                <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('classes.manage-members')): ?>
            ...
            <?php if($classe->membres->count() > 0): ?>
                <div id="membersContainer">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        <?php $__currentLoopData = $classe->membres; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $membre): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <div class="member-card bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors border border-slate-200 overflow-hidden" data-member-id="<?php echo e($membre->id); ?>">
                                <!-- Header avec checkbox et actions -->
                                <div class="p-4 pb-3">
                                    <div class="flex items-center justify-between mb-3">
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('classes.manage-members')): ?>
                                            <input type="checkbox" class="member-checkbox w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500" value="<?php echo e($membre->id); ?>">
                                        <?php else: ?>
                                            <div></div>
                                        <?php endif; ?>
                                        ...
                                        <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('classes.manage-members')): ?>
                                            <div class="relative">
                                                <button onclick="toggleMemberMenu('<?php echo e($membre->id); ?>')" class="p-1 text-slate-400 hover:text-slate-600 hover:bg-slate-200 rounded-lg transition-colors">
                                                    <i class="fas fa-ellipsis-v text-sm"></i>
                                                </button>
                                                <div id="memberMenu-<?php echo e($membre->id); ?>" class="hidden absolute right-0 top-8 w-40 bg-white rounded-lg shadow-lg border border-slate-200 z-20">
                                                    <div class="py-1">
                                                        <button onclick="viewMember('<?php echo e($membre->id); ?>')" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center">
                                                            <i class="fas fa-eye mr-2 text-blue-600"></i> Voir le profil
                                                        </button>
                                                        <button onclick="editMember('<?php echo e($membre->id); ?>')" class="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 flex items-center">
                                                            <i class="fas fa-edit mr-2 text-yellow-600"></i> Modifier
                                                        </button>
                                                        <div class="border-t border-slate-100"></div>
                                                        <button onclick="removeMember('<?php echo e($membre->id); ?>')" class="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center">
                                    ...
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div class="relative flex-shrink-0">
                                            <?php if($membre->photo_profil): ?>
                                                <img src="<?php echo e(asset('storage/' . $membre->photo_profil)); ?>" alt="<?php echo e($membre->nom_complet); ?>" class="w-12 h-12 rounded-full object-cover">
                                            <?php else: ?>
                                            ...
                                            <div class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white
                                                <?php if($membre->statut_membre === 'actif'): ?> bg-green-500
                                                <?php elseif($membre->statut_membre === 'inactif'): ?> bg-red-500
                                                <?php elseif($membre->statut_membre === 'visiteur'): ?> bg-yellow-500
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h4 class="font-semibold text-slate-900 truncate"><?php echo e($membre->nom_complet); ?></h4>
                                            <p class="text-sm text-slate-600 truncate"><?php echo e($membre->email); ?></p>
                                            <?php if($membre->telephone_1): ?>
                                                <p class="text-sm text-slate-500 truncate"><?php echo e($membre->telephone_1); ?></p>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="px-4 pb-4">
                                    ...
                                    <?php if($membre->date_naissance): ?>
                                        <div class="text-xs text-slate-400 mb-2">
                                            <?php echo e($membre->date_naissance->diffInYears(now())); ?> ans
                                            <?php if($membre->date_naissance->format('m-d') === now()->format('m-d')): ?>
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                            <?php if($membre->statut_membre === 'actif'): ?> bg-green-100 text-green-800
                                            <?php elseif($membre->statut_membre === 'inactif'): ?> bg-red-100 text-red-800
                                            <?php elseif($membre->statut_membre === 'visiteur'): ?> bg-yellow-100 text-yellow-800
                                            <?php else: ?> bg-blue-100 text-blue-800
                                            <?php endif; ?>">
                                            <?php echo e(ucfirst(str_replace('_', ' ', $membre->statut_membre))); ?>

                                        </span>

                                        <?php if($membre->statut_bapteme === 'baptise'): ?>
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                                <i class="fas fa-check mr-1"></i> Baptisé
                                            </span>
                                        <?php endif; ?>

                                        <?php if($membre->responsabilites && is_array($membre->responsabilites)): ?>
...
function loadAvailableClasses() {
    fetch("<?php echo e(route('private.classes.index')); ?>", {
        headers: {
            'Accept': 'application/json',
            'X-Requested-With': 'XMLHttpRequest'
        }
    ...
    .then(data => {
        ...
        if (data.success && data.data.data) {
            ...
            data.data.data.forEach(classe => {
                if (classe.id !== '<?php echo e($classe->id); ?>') {
...
document.getElementById('userSearch').addEventListener('input', function() {
    ...
    searchTimeout = setTimeout(() => {
        fetch(`<?php echo e(route('private.classes.utilisateurs-disponibles', $classe->id)); ?>?search=${encodeURIComponent(query)}`, {
            headers: {
                'Accept': 'application/json',
                'X-Requested-With': 'XMLHttpRequest'
            }
...
<?php echo $__env->make('layouts.private.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Dell\Desktop\MICRISERVICES\methodiste_belle_ville\resources\views/components/private/classes/membres.blade.php ENDPATH**/ ?>