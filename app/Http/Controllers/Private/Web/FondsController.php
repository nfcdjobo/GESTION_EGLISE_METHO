...
class FondsController extends Controller
{
    ...
    /**
     * Génération d'un reçu fiscal PDF pour téléchargement direct
     */
    public function generateReceipt(Request $request, Fonds $fonds, ReceiptService $receiptService)
    {
        ...
        try {
            ...
            $errors = $receiptService->canGenerateReceipt($fonds);
            if (!empty($errors)) {
                return $this->responseFormat(null, implode(', ', $errors), 400);
            }

            DB::beginTransaction();

            // Générer le numéro de reçu s'il n'existe pas
            if (empty($fonds->numero_recu)) {
            ...
            // Générer le reçu pour téléchargement direct (sans sauvegarde)
            $response = $receiptService->generateForDownload($fonds);

            DB::commit();

            Log::info('Reçu fiscal téléchargé', [
                'transaction_id' => $fonds->id,
    ...
    /**
     * Prévisualiser le reçu en HTML
     */
    public function previewReceipt(Fonds $fonds, ReceiptService $receiptService)
    {
        try {
            if (!$fonds->peutGenererRecu()) {
                return $this->responseFormat(null, 'Impossible de générer le reçu pour cette transaction.', 400);
            ...
            $html = $receiptService->generateHTML($fonds);
            ...
            dd($e->getMessage());
            Log::error('Erreur prévisualisation reçu', [
                'error' => $e->getMessage(),
                'fonds_id' => $fonds->id
            ]);

            return $this->responseFormat(null, 'Erreur lors de la prévisualisation.', 500);
        }
    }

    /**
     * Envoyer un reçu par email (génération temporaire)
     */
    public function sendReceiptByEmail(Request $request, Fonds $fonds, ReceiptService $receiptService)
    {
        try {
            $request->validate([
            ...
            $emailSent = $receiptService->sendByEmailDirect($fonds, $request->input('email'));

            if (!$emailSent) {
                return $this->responseFormat(null, 'Échec de l\'envoi de l\'email.', 500);
            }

            return $this->responseFormat(null, 'Reçu envoyé par email avec succès.');

        } catch (ValidationException $e) {
            dd($e->getMessage());
            return $this->responseFormat(null, 'Email invalide.', 422, $e->errors());
        } catch (Exception $e) {
            dd($e->getMessage());
            ...
            return $this->responseFormat(null, 'Erreur lors de l\'envoi de l\'email.', 500);
        }
    }

    /**
     * Afficher le formulaire de génération de reçu
     */
    public function showReceiptForm(Fonds $fonds)
    {
        try {
            $fonds->load(['donateur', 'culte', 'collecteur', 'validateur']);
            ...
            if (request()->expectsJson()) {
                return response()->json([
                    ...
                    'data' => [
                        'fonds' => $fonds,
                        'peut_generer_recu' => $fonds->peutGenererRecu(),
                        'informations_recu' => [
                            'numero_recu' => $fonds->numero_recu ?: 'Sera généré automatiquement',
                            'date_emission' => now()->format('d/m/Y'),
                            'donateur' => $fonds->nom_donateur,
                            'montant' => $fonds->montant_format
                        ]
                    ]
                ]);
            }

            return view('components.private.fonds.receipt-form', compact('fonds'));

        } catch (Exception $e) {
            dd($e->getMessage());
            Log::error('Erreur affichage formulaire reçu', [
                'error' => $e->getMessage(),
                'fonds_id' => $fonds->id
            ]);
    ...
    private function getEgliseInfo(): array
    {
        return [
            'nom' => config('church.name', 'ÉGLISE ÉVANGÉLIQUE DE RÉVEIL'),
            'adresse' => config('church.address', 'Cocody Angré 7ème Tranche, Rue L126 - Abidjan, Côte d\'Ivoire'),
            'telephone' => config('church.phone', '+225 27 22 45 67 89'),
            'email' => config('church.email', 'contact@eglise-revelation.ci'),
            'website' => config('church.website', 'www.eglise-revelation.ci'),
     ...
     */
    private function convertirMontantEnLettres($montant, $devise = 'XOF'): string
    {
        // Tableau des nombres en lettres
        ...
        $convertirCentaines = function ($nombre) use ($unites, $dizaines, $dix_a_dix_neuf) {
            $resultat = '';
            ...
            if ($reste >= 10 && $reste <= 19) {
                ...
                if ($dizaine > 0) {
                    if ($dizaine == 7 || $dizaine == 9) {
                        $resultat .= $dizaines[$dizaine - 1] . '-' . $dix_a_dix_neuf[$unite];
                    } else {
                        $resultat .= $dizaines[$dizaine];
                        if ($unite > 0) {
                            if ($dizaine == 8 && $unite == 1) {
                                $resultat .= '-un';
                            } else {
        ...
        if ($montantEntier == 0) {
            return 'zéro ' . ($devise == 'XOF' ? 'francs CFA' : $devise);
        }

        $resultat = '';

        // Millions
        $millions = intval($montantEntier / 1000000);
        if ($millions > 0) {
            if ($millions == 1) {
                $resultat .= 'un million';
            } else {
                $resultat .= $convertirCentaines($millions) . ' millions';
            }
            $montantEntier %= 1000000;
            if ($montantEntier > 0) {
                $resultat .= ' ';
            }
        }
        ...
        // Milliers
        $milliers = intval($montantEntier / 1000);
        if ($milliers > 0) {
            if ($milliers == 1) {
                $resultat .= 'mille';
            } else {
        ...
        if ($montantEntier > 0) {
            $resultat .= $convertirCentaines($montantEntier);
        }

        // Ajouter la devise
        if ($devise == 'XOF') {
            $resultat .= $montantEntier > 1 ? ' francs CFA' : ' franc CFA';
        } else {
            $resultat .= ' ' . $devise;
        }

        return ucfirst(trim($resultat));
    }
    ...
    private function calculerRegulariteDonateur($donateurId, $premierDon, $dernierDon)
    {
        try {
            ...
            if ($frequenceMoyenne <= 30)
                return 'tres_regulier';
            if ($frequenceMoyenne <= 90)
                return 'regulier';
            if ($frequenceMoyenne <= 180)
                return 'occasionnel';
    ...
    private function calculerScoreFidelite($donateur)
    {
        try {
            ...
            if ($donateur->total_dons > 100000)
                $score += 20;
            elseif ($donateur->total_dons > 50000)
                $score += 15;
            elseif ($donateur->total_dons > 20000)
                $score += 10;
        ...
        try {
            ...
            for ($mois = 1; $mois <= 12; $mois++) {
                ...
                if ($donneesHistoriques->count() >= 2) {
                    ...
                    $facteurSaisonnier = 1;
                    if (in_array($mois, [12, 4]))
                        $facteurSaisonnier = 1.2; // Noël, Pâques
                    if (in_array($mois, [7, 8]))
                        $facteurSaisonnier = 0.8; // Vacances
    ...
    private function recommanderObjectifs($historique)
    {
        try {
            ...
            for ($mois = 1; $mois <= 12; $mois++) {
                ...
                $facteurSaisonnier = 1;
                if (in_array($mois, [12, 4]))
                    $facteurSaisonnier = 1.15; // Noël, Pâques
                if (in_array($mois, [7, 8]))
                    $facteurSaisonnier = 0.85; // Vacances d'été
